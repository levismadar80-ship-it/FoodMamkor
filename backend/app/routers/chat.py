...
Design notes:
  ...
  - System prompt is in Hebrew. The bot persona is feminine, but it
    addresses users in neutral plural voice (MEH-940), scoped tightly to
    the site so the bot doesn't drift into general-purpose chitchat.
...
# Hebrew system prompt — scoped tightly to the site. The bot persona is
# feminine, but it addresses users in neutral plural voice (MEH-940) per
# the brand voice ADR / COPY_BANK.
#
...
**מה זה מהמקור**
מהמקור זה אתר שמחבר בין אנשים שמחפשים אוכל אמיתי ומקומי לבין בתי עסק מקומיים — בשר במרעה, לחם מחמצת, חלב גולמי, ירקות אורגניים ועוד. אפשר לחפש לפי קטגוריה, לפי עיר, או על המפה לפי קרבה אליכם, ולדבר ישירות עם בית העסק דרך WhatsApp.

**האם האתר בחינם**
כן, לגמרי חינם — גם לגולשים שמחפשים אוכל, וגם לבתי עסק שרוצים להירשם ולהופיע באתר. בתי עסק יכולים לבחור לשדרג לתוכנית פרמיום בהמשך (כדי להופיע בראש הרשימה), אבל זה לגמרי אופציונלי — ההרשמה הרגילה חינמית ומספיקה.
...
**עוד נושאים שגולשים שואלים**
- **כמה זמן לוקח האישור של העסק?** — אחרי שבית עסק ממלא את טופס ההרשמה, הצוות שלנו בודק את הפרטים ובדרך כלל מאשר את העסק שלו תוך יום-יומיים. מרגע האישור העסק מופיע באתר וגולשים יכולים למצוא אותו במפה ובחיפוש.
- **איך יוצרים קשר עם בית עסק?** — בכל עמוד של עסק יש כפתור WhatsApp שפותח שיחה ישירה עם בית העסק. אפשר גם לראות את הטלפון, האינסטגרם והאתר אם הוסיפו אותם.
- **איך מדווחים על בעיה?** — בכל עמוד של עסק או מוצר יש כפתור "דווח על עסק". אפשר גם לפנות אלינו ישירות דרך טופס יצירת הקשר שבדף "אודות" — הצוות שלנו יטפל בפנייה בהקדם.

**שימי לב**
- אם נשאלת על משהו שלא כתוב כאן (תקלה טכנית, החזר כספי, בעיה עם בית עסק) — הפני אותם לטופס יצירת הקשר שבדף "אודות" או דרך הלינק בתחתית האתר.
- אל תתחזי לבית עסק או לצוות האתר — את עוזרת AI.

**פורמט התשובה**
ענה בטקסט פשוט בלבד. אל תשתמשי ב-markdown, כוכביות (`**`), bold, כותרות עם `#`, רשימות עם `-` או `*`, או כל סימן פורמט אחר. כתבי כמו שמדברים בצ'אט — רק המילים עצמן."""
...
def chat(request: Request, body: ChatRequest) -> ChatResponse:
    ...
    if client is None:
        ...
        return ChatResponse(
            reply="העוזרת לא זמינה כרגע 🌿 נסו שוב בעוד כמה רגעים, או פנו אלינו דרך טופס יצירת הקשר באודות.",
        )
    ...
    try:
        ...
        return ChatResponse(
            reply="משהו השתבש בצד שלי 🌱 נסו שוב בעוד רגע. אם זה נמשך — דווחו לנו דרך טופס יצירת הקשר.",
        )

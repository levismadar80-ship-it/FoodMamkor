...
prompt: Review the PR diff. Backend: backend/app/services/email.py:42 —
  ...
  CHANGELOG entry: "Fixed email delivery flakiness (MEH-XX)." HANDOFF
...
prompt: Review the PR diff for MEH-361. Two files: bio_generator.py:125,
  ...
  msg.content[0].text. CHANGELOG entry quotes the exact pattern from
  chat.py:246. HANDOFF lists 4 content shapes verified statically
  (typical, tool-then-text, empty, no-text). git log -5 shows 78fabef
  is current staging tip — matches branch base.

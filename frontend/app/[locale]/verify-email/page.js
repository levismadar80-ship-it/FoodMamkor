...
// VerifyEmailClient.jsx keeps the "use client" token flow (useSearchParams
// inside its own <Suspense>) untouched — same shape as /search.
export async function generateMetadata({ params }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "seo.verify_email" });

  return {
    // title.absolute prevents layout's `%s | ${BRAND_NAME}` template appending
    // (seo.verify_email.title already includes the brand suffix).

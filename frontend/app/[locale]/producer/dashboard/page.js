...
/**
 * Producer dashboard — Overview (hub) index.
 *
 * MEH-964 Phase 1 (chunk 1A): this page is now the slim Overview index under
 * the shared dashboard layout (layout.js — tab bar + auth gate). The edit
 * forms, deep analytics, and quick links were RELOCATED VERBATIM to nested
 * spoke routes (edit/ · insights/ · tools/) — relocate-don't-rewrite.
 *
 * The Overview keeps: greeting, status banners, holiday hint, vanity link,
 * availability card, the completeness card, and the top-line 4-KPI hero.
 *
 * NOTE (MEH-964 1A): the top-line 4-KPI hero stays HERE (the KPIs belong to
 * the Overview); only the DEEP analytics (windowed cards + charts) moved to
 * insights/. The hero is rebuilt to the locked RTL design in chunk 1B — for
 * 1A it is the existing strip, relocated as-is. Do NOT also render these KPIs
 * in insights/ — that would recreate the MEH-961/MEH-963 stale-duplicate bug.
 *
 * Fetches in parallel from two endpoints so the UI can render the hero
 * (availability toggle) immediately even if analytics is slow:
 *   - GET /producers/me/dashboard  — producer meta + legacy fields
 *   - GET /producers/me/analytics  — rich metrics + 30d chart + top cities
 *   - GET /producers/me            — full profile (completeness fields)
 */

...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: buildAlternates("/about/for-businesses/guides/product-photography", locale),
  };
}
...
export default async function ProductPhotographyGuidePage({ params }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: "guides.product_photography" });
  return (
    <GuideArticle
      title={t("title")}
      subtitle={t("subtitle")}

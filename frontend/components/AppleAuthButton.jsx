...
export default function AppleAuthButton({ onSuccess, onError }) {
  ...
  useEffect(() => {
    ...
    const handleAppleAuth = async (event) => {
      try {
        ...
        onError?.(err.response?.data?.detail || "שגיאה בהתחברות עם Apple");
      }
    };
  ...
  return (
    <button
      ...
      className="w-full bg-black text-white py-3 rounded-[12px] hover:bg-gray-900 transition font-medium flex items-center justify-center gap-2 mt-3"
    >

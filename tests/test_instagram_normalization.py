...
MEH-1616 extends the same helper to the outreach_leads table: the admin list
...
# Shared by the helper matrix and the per-schema matrices below, so a case
# added here is automatically enforced on every schema that writes the column.
_NORMALIZER_CASES = [
...
# --- MEH-1616: outreach_leads.instagram, same helper, same matrix ---
#
# OutreachLeadOut / OutreachPrefillResponse are deliberately NOT wired: they
# are read schemas, and normalizing on read would mask which stored rows are
# still dirty (the count query in the ticket depends on reading them raw).
_OUTREACH_WRITE_SCHEMAS = [
    pytest.param(
        lambda ig: OutreachLeadCreate(name="חוות הניסוי", instagram=ig),
        id="OutreachLeadCreate",
    ),
    pytest.param(
        lambda ig: OutreachLeadUpdate(instagram=ig),
        id="OutreachLeadUpdate",
    ),
]


@pytest.mark.parametrize("schema_factory", _OUTREACH_WRITE_SCHEMAS)
@pytest.mark.parametrize(("raw", "expected"), _NORMALIZER_CASES)
def test_outreach_write_schemas_normalize(schema_factory, raw, expected):
    """The full MEH-1608 matrix, re-run against both outreach write schemas.

    The admin list composes the profile link from the stored value exactly as
    the public ContactCard does (admin/outreach/page.jsx:223), so the same
    inputs must produce the same canonical handle in this table too.
    """
    assert schema_factory(raw).instagram == expected


def test_outreach_create_max_length_applies_to_raw_input():
    """ACKNOWLEDGED boundary: OutreachLeadCreate.instagram carries
    max_length=100 (matching models.py String(100)), and the validator runs in
    the default "after" mode — so the cap is enforced against the RAW input,
    not the normalized handle. A >100-char paste is rejected rather than
    normalized.

    "before" mode would move the cap onto the normalized value, but it also
    hands non-str input straight to .strip() — raising a bare AttributeError
    (a 500) instead of a clean ValidationError. Measured: a realistic share
    URL with an igsh token is 87 chars, comfortably under the cap, so "after"
    is the correct trade. Documented rather than silently accepted.
    """
    realistic = "https://www.instagram.com/maafiat_hasade_zichron/?igsh=MzRlODBiNWFlZDFlNiZhbXA9MQ%3D%3D"
    assert len(realistic) < 100
    assert OutreachLeadCreate(name="חוות הניסוי", instagram=realistic).instagram == (
        "maafiat_hasade_zichron"
    )

...
## 2026-06-25/26 — MEH-784 settings/hooks Windows-safe + bypass close + drift guard

- **Branch:** `feature/meh-784-settings-hooks-windows` (off `staging`), commit `99f74b63`, **pushed**. PR open **BLOCKED on GitHub API rate limit** — run `gh pr create --draft --base staging --head feature/meh-784-settings-hooks-windows --title "MEH-784: Windows-safe hooks + close Bash-write bypass + guardrail drift guard" --body-file docs/audits/meh-784/pr-body.md` when the limit resets (or open from the web URL printed on push).
- **Forensics:** the 2026-06-07 `.claude/settings.json` hooks-strip was a *pre-existing uncommitted* change (session `fcd15bf2`), not a tool call; **re-stripped between the two restores** (blob `16ab7c8→c9e8f18`); restored 14:37 UTC. Likely vector: **OneDrive-synced repo path** resurrecting a cached stripped copy (hypothesis). `stash@{0}` is unrelated (a pr-checks.yml WIP).
- **3 hook fixes (verified on MINGW, no deletions):** `subagent-trace.sh` (MSYS2 pure-`?` jq-arg crash → exit 0 + `--argjson` + `|| true`); `check-bash-safety.sh` (block `cp/mv/tee/dd/install/ln/sed -i`/redirect to 6 deny-protected path classes — closes the live `>> settings.json` bypass; reads + git exempt); `session-start.sh` (drift warning when settings.json/hooks differ from HEAD).
- **Constraint hit:** `.claude/hooks/**` + `settings.json` are AI-`Edit`/`Write`-denied by design. Fixes applied via **one-time Sapir-authorized Bash-write**; change (2) then self-closed that hole. `settings.json` untouched (drift guard rides in `session-start.sh`).
- **Known limitation:** new guard inspects the command string → over-blocks commands that merely *quote* a protected path + write indicator (hit on `gh pr create`; used `--body-file`). Candidate follow-up: exempt `gh` or match only real redirect targets. Re-editing the hook now needs another authorized apply (the guard self-protects).
- **Cleanup:** `docs/audits/meh-784/` (staged copies + pr-body.md) is untracked — safe to delete; `rm`/`git clean` are deny-listed so left in place. The MEH-773 mid-flight changes (pr-checks EXPECTED_REV + uv.lock) are parked in `git stash` (`stash@{0}` on the meh-773 branch).

...
- **Done:** wrapped `:339-340` in `{step < STEP.CONFIRM && (<>…</>)}`, mirroring the 3 existing guards. Logic-guard only; **zero copy/key/state change**. 1 file (`RegisterProducerClient.jsx`), +9/−2. `npm run build` green.
- **Pending:** open draft PR off this branch → Sapir mobile-QA at 375px on staging (hero hidden on CONFIRM, success heading retained, steps 1-4 unchanged) → Sapir merges (Rule 23 — visible UI change).

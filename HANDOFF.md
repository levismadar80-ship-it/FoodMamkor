...
- **Batch status (updated 22/07):** MEH-1437 **Done** (Sapir ran the queries: vegan+gf = 2 products >0, lf = 0 — gate passed). MEH-1439 shipped (this entry). **MEH-1438 (vegetarian flag) IN PROGRESS** on `feature/meh-1438-vegetarian-flag`: Chunk A (model + migration draft) committed, migration `c5d9f3a1b2e8` applied to staging by Sapir; Chunk B (schemas + `producer_listing` `is_vegetarian OR is_vegan` filter + `has_vegetarian_products` aggregation + pytest) committed. **Open scope-gap:** the `?vegetarian` endpoint param needs 2 lines in `routers/producers.py` (not in MEH-1438 file_locations) — awaiting Sapir approval before Chunk C. No auto-merge (HIGH-RISK).

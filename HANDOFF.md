...
> Last updated: 2026-05-20 (**MEH-475 settings sweep S1** — PR #755 wires settings chrome + ProfileTab into new `settings.common.*` (8 keys) + `settings.profile.*` (21 keys) namespaces. 15 source strings → 29 keys. Scanner residual on `settings/page.jsx` 103→88. **STOP at S1/S2 chunk boundary per cumulative-runtime guardrail.** S2 SecurityTab (28 strings, HIGH-RISK auth surface — PasswordChangeCard + LogoutAllDevicesCard + DangerZoneCard) + S3a BusinessTab (~10 strings) + S3b ProductsSection (~50 strings) DEFERRED to next session. MEH-629 #2 fix at L377-492 verified intact. Auth flow handlers (api.post / api.patch / form onSubmit) untouched in S1.)
...
- 🟡 **settings/page.jsx sweep — S1 DONE, S2/S3 PENDING.** PR #755 at `2b5bd18` wired chrome + ProfileTab (15→29 keys); scanner residual 103→88. Remaining chunks deferred to next session per cumulative-runtime guardrail:
  - **S2 SecurityTab (28 strings, HIGH-RISK)** — PasswordChangeCard (L380-548), LogoutAllDevicesCard (L554-613), DangerZoneCard (L619-716). All auth-sensitive: password change form (api.patch /users/me/password), logout-all-devices (auth context), account deletion (api.delete via deleteAccount). Each component is a top-level card with its own form + state.
  - **S3a BusinessTab core (~10 strings)** — Producer profile fields at L716-808.
  - **S3b ProductsSection (~50 strings)** — Product CRUD at L809-1320, includes image upload + product add/edit/delete with optimistic updates. Largest single sub-component.

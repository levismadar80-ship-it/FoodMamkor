...
## 2026-07-26 — MEH-1542 lang-label + MEH-1541 established_year (sequential)

- **MEH-1542** (LOW-RISK copy, `feature/meh-1542-lang-label`, `Closes MEH-1542`, **PR #2146 MERGED**): AccountSheet language-toggle label `עב / EN` → full native names `עברית / English` (W3C/NN-g best practice, no flags). One hardcoded string in `AccountSheet.jsx:195` (not an i18n key); `dir="ltr"` + order logic untouched, `LanguageToggle.jsx` untouched. build green; Playwright self-QA 375px of the open sheet (`qa-artifacts/MEH-1542/accountsheet-375.webp`). The "CI gate failed" webhook was the **stale draft-run gate** (superseded-run false-failure, rule 21) — the ready-flip's fresh run went green and auto-merged.
- **MEH-1541** (YELLOW, `feature/meh-1541-established-year`, `Closes MEH-1541`, **PR #2166**): `producers.established_year` heritage line end-to-end. Migration `e4a9c1f7b2d3` (chains onto head `d7b2f4a9c6e1`) was added to the branch by Sapir via the GitHub web UI (PR #2167 merged into the branch); I verified it byte-identical to my provided file, then ran `alembic upgrade head` (full chain green) + `alembic check` ("No new upgrade operations detected" — no drift) locally against Postgres 16. Backend: nullable INTEGER col, `ProducerUpdate` range validator (1800..current Israel-tz year → 422 "שנת ההקמה לא תקינה"; nullable → null clears), `ProducerDetailOut` serialize (public; Admin/Owner inherit), `established_year` in `_PRODUCER_WRITABLE_FIELDS` (admin PUT auto-covered by bulk setattr). Frontend: `OwnerStoryCard` number field (same PUT, surfaces the 422), admin `ProducerForm.jsx` field, `ProducerHeader.jsx` quiet "מאז {שנה}" masthead line (DM Sans, fg-muted, numerals `dir="ltr"`, null-safe → absent from DOM). i18n he+en twins (English values, not mirror).
- **Verify (MEH-1541):** `pytest tests/test_meh_1541_established_year.py` **8 passed** + `tests/test_api.py` **256p/4s** (real Postgres) · full vitest **1626p/10s** (3 `EditTabOwnerStoryCard` PUT-shape assertions updated for the new field + 2 new tests) · `npm run build` green · Playwright self-QA @375+@1440 of all three surfaces WITH year (1940) + masthead WITHOUT (line absent) — `qa-artifacts/MEH-1541/`, 172 KB webp (8 shots). Docs updated: DATA.md, `.ai/diagrams/db-schema.md`, CHANGELOG. Auto-merge enabled on green CI. **DoD:** preview-first mobile QA waived per the ticket's YELLOW grant (Sapir approves outcome, not preview).
- **Process note:** the Alembic STOP gate was lifted by Sapir mid-task (MEH-836 removed the `alembic/versions/**` deny + Dockerfile auto-applies `upgrade head` on Railway boot), so this ran Chunk 0→2 end-to-end rather than pausing for a manual apply.

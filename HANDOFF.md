...
> Last updated: 2026-04-30 (MEH-306 sub-B — password policy wire-up frontend, draft PR pending; both A + B ready for review-cycle merge)
...
Status: **DRAFT** PR pending. Holds Ready-for-Review until A + B
review together.
...
- (this commit) docs — CHANGELOG + HANDOFF
...
**Merge order (unchanged from sub-A handoff):**
1. CI green on both A + B.
2. Smadar reviews both, flips both Ready-for-Review.
3. **A merges to staging FIRST**, B merges immediately after — same
   review cycle. Avoids a window where the API rejects 8-char passwords
   that the frontend still allows.

...
- **Phase-0 (file:line):** map fetches via `useProducersFeed` → `GET /producers` with NO lat/lng/radius_km (non-geo path) + client-side distance filter (`MapClient.jsx:26-31`, `useProducersFeed.js:42-45`) → never enters geo mode → the geo-only filter change can't reach it. **MapClient.jsx untouched** (spec's "separate path, leave it & note it" case). `home.producers.geo_empty` already exists (he:454, en:440) — no new key.

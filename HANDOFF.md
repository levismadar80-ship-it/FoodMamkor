...
## 2026-07-26 — MEH-1553 email_verified creation-paths fix + pinning test

- **MEH-1553** (auth = HIGH-RISK, but **end-to-end authority per הכרעת ספיר 26/07** — Phase 0 → Phase 1 → self-QA → merge, no WAIT between phases; `feature/meh-1553-email-verified-creation-paths`, `Closes MEH-1553`): closed a class of relying on the `email_verified` **column default (False)** for a security-relevant flag instead of setting it explicitly. **Exactly 3 files.**
- **The fixes:** (1) `auth.py::register_producer_oauth` new-user `User(**kwargs)` (`auth.py:919`) now sets `email_verified=True`, one-to-one with the `/auth/google` (`auth.py:781`) + `/auth/apple` (`auth.py:1107`) siblings — without it an OAuth business owner fell to default False, saw the verify banner, and was blocked by `require_verified_producer` (MEH-170 gap → MEH-1164 money-path blocker). (2) `scripts/create_admin.py` both branches (existing→upgrade + new) set `email_verified=True`. (3) misleading upgrade-path comment (`auth.py:553`) corrected: verification is **not enforced** here (guard only needs a valid JWT); unverified state is covered downstream by the banner + `require_verified_producer`.
- **Pinning test** `tests/test_email_verified_creation_paths.py` — **placed at repo-root `tests/`, NOT the spec's `backend/tests/`** which doesn't exist (repo convention per `tests/CLAUDE.md`; noted in PR). 6 paths pinned: register-password→False+token · register/producer non-upgrade→False+token · google new→True · apple new→True · **register/producer/oauth new→True (central regression pin)** · create_admin both branches→True. Reuses `tests/test_producer_oauth.py` monkeypatch pattern; `create_admin` loaded via importlib + `main()` with injected argv.
- **Phase 0 `User(` audit (grep, in PR body):** 6 sites — `auth.py:304`/`652` correct False+token (password flows), `auth.py:781`/`1107` correct True (OAuth siblings), `auth.py:919` **fixed** True, `create_admin.py:67` **fixed** True; `models.py:359` is the class def (N/A). **No out-of-scope site with a wrong flag → STOP-(a) never fired.**
- **DoD exception (per ticket):** backend + script + tests only, no UI → "נבדק בנייד" + preview waived. **No schema / Alembic / env / data backfill.** Verify: new test 8/8 · ruff `check .`+`format --check` (backend, CI-scoped) green. **Manual (Sapir, RED):** `UPDATE users SET email_verified = true WHERE role = 'admin'` in prod.

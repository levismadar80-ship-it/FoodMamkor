...
## 2026-07-13 — MEH-991 Chunk 3: VRT baseline refresh (footer /share link) — PR #1711 open, ready for review

- **Branch:** `feature/meh-991-vrt-baseline-refresh` off `origin/staging` (`0626301`). **PR #1711, ready for review, NO auto-merge** (standing batch rule: every PR gets an explicit human look; auto-merge armed only on a pointable written approval).
- **What:** 6 stale `parity.spec.ts` baselines regenerated — `/about`, `/login`, `/register` × desktop/mobile.
- **Careful-regen process (as required):** (1) ran failing specs + classified each diff, (2) reported the intended-vs-regression table, (3) waited for explicit `go`, (4) regenerated ONLY on confirmation via `vrt-update.yml` (run #43, success) — the environment-faithful runner path, never a sandbox `--update-snapshots`.
- **Root cause of the drift (verified before regen):** MEH-1160 (#1689) added the "ספרו עלינו" → `/share` footer link — an 8th footer nav entry that grows every `fullPage` route. `/login` + `/register` also carry MEH-1145/MEH-1128 `ui/Input` adoption (invisible at rest). All intended, no regression.
- **Verification:** bot commit `174769a` changed **exactly** the 6 predicted baselines and nothing else — home/map/producer-detail regenerated byte-identical → not committed. Strongest confirmation the classification was right.
- **Gate note:** the bot's `GITHUB_TOKEN` push doesn't trigger `pr-checks`/`deploy` — this docs commit (pushed as myself) fires the required `CI gate` + `Deploy gate` against the fresh baselines.
- **Next:** await required gates green + human review of the baseline images → Sapir merges. Do NOT arm auto-merge.

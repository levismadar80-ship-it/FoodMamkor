...
- **Auto-review (PR #1490) applied:** dialog += `role="dialog"`/`aria-modal`/Escape + Cancel-disabled-while-deleting (a11y parity with Chunk A); new `AdminContentCategoryDelete.test.jsx` (4 cases). Skipped: focus-trap (pre-existing, shared with `users/page.js` — follow-up candidate) + "drop MEH anchor" (conflicts code-exec §15).

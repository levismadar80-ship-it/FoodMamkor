...
## 2026-07-14 — MEH-1171 conversion — checkpoints 7-16 done, LOOP PAUSED for scope decision (draft PR #1729)

- **16 sections converted** (CP2-16), all green ×2 locally + CI-clean (only E2E red is the out-of-scope F14 producer-detail axe+parity). New specs under `frontend/e2e/flows/manual/`: legal-pages, map-legend, map-near-me, city-search, register-wizard-upgrade, home-filter-chips, smart-search, content-pages (/about/process + /join + /share + footer + /join & process remainders), producer-detail-cta (F14 fixme), producer-detail-state, admin-status-labels, admin-pages-load, about-process-detail + pytest cluster `tests/test_manual_backend.py`. Reusable infra: `scripts/local-backend.sh` §4b/§4c/§4d fixtures; admin-auth (SMOKE_ADMIN + browser /api/auth/login) pattern.
- **NEW FINDING F14** (matrix §Findings) — a real serious-a11y bug: `frontend/app/[locale]/producer/[id]/components/ProducerSections.jsx:155-156` (and `components/ProducerCard.jsx:238`) render `<div aria-label=…>` on a roleless div → axe `aria-prohibited-attr` on the missing-image placeholder. Explains the whole CI producer-detail cluster (axe + 40% VRT diff + sticky-bar). Trivial fix (role="img" / real <img alt> + VRT baseline regen) — Sapir's call, out of MEH-1171 scope.
- **⏸ LOOP PAUSED — scope decision needed:** 16 checkpoints covered all the highest-value flagged clusters; the remaining CONVERT-PW is a long tail (~hundreds of niche/fragile/auth-write rows) that won't converge in an autonomous loop. **Decision for Sapir:** (a) greenlight the MANUAL_TESTING.md rewrite for what's converted (pointer stubs + Tier-3 checklist + delete 138 STALE + KEEP-RUNBOOK verbatim + fix F11 :2215/:2273/:784-788), then flip #1729 ready; OR (b) name which specific remaining sections matter and resume conversion. Live anchor: `docs/qa/conversion-progress.md`. Also still pending: Sapir's **F6/F7** resolutions on MEH-1176 (checked every wake through CP16 — none yet).

## 2026-07-13 — MEH-1171 Ticket A conversion — checkpoints 7-12 (draft PR #1729, in progress)

- **Branch:** `feature/meh-1171-manual-testing-conversion` (draft PR **#1729**). Matrix approved by Sapir 13/07. Live checkpoint anchor = `docs/qa/conversion-progress.md` (updated + committed every section — read it first on resume). Zero app-code edits, zero edits to existing test files, all new specs green ×2 locally against the live local stack (`scripts/local-backend.sh` + `npm run start`).
- **Converted this session (5 new specs under `frontend/e2e/flows/manual/`, all pushed):** CP7 `register-wizard-upgrade` (MEH-853 upgrade submit body + auth-lands-on-DETAILS + city ✕) · CP8 `home-filter-chips` (chip → live GET /api/producers param + aria-pressed, multi-select AND) · CP9 `smart-search` (recent/trending dropdowns, ≥2-char debounce guard, grouped autocomplete, keyboard nav, recent-persist, results/empty states) · CP10 `content-pages` (/about/process + /join + /share + footer links, batched) · CP11 `producer-detail-cta` (mobile StickyContactBar scroll in/out + desktop single-CTA). Earlier session: CP2-6 (legal-pages, map-legend, map-near-me, city-search, pytest cluster `tests/test_manual_backend.py`).
- **CI:** CP7 CI-green (137 passed on a2bb613). The producer-detail VRT baseline was **regenerated on staging (d0560b44..1549dfe4) and merged into this branch at CP10** — the standing `parity.spec.ts:127` E2E red should now clear (verify next E2E run). `Playwright E2E (Vercel preview)` is NON-required regardless.
- **Reusable gotchas documented in the checkpoint file:** two-instance responsive mounts (hero-search, share actions, MapPane) → always `.locator("visible=true")`; Next.js RSC route-prefetches to `/producers?...&_rsc=` must not be mistaken for the axios `/api/producers` call; transform-hidden elements need `toBeInViewport()` not `isVisible()`; verbatim-copy checklist drift logged as doc-stale notes, never test-weakening.
- **CP12 (added):** `producer-detail-state` — MEH-291 availability banners (on_vacation / full_this_week) + MEH-51 kashrut strip, DATA-DRIVEN find-or-skip via `/api/producers?availability_state=`; `local-backend.sh` §4c seeds the state fixtures; `gotoWithMarker()` rides out the `[slug]` page's 60s ISR window after a local re-seed.
- **Next (resume here):** continue CONVERT-PW sections in matrix order (navbar MEH-731 states are fragile/borderline-visual — needs class assertions or fixme; MEH-294 status labels need admin auth). Then the **MANUAL_TESTING.md rewrite** (pointer stubs MEH-671 pattern + unified Tier-3 checklist + delete 138 approved STALE + KEEP-RUNBOOK verbatim), then full suite green ×2 pasted, then flip #1729 draft→ready with CHANGELOG+HANDOFF. **Watch Linear MEH-1176 for Sapir's F6/F7 resolutions — they take precedence** (checked at CP12 resume: no new resolution yet).

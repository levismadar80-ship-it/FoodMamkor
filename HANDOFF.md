...
- **MEH-1551** (YELLOW, frontend-only, `feature/meh-1551-contactcard-degenerate-states`, `Closes MEH-1551`): two ContactCard edge states from Sapir's 26/07 screenshot ("מאפיית רוח השדה"). **(1)** `channels.length === 1` used to render a lone unlabeled 44px circle (`ContactCard.jsx:144`, MEH-1334 assumed 3-6) → now one full-width **labeled quiet row** (icon + Hebrew label, label is the accessible name, no `aria-label`); new i18n `producer.detail.contact_card.single.{key}` (7+7 he/en). **(2)** the `:223-231` reveal pill was a **sibling below** the circle row → deleted; the desktop reveal (`matchMedia >=1024px`) now swaps the phone affordance **in place** (same flex slot, `revealed-phone` testid) via a shared `onChannelClick` handler that serves both the circle and the single-row. `trackContactClick`/`armMailtoFallback` unchanged. **The `>=2` circle branch is byte-identical** (acceptance criterion — verified in the diff + the four-channel self-QA). build 0 · full vitest **1630p/10s** (+6 `ContactCardStates.test.jsx`; `ContactCardEmailFallback.test.jsx` selector updated — email is the sole channel there, same behavior) · grep gate `phoneRevealed && producer.phone` sibling = **0** · Playwright self-QA @375+@1440 via the MEH-1497 route-mock (`next start`, no backend — middleware `producerExists` fails-open on unreachable fetch, so any UUID reaches the page and the client `/api/producers/{id}` fetch is `page.route`-mocked): single row · 4-channel regression · reveal-in-place (`qa-artifacts/MEH-1551/`, 92 KB webp). Frontend-only → pytest unaffected.
- **MEH-1554** (GREEN docs-only) + **MEH-1552** (GREEN tests-only, blocked-by 1551 merged; single Sapir gate = `vrt-update.yml` dispatch) — IN PROGRESS this session, see the end-of-session note when written.
- **Branch/authority:** harness assigned `claude/meh-1551-1554-1552-ly26k9` (violates the `claude/*` ban + branch-name gate, workflow rule 3) → used the tickets' own `feature/meh-*` branches, one per ticket, each cut off fresh `origin/staging`. ADR-016 end-to-end authority granted by Sapir 26/07: execute → self-QA → auto-merge on green (except the one MEH-1552 Sapir gate).

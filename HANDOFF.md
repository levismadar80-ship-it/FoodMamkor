...
## 2026-07-13 — MEH-1190…1196 sweep — MEH-1191 done (stacked on MEH-1190)

- **MEH-1191 (High · YELLOW):** `feature/meh-1191-whatsapp-optin-requires-phone` **branched off `feature/meh-1190-...`** (needs `ProfileUpdate.phone`; 1190 not yet merged — bot can't merge, see blocker note below). Phase-0 confirmed `ProfileUpdate.phone` present (schemas.py:1949). WhatsApp opt-in toggle no longer saves a silent no-op: no-phone → inline just-in-time phone field (PATCH /users/me, `validateIsraeliPhone` gates CTA) → toggle flips ON + persists `whatsapp_opt_in=true` same interaction (`save({whatsapp_opt_in:true})` override); cancel/empty → stays OFF. Backend `PUT /alerts` 422s on opt-in+no-phone (`:194` guard STAYS). Locked copy `phone_required_prompt`/`phone_save_cta` (he+en); cancel reuses `close_aria`. pytest `test_alerts_optin.py` ×3. Build+parity green; pytest deferred to CI. PW QA 375+1440 (`qa-artifacts/MEH-1191/`) confirms field-appears + PUT `whatsapp_opt_in:true` + with-phone immediate flip.
- **⚠ MERGE BLOCKER (whole sweep):** bot merge on #1735 returned 405 — required ruleset check **"Playwright E2E (Vercel preview)" is failing**. Feature branches get **no Vercel preview** (MEH-1044) → the E2E job falls back to a local `next start` with **no backend** → API-dependent specs fail on every feature PR (fails on 1190's settings-only diff too, so not a regression). Bot **cannot** auto-merge any sweep ticket; matches the repo's "merge stays Sapir-only" note. All PRs are green on the gates runnable here (build/vitest/parity) + full local QA. **Sapir merges** (1190 → 1191 → 1193 → 1194 → 1196; 1195 stays DO-NOT-MERGE).

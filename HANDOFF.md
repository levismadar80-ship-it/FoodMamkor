...
## 2026-07-17 — MEH-1258 license card + MEH-1260 kashrut expiry (batch 1258→1260→1259, full authority)

- **MEH-1258 — MERGED (PR #1814, squash `57c82c22`):** new `LicenseCard` on `/producer/dashboard/edit` (anchor `#license`, mounted right after Categories — spec said "between Categories and Images" but the MEH-1132 funnel renders Images first; deviation documented in the PR). Mirrors the admin `ProducerLicenseField` (numeric, `dir="ltr"`, maxLength 20, non-blocking amber format warning — copy REUSED from `admin.producers.form.fields.license_format_warning`); license-required hint via `requiresProducerLicense(profile.categories)`; save `PUT /producers/me { producer_license_number }` (whitelisted since MEH-530); MEH-999 2c clear-guard stays server-side, Hebrew 422 inline via `detailToMessage`; masked `•••`+last-4 header chip. NEW `EditTabLicenseCard.test.jsx` (6). vitest 1127 green (incl. en-parity), build green, 375px self-QA `qa-artifacts/MEH-1258/edit-license-375.webp` (real page via `next start` + stubbed API; `#license` deep-link auto-opens). **Sapir: mobile QA on staging** — https://staging.mehamakor.online/he/producer/dashboard/edit#license
- **MEH-1260 — kashrut expiry enforcement (this branch `feature/meh-1260-kashrut-expiry-enforcement`):** expired cert no longer earns the "כשר" badge (`badges.js` kosher case), no longer passes `?kosher=true` and lands in `?kosher=false` (`producer_listing.py`, naive-utcnow to match `admin_kashrut.py:73`), and `KashrutBadgeStrip` hides entirely when expired. Legacy NULL `expires_at` rows stay valid. NO schema change — `kashrut_expires_at` already serializes on `ProducerListOut` (schemas.py:804), so the STOP-e condition didn't trigger. 3 new pytest cases + 1 new vitest case.
- **Env note:** backend pytest not runnable in this sandbox (no postgres) — verified by CI's Backend tests leg per the MEH-1235-1239 precedent. Backend deps also not installed (pip denied).
- **Next:** MEH-1259 (hide unverified organic badge, MEH-986 pattern) after 1260 merges — both edit `badges.js`, sequential by design.

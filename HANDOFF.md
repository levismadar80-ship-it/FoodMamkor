...
## 2026-07-26 — MEH-1537 contact-field validation parity (YELLOW)

- **MEH-1537** (`feature/meh-1537-contact-validation-parity` off fresh `origin/staging`, `Closes MEH-1537`): server = source-of-truth for `contact_email`/`phone`/`whatsapp_group` format on every Producer write path. **Phase-0 correction (surfaced, meta-pattern #1):** the ticket's "ProducerUpdate.contact_email is plain str" evidence is **stale** — it's already `EmailStr` on all three write schemas (Register:207, AdminCreate:731, Update:860). Real gaps closed: `phone` had zero format validation (4 schemas), `whatsapp_group` no scheme/host guard, and `EmailStr` 422s `""` (dashboard sends `""` for a cleared field). **No STOP triggered** — no router bypasses the schema (owner+admin PUT both go through `ProducerUpdate`), and `wa.me` builders re-normalise via `normalizePhone` (`lib/utils.js:115`) so a separator-stripped stored phone is safe. Three **shared** validators in `schemas/schemas.py` (`_phone_validator` audit-regex `^(972)?0?[0-9]{8,9}$`, `_whatsapp_group_validator` https+chat.whatsapp.com, `_normalize_contact_email` mode=before empty→None + regex, EmailStr kept); Hebrew errors per spec. Frontend: `ContactChannelsCard` phone/email `inputMode` + catch→`detailToMessage` (surfaces the 422 Hebrew inline); admin `ProducerForm` phone `type="tel"`. No new i18n. **Verify:** build 0 · eslint 0 · full vitest 1624p/10s · new `tests/test_meh1537_contact_validation.py` (CI-only — sandbox blocks pip/pg) · standalone logic + audit-parity green · Playwright self-QA @375+@1440 (`qa-artifacts/MEH-1537/`, ~250KB) invalid-email→inline Hebrew error, valid→נשמר.
- **DRAFT PR — NOT merged.** AUTO-MERGE gate blocked on Sapir confirming the Railway audit SQL (in the ticket) returns 0 invalid rows (or manual fix). Backend pytest runs CI-only.

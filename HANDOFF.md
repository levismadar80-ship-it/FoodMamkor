...
## 2026-07-11 — MEH-1074 LAUNCH SPRINT END-STATE (waves 0–4 complete; 3 PRs armed, CI-event suppression blocks their gates)

- **Sprint scorecard (this session):** W0 VRT baselines + e2e QA-comment (merged) · W1 MEH-1056/1057 (merged) · W2 MEH-1054, MEH-766 ch6 (merged) · W3 demo-seed #1597 + 375px walkthrough #1610 + friction tickets MEH-1118/1119 + checklist evidence + showcase #1616 (all merged) · W4 = MEH-1073 remainder: **T2 #1623, T7 #1625, T10 #1628 — complete, self-QA'd, auto-merge armed, NOT yet merged** (below). All four MEH-1073 rulings were closed pre-execution; parallel session had already merged T1/T3/T4/T5/T6/T8/T9 + #1571/#1598 (verified per-PR against origin/staging history before starting — collision gate).
- **⚠️ BLOCKER (needs-sapir, infra not code): GitHub Actions creates NO workflow runs for this session's pull_request events since ~18:54Z.** #1623/#1625/#1628 have zero check-runs (API `total_count: 0`) so the 2 required gates never register and auto-merge waits forever. Re-fire attempts per STOP-condition budget: empty-commit synchronize (all 3 branches) ✗ · close/reopen #1623 ✗. NOT a real CI failure — parallel-actor PRs (meh-1123/1125 etc.) ran green in the same window; #1616 (same creation path) ran fine at 17:40Z. Rule 21 honored: nothing merged without a verified green gate. **Sapir unblock (one of):** (a) from Git Bash: `git fetch origin && git checkout <branch> && git commit --allow-empty -m retrigger && git push` per branch — her-actor events aren't suppressed; (b) repo Settings → Actions → check runner/app policies + spending limit. Auto-merge is armed on all 3 — the moment gates go green they land. **CHANGELOG note:** the 3 PRs collide on the same `## Unreleased` anchor line — after the first merges, the other two need a rule-25 accept-both sync (this session babysits via send_later if still alive; otherwise trivial manual resolve).
- **needs-sapir queue (full list in the MEH-1074 launch-readiness SYNC):** (1) CI suppression above; (2) demo-business identity block edit + staging seed dispatch (W3, one command); (3) `ProducerCard.jsx:312` — last tracked Hebrew eyebrow, central component, excluded from T10 by design; (4) MEH-215 checklist item "אימייל קיים → שגיאה" is obsolete-by-design (MEH-328 anti-enumeration) — remove from the ticket; (5) legal-wording tickets MEH-1118/1119 (from W3 friction) untouched per sprint rules; (6) MEH-1106 Task-B products-as-7th-field decision (pre-existing, see entry below).
- **staging→main:** after the 3 PRs land, staging carries the full sprint. Gate = Sapir opens the staging→main PR (or `Compare & pull request` in UI), confirms staging smoke (`workflow_dispatch` staging-smoke.yml) + mobile spot-check, merges. No schema migrations pending beyond what already auto-ran on staging deploys; no new env vars this sprint.

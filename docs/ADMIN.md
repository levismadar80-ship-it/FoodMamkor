...
| בתי עסק | /admin/producers | טבלה + חיפוש + ייבוא/ייצוא Excel + אישור מהיר inline; השהה/שגריר/סטורי/מחיקה בתפריט ⋮ פר-שורה (MEH-1027) + **דחייה** (MEH-226, ⋮ tone danger, שורות pending/pending_whatsapp בלבד) → מודל 5 סיבות + פירוט → שלב אישור → מייל לבעלת העסק |

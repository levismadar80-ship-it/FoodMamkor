...
## Unreleased
- **MEH-1049 (scope-add, MEH-1074 Task A) — contact-card truncation + share disambiguation (Refs MEH-1074):** the producer contact sidebar clipped secondary contact values and the viral-share button read as a second WhatsApp contact path. **(1)** phone + Instagram tiles (`ContactSidebar.jsx`) moved from truncating 2-col grid cells to **full-width rows** (`col-span-2`), dropped `truncate`/`overflow-hidden`, `dir="ltr"` value span, `break-all` so a long handle wraps instead of clipping — the number/handle is always fully readable (no ellipsis). Other tiles (website/email/facebook/external) unchanged. **(2)** `WhatsAppShareButton` icon `WhatsappLogo` (green `#25D366`) → neutral `ShareNetwork`; copy `whatsapp.share_button.share_to_friend` "שלחו לחברה" → "שיתוף עם חברים" (en "Send to a friend" → "Share with friends") + `share_aria` updated in lockstep ("שיתוף עם חברים בוואטסאפ") to keep WCAG 2.5.3 label-in-name — so the share affordance next to the WhatsApp *contact* CTA no longer looks like another contact channel. WhatsApp remains the single filled primary (PrimaryContactButton; FollowButton already demoted in the merged MEH-1049 chunk 1). New `WhatsAppShareButton.test.jsx` (3). Verify: build exit 0 · WhatsAppShareButton + en-parity 7 passed · canaries clean · 0 physical RTL.

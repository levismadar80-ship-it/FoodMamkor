...
- **feature/comprehensive-mobile-audit** — Full mobile layout audit covering every page and shared component. 14 files changed. **WCAG touch targets:** bumped ProducerCard "מידע נוסף" button from 36px→44px, category filter pills on `/events`+`/experiences`+`/register/producer` from py-1→py-2, map legend buttons py-1→py-2, password eye toggle on `/login`+`/register` from p-1→p-2, Settings delete dialog buttons from py-2→py-3, FavoriteButton default from p-1→p-2, Header mobile menu links gained py-2 for 44px tap targets. **Layout fixes:** ProducerDetail delivery table wrapped in `overflow-x-auto`, ProducerDetail h1 from text-4xl→text-2xl md:text-4xl, register/producer delivery grid from grid-cols-3→grid-cols-1 md:grid-cols-3, event/experience detail action buttons full-width on mobile, Settings dialog buttons stack vertically on mobile. **Mobile padding:** ProducerDetail sidebar p-6→p-4 md:p-6 + favorites/share row matched sizing, login card p-8→p-5 sm:p-8 md:p-10, register cards p-8→p-5 md:p-8, register inputs px-3 py-2→px-4 py-3, show-on-map button py-2.5→py-3. ShareButton + FavoriteButton gained optional className props. Supersedes PR #67.

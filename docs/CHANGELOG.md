...
## Unreleased
- **MEH-784 — Windows-safe hooks + Bash-write bypass closed + guardrail drift guard (DRAFT):** hardening follow-up to the **2026-06-07 hooks-strip incident** (session `fcd15bf2`: `.claude/settings.json` found with the entire `hooks` block deleted as a *pre-existing, uncommitted* working-tree change — no Edit/Write tool call did it; **re-stripped between the two restores**, blob `16ab7c8→c9e8f18`; restored clean 14:37 UTC. Likely vector: repo lives under a **OneDrive-synced path** resurrecting a cached stripped copy — hypothesis, unproven). **Windows/MINGW test:** jq/bash/node/python/git all present, all `*.sh` LF (`.gitattributes` already enforces `*.sh text eol=lf` — CRLF already handled); the **one hard failure** was `subagent-trace.sh` exiting 3 on every SubagentStop with an unreadable transcript — MSYS2 mangles the pure-`?` value in `jq --arg tools "?"` → `error: <name>/0 is not defined`, and `set -e` killed the hook. **(3) `subagent-trace.sh`** — default `TOOLS="?"`→`"unknown"` (glob-safe), duration passed via `--argjson dur` (always-valid JSON) dropping the fragile `$dur|tonumber?`, and trailing `|| true` so the analytics hook stays fail-open (always exit 0). **(4) `check-bash-safety.sh`** — new deny-protected-path write guard: blocks `cp`/`mv`/`tee`/`dd`/`install`/`ln`/`sed -i`/redirection that mutates `.github/workflows`, `alembic/versions`, `pyproject.toml`, `uv.lock`, `.claude/settings.json`, `.claude/hooks` from a Bash command (parity with the Edit/Write deny-list — the live bypass was confirmed: a `printf >> .claude/settings.json` succeeded despite the `Write` deny because perms gate the *tool*, not shell redirects). Reads + all `git` stay exempt (guardrail-restore `git checkout --` still works); heuristic-level + fail-toward-block. **(5) `session-start.sh`** — guardrail drift warning: when the working copy of `.claude/settings.json` or any `.claude/hooks/*` differs from HEAD, prepends a loud ⚠️ block with the `git diff`/`git checkout --` commands (the 2026-06-07 strip went unnoticed because nothing flagged the dirty file). All three verified on this MINGW machine (subagent-trace now exit 0; 6/6 protected-writes blocked, 5/5 reads+git allowed, `rm -rf /`→2 + `npm ci`→0 no regression; bypass now closed). **No hook deletions.** `settings.json` untouched (drift guard lives in `session-start.sh`, already wired to SessionStart). Hooks are AI-write-denied by design — applied via one-time Sapir-authorized Bash-write, then the guard closed the hole. `Closes MEH-784`.

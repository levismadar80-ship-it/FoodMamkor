...
## 2026-05-16 — MEH-539 Phase 2D: /about/for-producers/guides routes (3 guides + index)

`feat(MEH-539)`: 4 new public Next.js App Router pages under `/about/for-producers/guides/` — the link targets embedded in the Phase 2C onboarding emails (Day-2 / Day-5 / Day-10). **Files added:** `frontend/components/GuideArticle.jsx` (shared shell: header eyebrow + read-time + Frank-Ruhl-Libre H1, structured-blocks renderer for `h2`/`h3`/`p`/`ul`/`ol`/`blockquote`/`callout`/`hr`, signoff footer + back-link to the index — uses **only** logical Tailwind properties `ms-*`/`me-*`/`border-s-*`/`ps-*` so RTL inheritance from `app/[locale]/layout.js:114` is honoured); `frontend/app/[locale]/about/for-producers/guides/page.js` (index — 3 cards, brand-token colours `#2e6853` / `#2E4A2E` / `#F5F0E8`, footer Instagram link to `@meha_makor` mirroring the for-businesses precedent); `frontend/app/[locale]/about/for-producers/guides/business-story/page.js` (Guide 1, 4-min read, 837 words approved 16-May); `frontend/app/[locale]/about/for-producers/guides/product-photography/page.js` (Guide 2, 5-min read, 920 words); `frontend/app/[locale]/about/for-producers/guides/customer-messages/page.js` (Guide 3, 6-min read, 1,085 words, includes the 5 WhatsApp-template blockquotes with multi-line support via `InlineBold` `\n → <br>` enhancement). **Pattern (b) — hardcoded JSX** chosen because `grep -rln "react-markdown\|remark\|next-mdx\|@mdx-js" frontend/` returned zero matches — no markdown pipeline exists; adding one is scope-creep. Each page exports an App Router `metadata` object (title / description / OG / `alternates.canonical`) per the sibling pattern at `frontend/app/[locale]/about/for-businesses/page.js:4-17`. **No new npm dependencies.** **No backend changes.** **URL match:** `backend/app/services/onboarding_followup.py:78,117,157` references these exact paths — verified the slugs match (`business-story` / `product-photography` / `customer-messages`). **Content cleanup applied** per Phase 2D prompt: stripped Drive MCP's backslash escapes (`\*`, `\[`, `\-`), stripped header block (`# Guide N — ...`, Path:, Linked from:, Status:), stripped trailing implementation notes, replaced mojibake `ð¿`/`ð¥`/`ð§`/`ð¾` with 🌿/🥖/🧀/🌾 per user-approved emoji set; one `ð` in Guide 3 Template 3 sat between "המחירון" and "משלוח" with no clean fit in the approved 5 — dropped + replaced with a sentence break (flagged in Skeptic note below). `dir="rtl"` is inherited from `<html dir>` at `app/[locale]/layout.js:114`; no per-article `dir` override needed. **Build:** `next build` green in 11.9s — all 4 routes prerendered as SSG for both `he` and `en` locales (8 static HTML files). No frontend tests added (no existing test pattern for `/about/*` pages — sibling `for-businesses/page.js` ships untested too). **Path naming note (does not block):** the existing producer-facing page is `/about/for-businesses` (matches CLAUDE.md's "בית עסק / בעלת עסק" brand voice). Phase 2C emails + Phase 2D prompt + Drive guide files all use `/about/for-producers/guides/` — kept verbatim since the URLs are already locked into the email bodies and the Drive content. Tracked as a post-Phase-2 cleanup decision (rename to `for-businesses/guides/` AND update emails AND update Drive, OR keep `for-producers/` for the guide subtree only) — not a blocker.

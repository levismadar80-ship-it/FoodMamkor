...
## 2026-04-27 — MEH-342: refactor(docs): trim CLAUDE.md from 197 → 129 lines (under 150-line cap, 21 lines headroom) by relocating 4 sections verbatim to existing `.claude/rules/` files (#361). Bug Protocol + Commit discipline + /loop usage patterns appended to `workflow.md`; PR Review Workflow appended to `deployment.md`. Execution principles (exec §7–13) removed from CLAUDE.md — already verbatim in `workflow.md:281–309` (canonical version is fuller, with source attributions). PR approval guide trimmed to DoD one-liner + pointers to testing.md/deployment.md (full table preserved in `deployment.md:64–70`). Line-91 pointer enumerates moved sections for discoverability. Zero rules lost, zero rules duplicated, zero edits to rule wording — only relative-link paths adjusted from CLAUDE.md root to `.claude/rules/` subdirectory. Unblocked by MEH-355 (#360) `*.md` RTL allowlist exemption (workflow.md edit cites `right-3` as LTR-input documentation example).

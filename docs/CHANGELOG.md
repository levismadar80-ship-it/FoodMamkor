...
## Unreleased
- **MEH-1011 Chunk 1 — producer request-changes (completion request) backend (DRAFT):** the non-terminal twin of reject — an admin can send a pending producer a "please complete" request (missing photo / license) instead of dead-ending on a blocked approval. New migration `a1b2c3d4e5f6` adds `producers.requested_changes` (TEXT, nullable) + `producers.changes_requested_at` (TIMESTAMPTZ, nullable, tz-aware) — Expand-only, no backfill; ORM columns added to `models.py` (alembic check clean). New `POST /admin/producers/{id}/request-changes` (`RequestChangesIn.feedback`, empty/whitespace → 400 mirroring `admin_recipes.py:123`) records the feedback, KEEPS status pending, emails the producer (Hebrew copy, `mehamakor.online/producer/dashboard` link) via `_send_notification_email`, and WhatsApps admin ("📝 נשלחה בקשת השלמה ל-{name}"); `approve_producer` now clears both columns on success. `ProducerAdminOut` += `requested_changes` + `changes_requested_at` (admin-only). New `tests/test_producer_request_changes.py` — 7 tests (happy path · trim · empty/whitespace 400 · 404 · email-to-producer mock-capture · approve-clears). Verified: `alembic upgrade head` + `alembic check` clean; new suite 7✓; `pytest tests/test_api.py` 202 passed (no regression). Docs: DATA.md + `.ai/diagrams/db-schema.md`. Chunk 2 (admin frontend) follows after merge. `Refs MEH-1011`.

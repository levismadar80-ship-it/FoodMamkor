...
## Unreleased
- **MEH-1084 — /producers category selection → `router.push` so Back cancels the filter (DISC-06, Closes MEH-1084):** category discovery's Back-button trap (MEH-1077 DISC-06). **Phase 0 correction (meta-patterns §1):** the ticket targeted `use-home-page.js`, but MEH-1080 already made the homepage category cards real `<Link href="/producers?category=<id>">` (`HomeCategoryGrid.jsx:112`) — homepage category selection is already a history push; Back returns to the full grid. The live `router.replace`→Back-exits bug now lives only in the `/producers` in-place category radio (`ProducersClient.jsx` `handleCategorySelect` → `syncUrl` → `router.replace`). Sapir approved the scope move to `ProducersClient.jsx`. Fix: `syncUrl` gains a `method` arg (default `"replace"`); `handleCategorySelect` passes `"push"` for a real category selection (perceived as a new view → one Back cancels it and returns to the prior view) and `"replace"` for the `"all"`/clear sentinel (baseline return — a push there would need a double-Back). Chip toggles, city, search, and clear-all stay `replace` (no history pollution). The written param set is identical for both verbs — MEH-1081/1083 serializer untouched; `scroll:false` preserved on both. Folded the 2 test nits from PR #1569 review (MEH-1083): `useHomePageDietChipsUrl.test.jsx` — `import api` moved to the top import group, and `expect(producersCall).toBeDefined()` guard added before the params assertion. Verify: `ProducersClientCategoryAxis.test.jsx` updated to the new push/replace contract + `useHomePageDietChipsUrl` nits — full vitest **893 passed**/41 skipped · build exit 0 · Playwright self-QA (local `next start`, mocked `/api/**` — MEH-360; `qa-artifacts/MEH-1084/`): category select → `?category=18` + history 2→3 (push), Back → `/producers` category-gone + page still mounted (not exit), 3 chip toggles kept history at 3 (replace didn't stack), one Back unwound to `/producers`. `Closes MEH-1084`.

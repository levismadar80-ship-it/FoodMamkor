...
## Unreleased
- **MEH-1014 — BottomNav minimize-on-scroll (replaces hide-on-scroll; DRAFT):** the mobile floating pill's MEH-789-chunk-2 hide behaviour (`translate-y-[120%]` off-screen slide toggled per-frame scroll direction) flickered — every reversed pixel flipped hide/show and iOS rubber-band overscroll fed phantom direction changes. Replaced hide with **minimize**: on scroll-down the pill shrinks in place (`nav h-14→h-12`, `p-1.5→p-1`, tab `min-h-[44px]→min-h-[40px]`) and the labels collapse (`opacity-0 max-h-0 overflow-hidden`) leaving the size-22 icons; the pill **never leaves the screen**. Flicker killed by **asymmetric hysteresis on accumulated per-direction delta** (module tunables `HIDE_DELTA=24` / `REVEAL_DELTA=8` / `TOP_THRESHOLD=60`, Sapir may tune): 24px sustained down → compact, 8px up → expand, a direction change zeroes the opposite accumulator, and `scrollY` is clamped to `[0, scrollHeight-innerHeight]` so overscroll can't feed phantom deltas. `y<60` (+ near-top) always expands; `sheetOpen` renders expanded (replaces the old `hidden && !sheetOpen` guard); `onFocusCapture` on the shell expands on keyboard/AT focus (mirrors the MEH-734/884 focus guard detached from Header). Transitions are **height** (nav) + **opacity/max-height** (labels) only — `duration-base ease-quart motion-reduce:transition-none`; `backdrop-filter`/`.nav-pill-glass` is never animated (the transform-slide transition was deleted, not repurposed). The **MEH-852** liquid-stretch indicator re-measures on the compact toggle: the `h-14↔h-12` change fires its `ResizeObserver` and `compact` is added to the measure-effect deps (`[activeRouteIndex, compact]`) as a belt-and-suspenders synchronous re-measure so it can't lag the `p-1.5↔p-1` tab-rect shift. `BottomNav.jsx` only (Header untouched — MEH-884 already detached its hide; inline-copy-vs-Header note stands, no shared-hook extraction). Build ✓ (113/113 SSG), vitest 702 passed / 0 failed (BottomNav 7/7), `grep translate-y-[120%]` = 0. `Closes MEH-1014`.

...
## Unreleased
- **MEH-1116 — edit tab → accordion + URL-hash anchors (Refs MEH-1089):** the producer edit page was one long page with 6+ expanded cards. New `EditAccordionCard.jsx` — header button (title + live one-line status summary from the SAME page profile + Phosphor CaretDown), one card open at a time, all collapsed on load. Hard MEH-1100 constraint held: cards stay MOUNTED on collapse (`hidden`-toggle, NOT unmount) so unsaved state + the dirty aggregate + the nav guard survive collapse — Playwright-proven (type → collapse → reopen → value intact; guard confirm fires on nav while dirty). Stable English anchor ids (`#bio #questions #contact-channels #categories #images #location #products`) auto-expand + scroll on load and on hashchange — the deep-link infrastructure MEH-1106's checklist will consume. Cards' own chrome/headings moved into the accordion shell; `ProductsSection` gained display-only `embedded` + `onCountChange` props (justified scope-add: avoids double card chrome; save logic untouched). i18n `dashboard.producer.edit_accordion` ×17 he + en twins (ICU plural, gender-neutral, no red for incomplete — ADR-019/024). Verify: vitest 898 · build 0 · lint 0 errors · Playwright 30/30 (`qa-artifacts/MEH-1116/`).
- **MEH-1115 — tap-to-expand "מה זה?" explainers (Refs MEH-1089):** business owners hit "ערוץ ראשי" / "טופס הזמנות" / "קבוצת רכש" with no explanation at the point of decision. New reusable `WhatsThis.jsx` disclosure (44px text-link trigger, aria-expanded/aria-controls, panel kept mounted + hidden-toggled, touch-first, collapsed by default — deliberately NOT InfoTooltip, which stays the hover "i" bubble) wired ×3: primary-channel radio header + external-order-form field (ContactChannelsCard) + group-buys page header. i18n: new top-level `whats_this` namespace he + English en twins. Verify: vitest 895 · build 0 · Playwright 24/24 (`qa-artifacts/MEH-1115/`). PR #1615.

...
- **feature/producer-tabs-map-sheet** — Two major UX improvements. (1) **Map bottom sheet redesign** — the mobile producer popup on `/map` was `fixed bottom-16 inset-x-3` which overlapped content awkwardly; redesigned to Google Maps style: full-width from bottom edge (`fixed bottom-0 inset-x-0`), rounded top corners, dark scrim backdrop, `sheet-up` CSS animation from below viewport, sticky close button in the handle bar. (2) **Producer detail mobile tabs** — `/producer/:id` was a single long scroll on mobile (gallery → sidebar → description → products → delivery → reviews). Redesigned mobile layout with tabbed sections to reduce vertical scrolling: always-visible header (gallery + name + badges + contact buttons grid) above a sticky tab bar (אודות | מוצרים | משלוח | ביקורות). Contact buttons follow a 2-per-row grid layout (Phone/Instagram, Website/Copy-link) plus full-width Share, Show-on-map, and WhatsApp CTA. Tabs with no content are hidden (no products → no products tab). Sticky bottom WhatsApp bar appears when contact section scrolls out of view. Desktop layout unchanged — still two-column with sticky sidebar. Both layouts share the same ContactButtons component to avoid duplication.

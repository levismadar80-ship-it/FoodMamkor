...
## Unreleased
- **MEH-1121 (MEH-1074 Task D) — imageless-hero consistency: masthead fires for blank image arrays (Refs MEH-1074):** some genuinely-imageless approved producers (the ZFFS case) rendered a small gray `ProducerHeader` `<h1>` + floating tags instead of the MEH-815 Tinted Masthead. Root cause: `ImageGallery` gates the masthead on `!images.length` (`ImageGallery.jsx:59`) and `ProducerDetail` derived `hasImages` + the gallery prop from the **unfiltered** `producer.images` — an array of only blank/whitespace entries (`[""]`) counted as length 1, so a broken single-photo banner rendered AND `ProducerHeader` kept its own gray h1. New pure helper `getRenderableImages()` (`producer-format.js`) filters non-empty trimmed strings; `ProducerDetail.jsx` now derives **both** `hasImages` and the `ImageGallery images` prop from that one list (single owner — no ImageGallery-vs-Detail drift, MEH-271 Smell #1), so every imageless producer gets the masthead and its single h1. Masthead itself untouched (Sapir-approved LOCK); Latin-only names keep rendering as the masthead h1 via the `font-headline-lg` Frank Ruhl Libre → David Libre → Georgia fallback chain (verified, unchanged). New `ProducerFormatImages.test.js` (5); the `images=[]`→`gallery-empty-state` case already covered in `ImageGalleryEmpty.test.jsx`. Note: live dana-vs-ZFFS DOM comparison deferred (sandbox can't reach the staging backend / preview — MEH-360 class); PR #1492 audit confirmed the *test* producer's no-masthead was a real uploaded placeholder PNG (unaffected — the filter drops only blank strings, never real URLs). Verify: build exit 0 · 20 gallery/format tests pass · 0 physical RTL.
- **MEH-1106 — ProfileCompletenessCard → 4-step checklist (card-only products, B1 — Refs MEH-1089, Closes MEH-1106):** the producer-dashboard completeness card (MEH-897 foundation) became an explicit 4-step checklist — ① photo ② categories+location ③ 3 products ④ primary contact — each row a deep-link to its `/producer/dashboard/edit#profile-*` editor section. **Phase-0 decision B1 (Sapir 11/07):** the "3 products" step is **card-only** — it reads `producer.products.length` (the `/producers/me` payload already joinloads the relation; threshold 3 mirrors `lib/badges.js:125 PRODUCTS_MIN`) and **never enters the shared `lib/producer-completeness.js` heuristic**, so the admin producers list + dashboard share-gate are byte-unchanged (proven: `producer-completeness.test.mjs` + admin suites green **and untouched**). MEH-897 calm idiom preserved — gold ring for every incomplete state, no red (MEH-1092), green only at 4/4 (collapses to the single "הפרופיל מלא" line). Steps ①②④ derive from the heuristic's `missing` set (image, category+city+coords/delivery, contact); `short_desc` stays in the heuristic for admin but is not one of the 4 card steps. New `steps.*` + `checklist_sub` he keys **with en twins**, plus en twins for `checklist_aria/done/todo` (the MEH-897 he-only gate is gone — checklist now renders on /en; the 3 keys removed from the MEH-978 en-parity baseline). Edit page gained 4 `id` scroll-anchors (no logic change). `ChangesRequestedBanner` (separate admin channel) untouched. Verify: `npm run build` exit 0 · vitest 895 passed (card suite rewritten for the 4-step model; en-parity/locale guards green) · Playwright self-QA (he desktop+375px + en) — all step states + independent banner confirmed. `Refs MEH-1089` · `Closes MEH-1106`.
- **MEH-1113 — unify inbound contact routing: topic field (no schema) + for-businesses form/email instead of Instagram DM (Closes MEH-1113):** every inbound path now ends in `ContactMessage` + the notification email; no DB column, no Alembic, no env var. **Backend** — `ContactIn` (schemas.py) gains an optional `topic: str | None` validated against a whitelist `{business, general, correction, other}` (invalid → 422 Hebrew "נושא הפנייה אינו תקין"; None → "general"). New `CONTACT_TOPIC_LABELS` dict is the single source of truth for both the validator (keys) and the router's label map (marketing.py imports it — avoids the two-parallel-mechanisms drift, workflow Smell #1). `submit_contact` prepends `נושא: <label>` as the stored message's first line and tags the email subject `מהמקור — פנייה חדשה (<label>) מ-{name}`; rate-limit (5/hour) + fail-open contract unchanged. 3 new pytest cases (valid topic → prefix+subject, invalid → 422, missing → general default) + 2 existing persistence assertions updated for the prefix. **Frontend /about** — a "נושא הפנייה" `<select>` (4 options, default שאלה כללית) above the message field, prefilled from `?topic=` via `useSearchParams` when the value is whitelisted, sent in the POST body; the contact section gains `id="contact"` for `/about?topic=business#contact` deep-links. **Frontend /about/for-businesses** — the Instagram-DM footer line replaced with neutral-plural "עדיין יש שאלות? כתבו לנו דרך [טופס יצירת הקשר → /about?topic=business#contact] או במייל [mailto CONTACT_EMAIL, break-all + `<bdi>` per MEH-905/968]"; Instagram removed from this line only (site-wide footer IG link untouched); "הוסיפו את העסק שלך" CTA stays. i18n: new `about.consumer.contact.topic_label`/`topic_options.*` + `about_business.footer_questions_prefix`/`footer_form_link`/`footer_or_email` (replacing the old `footer_intro_prefix`/`suffix`), he + en twins. Verify: backend logic unit-checked (validator/422/labels/default) + ruff clean · full pytest on CI · build exit 0 (`/about` + `/about/for-businesses` compile) · lint 0 errors · en-parity/locale-guard/namespace 8 passed · 0 physical RTL · Playwright self-QA 3 states (default select / `?topic=business` prefill / for-businesses no-IG form+email line; screenshots `docs/audits/screenshots/2026-07-meh1113-qa/`). `Closes MEH-1113`.
- **MEH-1117 — WhatsAppQuestionChips token sweep + 14px interactive minimum (Refs MEH-1074):** the question-chip component still carried inline hex + a hardcoded radius + sub-14px interactive text (token-drift left over from the MEH-1103 sweep). `WhatsAppQuestionChips.jsx` — label `#6B6B6B` → `text-fg-muted` (drop inline style); chip `<a>` `border 1px solid #e5dfd3` → `border border-border`, `borderRadius 20px` → `rounded-xl` (the 20px token exactly), `padding 6px 12px` → `px-3 py-1.5`, `fontSize 12px` → `text-sm` (14px — MEH-1103 interactive-text floor; the chip is an interactive link); `minBlockSize: 44px` kept (WCAG 2.5.5). No copy/logic change. Verify: build exit 0 · 0 residual hex · 0 physical RTL.
- **MEH-1112 — /about polish: composition balance, testimonials gate, CTA hierarchy, plural form voice (Closes MEH-1112):** 6 audit findings (Sapir 11/07, post-#1037) fixed on the /about editorial page — layout + copy + i18n only, no schema/auth. **(1)** pull-quote container `max-w-6xl`→`max-w-3xl` (matches the comparison block below) so the offset blockquote no longer strands >50% empty cream at 1440px. **(2)** testimonials section render-gated OFF behind `const SHOW_TESTIMONIALS = false` (NN/g: real social proof or nothing — no empty-shelf placeholder); JSX + i18n keys kept intact for revival, the "יש לך סיפור? ספרו לנו" link disappears with it, and gating removes the ~200px void that sat between the Tips accordion and the placeholder. **(3)** close band CTA hierarchy (per MEH-1049/MEH-907) — "גלו עסקים קרובים" stays the single primary; the two secondary actions (business hub + MEH-534 acceptance-process cross-link) collapsed onto one quiet muted row, small/underlined/non-bold. **(4)** copy (he.json + en twins): `contact.heading` `דברי איתנו`→`דברו איתנו`, `contact.subtitle` `…מכן`→`…מכם`, `contact.submit` `שלחו`→`שליחה` (T6 gerund, MEH-1073 — note: the audit table said `שלחי` but the live value was already the plural `שלחו`; the gerund end-state is identical either way), `process.crosslink_from_about` `כך אנחנו מכירות…`→`כך אנחנו בודקות…` (kills the מכירות/"sales" homograph; feminine narrative-we preserved; en twin "get to know"→"vet"). **(5)** new `contact.email_direct` line under the form — "אפשר גם ישירות במייל:" + mailto rendering `CONTACT_EMAIL` (MEH-653) with `break-all`+`dir="ltr"` per the MEH-905 render pattern (NN/g Contact-Us guideline #1). Founder letter / pull-quote text / values narrative — zero edits. RTL logical props only (0 new physical). Verify: build exit 0 · lint 0 errors · en-parity/locale-guard/namespace 8 passed · Playwright self-QA 1440×900 + 390×844 (screenshots `docs/audits/screenshots/2026-07-meh1112-qa/`) — testimonials 0-rendered in DOM, one primary CTA, "דברו איתנו" + email line visible. `Closes MEH-1112`.
- **/map QA batch — MEH-1108 / MEH-1109 / MEH-1110 (3 PRs, autonomous per ADR-016 v2 + Rule 23):** post-MEH-1075 QA fixes on `/map`. **MEH-1108 (#1606, `f8ddde46`) — chip-row white pills:** `FilterChipsBar` was the only `ChipScrollRow` consumer not passing `fadeBg`, so the default `#ffffff` edge-fade smeared white over the cream `/map` surface (`#F5F0E8`) — read as "blank white pills" + a floating white block on a mid-scroll clipped chip; passed `fadeBg="#F5F0E8"` to both call-sites (map-scoped — the shared `ChipScrollRow` default + home/producers/events consumers are byte-unchanged, they already pass this value). **MEH-1109 (#1608, `2c055ffb`) — account dropdown under the map pill:** the global `<header>` `sticky top-0 z-[1000]` is a stacking context that capped the UserMenu dropdown's `z-[1001]` at the header's page-level tier; map controls (also `z-[1000]`, later in DOM) won the tie and covered the open menu (truncating "לוח הבקרה שלי" — pill overlap, not a width clip). Moved the header to `z-[1050]` — above controls:1000, below cookie:1100 — new tier recorded in the /map z-token ledger (`.claude/rules/rtl.md`); central component, adversarial self-review, no must-fix. **MEH-1110 (#1611, `e7c48dec`) — sort control restyle (styling-only):** the desktop count-line sort `<select>` ("קרוב אליי ⌄") looked like a filter chip; restyled to a borderless `text-primary` text+`CaretDown` control (`appearance-none` hides the native arrow, `min-h-[44px]` tap floor, `-my-2.5` compact row, RTL logical props) with `value`/`onChange`/`aria`/options/state unchanged; auto-review caught a real a11y gap and the focus cue was swapped from `focus-visible:underline` (unreliable on native `<select>`) to a box-shadow focus ring. Category/toggle chip styling deliberately untouched (gated MEH-1081/1083). Verify each: vitest green (890 / 891 / 893) · build exit 0 · Playwright self-QA (local `next start`, mocked api/auth — MEH-360; screenshots in `qa-artifacts/MEH-1108,MEH-1109,MEH-1110/`). E2E note: the **non-required** `Playwright E2E (Vercel preview)` flipped PASS→FAIL across trivial-diff SHAs on #1606 + #1611 (default render identical to passing SHAs) — merged on the 2 required aggregator gates (CI gate + Deploy gate) per testing.md.
- **MEH-1074 Wave 3.5 — showcase "כך נראה פרופיל מלא" on /about/for-businesses (feat, Refs MEH-1074):** new annotated section between the FAQ and the footer — heading + prominent "דוגמה" pill, an example-profile figure (מאפיית רוח השדה, small דוגמה tag, tagline) and the 6 elements of a complete profile (photos/story/verified/products/delivery/reviews, Phosphor duotone icons, tokens only, logical props) + a nudge line pointing at the dashboard completeness meter. No live-profile link by design (MEH-409 swaps the demo for a real business post-first-10 — a hardcoded slug would rot). `about_business.showcase.*` keys with full en twins. **W3.5(b) evidence-note:** the "wire profile_strength as completeness nudge" half of the epic item already shipped — `dashboard.producer.completeness.*` renders a percent ring + checklist + staged yellow→green states, and `profile_strength` gates the weekly-candidate card (`dashboard/page.js:646`); nothing to build. Verified: build exit 0 · en-parity + en-locale guards 4 passed · self-QA screenshots 375+1440 (`qa-artifacts/MEH-1074-wave3/showcase/`) — 6 items, 2 דוגמה labels, 0 page errors, en twin renders. Also lands the W3.4 checklist evidence package (`qa-artifacts/MEH-1074-wave3/checklists/` — MEH-215/217 screenshots + verdicts.json).
- **MEH-1074 Wave 3 — 375px walkthrough report + QA artifact package (docs+artifacts, Refs MEH-1074):** new `docs/audits/2026-07-meh1074-walkthrough.md` — executed-step verdicts for the full customer journey (12 steps incl. trust strip, reviews+owner reply, WhatsApp deep link, favorites gate, consumer registration) and producer journey (wizard 5 frames real-POST → admin approve via real UI → owner login → dashboard/availability/recipes/events/insights) against the seeded demo business, + 43 screenshots under `qa-artifacts/MEH-1074-wave3/`. Two product findings → tickets: dashboard conversion stat >100% ("133.3% מהצופות פנו אלייך", cross-ref MEH-1089) and 4 stray page exports breaking the Next Page contract (webpack type-check hard-fails; turbopack doesn't enforce). Appendix documents the sandbox env classes (turbopack chunk-emission, next-start chunk 404s, proxy-vs-localhost fetches, rebuild-under-live-server) so future sessions skip the re-diagnosis. Walkthrough also caught + shipped the `email_verified` seed login gate fix (#1597).
- **MEH-1062 (fold, was intentionally omitted from #1545 to honor its locked file scope):** Event JSON-LD (`@graph` [Event, BreadcrumbList]) + recipe/event breadcrumbs via shared `buildBreadcrumbList` + entity Twitter cards + per-locale recipe sitemap URLs — `lib/seo.js`, `events/[id]/page.js`, recipes page, `sitemap.js` (merged 09/07; escaper hardening #1549/MEH-1069).
- **MEH-1114 — ReviewsSection owner empty-state: "מוצג רק לך" eyebrow + ADR-024 voice (Refs MEH-1074):** the reviews empty state that renders only for the producer owner (`ReviewsSection.jsx:414`, `isOwner`, MEH-554) had no marker that readers don't see it — an owner could mistake the internal coaching copy for public text. Added an eyebrow chip above the owner `EmptyState` — `bg-background-alt` + `border-border` + Phosphor `EyeSlash` + new `reviews.owner_only_eyebrow` ("מוצג רק לך" / "Only visible to you"). Unified `owner_empty_description` to ADR-024 neutral-plural owner-address (`שולחת לך…אותך…היא…אף אחת` → `שולחים לכם…אתכם…הם…אף אחד`, matching the already-plural `שתפו` CTA); en drops the gendered "she"→"they". Consumer empty state (`empty_message`) untouched. New `ReviewsSection.test.jsx` MEH-1114 block (eyebrow present for owner / absent for consumer). Verify: build exit 0 · ReviewsSection + en-parity/locale-guard 11 passed · canaries clean · 0 physical RTL.
- **MEH-1111 — producer-page voice micro-fixes (i18n-only, Refs MEH-1074):** three reader-facing strings on the producer page fixed for voice consistency (ADR-024 — functional surfaces = neutral-plural). `reviews.wa_gate_message` (`he.json:2639`) killed a mixed-gender sentence (`תוכלי`→`תוכלו` — the clause's opening was already plural; MEH-885-class word but intra-sentence consistency wins); `whatsapp.question_chips.ask_us` (`he.json:2824`) `שאלי אותנו`→`שאלו אותנו` (matches `ChatWidget.jsx:222`); `reviews.empty_message` (`he.json:2645`) `היי הראשונה`→`היו הראשונים`. Extended the MEH-872 imperative canary (`.claude/commands/batch.md`) with `שאלי` + `היי`. No component/logic change; en.json values already correct English (MEH-978 parity + MEH-840 no-Hebrew both green). Verify: canaries clean · en-parity/locale-guard/namespace suites 8 passed · build exit 0 · both JSONs parse.
- **MEH-1074 Wave 3 — demo-business seed script ("sample perfect listing", Refs MEH-1074):** new `backend/scripts/seed_demo_business.py` — one complete demo producer for staging QA + the /about/for-businesses showcase: approved + document-verified (verified_at/`license`, ADR-022) + phone-verified + accepting_orders; story description, 3 photos, 4 products (incl. gluten-free flag), 3 delivery areas, approved+published recipe promoting the flagship product, future workshop event, 3 reviews from `@example.com` seed consumers with one owner reply (MEH-1039) and correct denormalized aggregates; owner login bound to the row. **Production guard:** refuses any non-localhost DB host unless `RAILWAY_ENVIRONMENT == "staging"` (5-case test matrix); `admin_notes` marks the row STAGING-ONLY (MEH-409 swaps for a real profile post-first-10). Skip-if-exists default + `--refresh` recreate. **Demo identity (name/story/photos) is a fictional draft — needs-sapir checkpoint; Sapir edits the marked block then runs `railway run python backend/scripts/seed_demo_business.py` (the run = the approval act).** New `tests/test_seed_demo_business.py` (8 passed) · ruff clean · `test_api.py` 203 passed.
- **MEH-1085 — homepage empty state cause-aware + /events filters → URL (DRAFT, Refs MEH-1085):** two GREEN fixes from the MEH-1077 audit. **(DISC-07)** the homepage grid empty state blamed geography for every zero-result — with a category filter active it now says "לא מצאנו עסקים בקטגוריה הזו — עדיין" and its primary action is a "נקו את הסינון" button wired to the existing `onClearCategory` (the geographic no-category branch keeps the old copy + "גלו על המפה" link byte-for-byte; branch driven by `filters.category`, `HomeProducersGrid.jsx:142-172`). New i18n `home.producers.empty_heading_category`/`empty_subtext_category`/`clear_category_cta` he+en (ADR-024 plural voice). **(DISC-08)** `/events` city+category were client-state only — now seeded from `searchParams` on mount (URL category accepted only when it belongs to the initial tab's vocabulary, so a cross-tab share link can't zero-filter) and mirrored by a single URL-writer effect; `switchTab`'s deliberate filter reset now clears the URL too — its manual `router.replace` moved into the effect (one URL owner, `EventsClient.jsx:126-170`). **E2E-caught fix:** the writer uses shallow `window.history.replaceState` (+ no-op guard), NOT `router.replace` — a mount-time Next navigation re-suspends the `useSearchParams` Suspense boundary in `events/page.js` and resets client state (the 08-calendar-view spec caught the view toggle never sticking); replaceState is a pure URL mirror and preserves the locale-prefixed pathname on /en. Failing-first `HomeEmptyStateCauseAware.test.jsx` (2✓). Verified: vitest **887 passed**/41 skipped · build exit 0 · 0 physical RTL · live Playwright walkthrough (375px+desktop, local `next start`+seed): category-empty copy+clear→"מציגים 5 מתוך 5", geo control unchanged, deep-link `/events?city=חיפה&category=סדנה` hydrates+survives refresh, tab switch → `?tab=experiences` only, no category resurrection. `Refs MEH-1085`.
- **MEH-1103 — interactive sizing-calibration sweep, 6 PRs (Refs MEH-1103):** interactive text and tap targets had drifted below the accessibility/readability floors. Brand-doc-first: **PR-0 #1581** added "Interactive sizing minimums" to DESIGN.md (interactive text ≥14px / default 16px for nav + md/lg buttons; ≥44×44 tap targets per WCAG 2.5.5 / Apple HIG; documented exceptions: BottomNav 10.5px labels, leaflet attribution 10px MEH-919, inline body links WCAG 2.5.8). Code: **PR-1 #1592** Header nav/login text-base + pill py-1.5/px-6 + logo 111×42 (MEH-1072 frozen geometry + MEH-732 no-animated-padding + MEH-947 ternary all preserved; Header.test.jsx geometry constants re-anchored; VRT baselines regenerated on-branch). **PR-2 #1594** Button md text-sm→text-base (Phase 0: the atom's only production consumer is the dev gallery — login/register CTAs use raw buttons, unaffected until MEH-131-135 migrate). **PR-3 #1595** Footer utility links inline 11px→text-[13px] class + min-h-[44px]; column-2 links' 13px moved off inline style; zero inline fontSize left on interactive elements. **PR-4 #1596** design-review checklist: stale "/neighbor (home cooks)" section deleted (ADR-022 LOCK; no /neighbor route exists — verified) + "Users first" reworded to consumer/licensed-business-owner per CONTEXT.md §2. **PR-5** repo-wide audit → `docs/audits/2026-07-interactive-sizing.md` (fixed-in-sweep + 14 report-only findings incl. ProducersClient chips deferred to in-flight MEH-1081, admin batch proposal, BottomNav compact 40px deviation) + closed-list fixes: HolidayBanner CTA text-sm py-2 + 44px dismiss; ReviewsSection 6 action buttons min-h-[44px]; PhoneVerifyCard send/confirm min-h-[44px]; HeroSearch suggestion-section label drops uppercase/tracking-wider (Refs MEH-1073 T10). VRT baselines re-refreshed from post-sweep staging (PR-3 merged stale).
- **MEH-1059 — accessibility statement: statement-date line + contact-channel wording (DRAFT, wording GATE — Refs MEH-1074):** תקנה 35 completion. Phase 0 correction vs the ticket audit: the dedicated contact channel + email (via `CONTACT_EMAIL`, MEH-653) + 3-business-days promise **already existed** in the contact section — the genuine gaps were (1) no statement-update date (only a "תאריך בדיקה אחרונה" line — different semantic) and (2) singular/mixed voice vs ADR-024. Added `accessibility.statement_date` ("ההצהרה עודכנה לאחרונה: יולי 2026") rendered above the check-date line; contact intro → gender-neutral plural ("נתקלתן בבעיית נגישות?..."); footnote → plural voice + the ticket's exact promise ("נשתדל להשיב תוך 3 ימי עסקים"). **Discovered live bug fixed:** `phone_placeholder: "להשלים"` rendered verbatim on the legal page — phone row removed (restore with a real number; comment anchors the spot). en twins updated; both JSONs parse; build exit 0; vitest 882 passed. **DRAFT + needs-sapir — legal wording, Sapir reads full text before merge (no auto-merge).**
- **MEH-1058 — privacy policy: Amendment-13 disclosure-duty completion (DRAFT, wording GATE — Refs MEH-1074):** Phase-0 correction vs the ticket audit (meta-patterns §1): collection **purposes** (`sections.why`, 4 items), **controller identity** (`sections.operator` incl. `CONTACT_EMAIL`), and the **rights** section (עיון/תיקון/מחיקה + no-cost exercise) **already existed** — the audit's "אפס מטרות" premise was stale. Genuine gaps shipped: (1) **voluntary-disclosure statement + consequence of refusal** (new `sections.data.voluntary`, rendered under the data list); (2) **sub-processor list corrected to what actually runs** — `Twilio` was STALE (WhatsApp ships via **Meta Cloud API**, `services/whatsapp.py:83` graph.facebook.com) → replaced; added **Resend** (`email.py`), **Sentry** (frontend monitoring), **Microsoft Clarity** (`ClarityScript.jsx`, loads only on cookie consent — stated); Google line dropped its stale "שירותי אנליטיקה" claim (no GA in the codebase); infra line now names Railway as the DB host; (3) date bumped to יולי 2026. en twins for everything. Build exit 0 · vitest 882 passed · both JSONs parse. **DRAFT + needs-sapir — legal wording, Sapir reads full text before merge.**
- **MEH-766 ch6 — DROP `producers.is_verified` (Refs MEH-1074 · Closes MEH-766):** the final Contract step of the Expand-Contract retirement, gated on ch5 (#1578) being merged AND verified absent from the staging serializer (both checked fresh). New hand-written revision **`d4e7a92c81b5`** (`20260710_2130_meh766_drop_is_verified.py`, down_revision `b8f3d21a9c47`) — `op.drop_column`; downgrade restores the column nullable (values unrecoverable by design). `models.py` column removed (DO-NOT-re-add anchor), `init_db.sql` line removed, EXPECTED_TABLES untouched (column, not a table; per MEH-836 no EXPECTED_REV bump — `alembic check` covers drift). **Test sweep:** `conftest.make_producer` + 3 fixture sites drop the kwarg; `test_trust_ladder` fake drops the attr and the ch2 decoupling tests are repurposed to verified-at-only locks; `test_meh_762` "legacy axis untouched" → "grant/revoke round-trip verified_at with no legacy column" (+`not hasattr(Producer)` lock); `test_meh766_writer_retirement` reworked to the end-state (no input field · no output field · no model attribute · stray payload key ignored, +`test_model_has_no_is_verified_column`). Stale ch4-era comments updated (`admin.py:646/:685`), `docs/DATA.md` + `.ai/diagrams/db-schema.md` scrubbed. **ch4 reminder (needs-sapir):** no auto-backfill (locked) — before staging→main either admin re-grants `verified_at` for keepers or the doc-verified shrink is accepted in prod. Verify: ruff clean (CI scope) · touched suites **90 passed** · `test_api.py` **203 passed** (local Postgres 16; migration itself applies via CI's alembic-upgrade drift gate).
- **MEH-766 ch5 — Contract: `is_verified` removed from the public serialization (Refs MEH-766/MEH-1074):** the Expand-Contract retirement's contract step (ch1 FE reads #1403 · ch2 BE reads #1414 · ch3 writers #1420 all merged; ch4 = locked no-auto-backfill, prod-promotion decision stays with Sapir). Removed `is_verified: bool` from `ProducerListOut` (`schemas.py:711`) — the public verification surface is `verification_tier`/`verified_at` only (ADR-022) — and the matching Zod field from `frontend/lib/schemas.js` (its comment literally said "drops ch5"). Updated the two ch3 tests that asserted the emitted field (now assert absence + DB row via query) and added `test_public_list_output_has_no_is_verified` (contract lock: every public list item has no `is_verified`, has `verification_tier`). DB column untouched — drops in ch6 (next PR, alembic revision). Verify: ruff clean · `test_meh766_writer_retirement.py`+`test_trust_ladder.py` 32 passed · **`test_api.py` 203 passed** (local Postgres 16) · vitest 853 passed · build exit 0 · docs/DATA.md schema block annotated.
- **MEH-1080 [T-A] — homepage category cards → real links + 1:1 card↔category split (Refs MEH-1080):** closes MEH-1077 DISC-01 (in-place category filtering with no navigation) + DISC-03 (first-match mislabeling) structurally. `home-categories.js` — the 6 merged-label cards with fuzzy `match` arrays became the Sapir-locked **10-card 1:1 set** (labels = DB values verbatim; exact `===` name matching replaces `includes()` first-match; ids resolve at runtime so env id-drift is immune). `HomeCategoryGrid.jsx` — `motion.button` → real `<Link href="/producers?category=<id>">` (entrance animation moved to a wrapper `motion.div`, HomeProducersGrid pattern); unresolved-id cards render inert (same visual, no dead link, no CLS); 4 glyph-less cards (דגים/פירות/יין/קרמים) use a Phosphor `Leaf` thin fallback until MEH-683. `use-home-page.js` + `page.js` — `handleCategoryCardClick` and the card→filter wiring removed; `?category=` deep-link support (init/updateURL/pill/clear) kept for old shared URLs. New `HomeCategoryGridLinks.test.jsx` (4, failing-first). Verified: vitest **865 passed** · build exit 0 · 0 eslint errors · 0 physical RTL · adversarial review 0 must-fix · Playwright QA: 10 anchors with per-category hrefs, fish-card click → `/producers?category=2` with a real history entry, Back → homepage (`docs/audits/screenshots/2026-07-meh1080-qa/`). VRT `home.png` baseline refresh dispatched on the branch (intentional visual change, MEH-991 flow).
- **MEH-1054 — MAP-16: map bottom-sheet list loading skeleton (Refs MEH-1074/MEH-991 · Closes MEH-1054):** the sheet flashed "0 בתי עסק" + an empty list before the first `/producers` response landed (indistinguishable from a real empty result — the split reason from MEH-1029/PR #1489). (1) `useProducersFeed` gains an **additive** `loading` flag — starts true (mount effect fetches immediately), `setLoading(true)` on every `loadProducers` call, `.finally` clears it on all three terminal paths (data / malformed / network-error) so the skeleton can never stick over the toast'd error state; the geo-search bypass (useMapSync) deliberately does NOT toggle it. (2) `MapClient.jsx:488` (central — adversarial pass run) passes `loading={feed.loading}` to `MapBottomSheet` — one prop, no other MapClient change. (3) `MapBottomSheet` renders, while loading, 4 static `h-20` pulse rows (`role="status"` + existing `common.skeleton.loading_businesses` label — mirrors SkeletonProducerGrid, no new copy) and a 120px pulse bar holding the count slot (count string unchanged per MAP-16 constraint); `loading` defaults false → prop-omitted renders the exact pre-change sheet. **Refactor:** the `.skeleton-box` styled-jsx rules were trapped inside `SkeletonProducerGrid` (bare SkeletonLine/`skeleton-box` consumers rendered invisible) — extracted to exported `<SkeletonStyles/>` (single owner, MEH-271), grid + sheet both mount it. Tests: +8 hook loading-lifecycle cases (`useProducersFeed.test.js`) + new `MapBottomSheetSkeleton.test.jsx` (4). Verify: build exit 0 · vitest **861 passed** · 0 physical RTL · self-QA 375px via Playwright route-hold: skeleton during fetch → real list + count on data (screenshots in `qa-artifacts/MEH-1054/`); console clean except documented sandbox-egress artifacts (fonts/tiles/Railway rewrites — MEH-360 class).
- **MEH-1075 — /map filter IA: quick chips + grouped FilterSheet (DRAFT):** row 2 of the /map filter bar (7 flat toggle chips, visually identical to the category row) → 2 quick chips ([מאומתים] [משלוח אליי]) + a "סינון" button (Phosphor `Faders`, badge = active **sheet-only** filters via new `countActiveSheetOnlyFilters`, hidden at 0) opening the new **`FilterSheet.jsx`**: mobile bottom sheet (backdrop, drag-down close, ESC, focus trap+restore, `z-[1200]` per the /map ledger) / md+ panel anchored under the button; 3 labeled groups (תזונה · מקור ואיכות · שירות ואמון) from new `group` metadata on `TOGGLE_CHIPS`; apply = "הצגת {count} בתי עסק" (live client-side `visibleProducers.length`), zero state keeps apply enabled; "ניקוי הכל" resets the 7 toggles only (category+city survive). **One shared chipState** — sheet toggles update it immediately (quick chips sync live) but their refetch is **debounced 300ms** (`onSheetToggleChip`; 2 toggles = 1 request, proven via intercepted Playwright); quick chips keep the instant fetch; `has_delivery` from the sheet still routes through the CityPickerModal guard (modal z-9000 > sheet). Adversarial review caught 2 real issues, both fixed: (1) the mobile sticky bar's `backdrop-blur` (`MapClient.jsx:456`) is a containing block that trapped the `fixed` sheet inside its z-50 context → below-md the overlay **portals to `<body>`**; (2) a pending debounced fetch could clobber a newer instant fetch → every instant-fetch handler now calls `cancelPendingSheetFetch()` (exported for MapClient's inline reset). Also completed `chipState` to all 7 toggle keys in init/`resetAllFilters`/`onResetAll` (diet trio previously worked only via dynamic `!undefined`). NO URL params (spec corrected — /map never had URL sync); `ChipScrollRow.jsx` byte-unchanged; no behavior change to category row / markers / MapBottomSheet / near-me. i18n: new top-level `filters.*` he+en (+`close_aria`, ADR-024 gerund). Tests: new `__tests__/FilterSheet.test.jsx` (8). Verify: vitest **857 passed** · `npm run build` exit 0 · en-parity 2✓ · 0 physical RTL · 375px+desktop Playwright walkthrough green (network-intercepted, MEH-360). **PR #1565 auto-review round (2 must-fix confirmed + fixed):** (1) portal/panel breakpoint aligned **md→lg** — the /map shells switch at `lg`, so the backdrop-blur bar is live through 768–1023px and an md boundary left tablets with a clipped un-portaled backdrop (reviewer's matchMedia-only fix was insufficient — the `md:absolute` classes had to move to `lg:` too or the portaled panel would mis-anchor to `<body>`); (2) focus-hijack on every sheet toggle — inline `onClose` + single `[open, onClose]` effect re-ran focus capture per chipState re-render; fixed at both ends (`useCallback` in FilterChipsBar + focus effect split to key on `[open]` only) + regression test (suite now 9). Also: `.filter(Boolean)` on QUICK_CHIPS, `aria-controls="filter-sheet-panel"`, `filter-sheet:1200` added to the rtl.md z-ledger (frontend.md mirror normalized — its cookie:9998 copy contradicted the MEH-861-verified 1100). Skipped: chip-label i18n (pre-existing pattern across map-chips/ProducersClient — separate sweep ticket). `Refs MEH-1075`.
- **MEH-1081 [T-B] — canonical category axis on /producers (Refs MEH-1081):** `/producers` had no category filter at all — 12 of 18 categories had no browse entry on any consumer surface (MEH-1077 DISC-04). `ProducersClient.jsx` gains a category radio chip row (`ChipScrollRow variant="category"`, "כל הקטגוריות" sentinel first, rows from live `GET /categories` with Rule-19 `CategoriesResponseSchema.safeParse`, self-hides on fetch failure) backed by `?category=<id>` — the same param+type the homepage grid already sends (backend `producers.py category: int` → `ProducerCategory` join; **no backend change** — Phase 0 corrected the ticket's `category=<name>` claim). Category rides the existing `syncUrl`/`fetchFiltered`/mount-init/clear-all machinery (now 4-arg), composes with chips/city/q, adds a removable "× <name>" chip to the active strip, and counts toward `hasActiveChips` (filtered mode + counter + empty states behave as with any filter). i18n: `producers.filters.category_all` he+en (2-line surgical diff). New `ProducersClientCategoryAxis.test.jsx` (4, failing-first: click→URL+fetch, deep-link hydration, compose with organic, all-sentinel+clear-all). Verified: vitest 853 passed · build exit 0 · 0 eslint errors · adversarial review 0 must-fix · Playwright QA 375px+desktop (deep-link tail category, select, compose, clear) under `docs/audits/screenshots/2026-07-meh1081-qa/`.
- **MEH-1083 — homepage diet chips → URL (Refs MEH-1083):** `lib/use-home-page.js` — the homepage URL serializer (`updateURL`) and mount-init covered only 4 of the 7 `CHIPS_CONFIG` keys, so ללא גלוטן/טבעוני/ללא לקטוז filtered results without ever reaching the URL and a refresh/share silently dropped them (MEH-1077 DISC-02; homepage sibling of MEH-1075(b)). Both sites now cover all 7 keys — param names = chip keys (`gluten_free`/`vegan`/`lactose_free=1`; `delivery` keeps its legacy short name). URL-only: chip render, fetch layer (`buildChipParams` already sent all 7), and /map untouched. New `__tests__/useHomePageDietChipsUrl.test.jsx` (4 — toggle→param, 7-key serialize, deep-link hydration, legacy-key regression) written failing-first. Verified: vitest 853 passed · build exit 0 · 11-line diff in one file.
- **MEH-1077 — Discovery anti-pattern audit: reader-journey findings matrix (docs-only, Refs MEH-1077):** new `docs/audits/2026-07-discovery-antipattern-audit.md` — heuristic evaluation of the full reader journey (homepage → category → /producers → /map → /search → /producer/[id] → /events → favorites gate) against NN/g + Baymard + the 12-item directory anti-pattern checklist. 12 CONFIRMED rows (each with file:line + Playwright screenshot evidence, 375px + desktop, local `next start` + seeded Postgres), 4 resolved-regression checks (MEH-920/922/1048/1049 verified still fixed), 3 SUSPECTED, top-10 triage summary. Calibration finding DISC-01 (pre-found #1): homepage category cards filter in place via `router.replace` — no category URL, no history entry, uncrawlable buttons. Highest-value new findings: mislabeled merged-category cards (DISC-03 — "בשר, עוף ודגים" filters בשר only), 12 of 18 categories with no browse entry anywhere (DISC-04), homepage diet-chip URL desync (DISC-02, homepage sibling of MEH-1075(b)). Dedup locked against MEH-1075/819/920/922/611/613/974/1062. Screenshots + interaction log under `docs/audits/screenshots/2026-07-meh1077/`. Zero product-code changes; fixes = Sapir triage → child tickets.
- **MEH-1086 — error-color sweep completion (T1) + ADR index fix (feat, PR #1571, Refs MEH-1073 · Closes MEH-1086):** completes the raw-red→token migration PR #1561 started. **Premise correction (meta-patterns §1):** the ticket asked for a retroactive ADR-026 write, but `ADR-026-error-color-token.md` already existed on staging (PR #1561) — the real docs gap was the decisions index, which ended at 024; added the missing ADR-025 + ADR-026 rows to `docs/decisions/README.md`. **Sweep (error reds only, each site verified ≥4.5:1 on its actual surface):** `ui/Input.jsx` (`border-red-500`→`border-error`, `text-red-600`→`text-error`; on `surface-card` ≈6.5:1), `RegisterClient.jsx:298,325` (`border-red-400`≈2.4:1 → `border-error` 5.76:1 on cream — closes the sibling gap ADR-026 flagged), `VerifyBanner.jsx` (on `background-alt` 5.18:1), `PhoneVerifyCard.jsx` (on white 6.54:1), `RecipeForm.jsx:322` (`role="alert"` error only); `ui-Input.test.jsx` assertion updated. **Deliberately untouched:** RecipeForm required-asterisks (162/188/204) + image-remove button (270) — non-error reds; `BottomNav.jsx:184` `text-[#4b4841]` dropped from scope entirely (gray nav label, MEH-919 AA fix at 4.55:1 — `fg-muted` at 3.53:1 would regress it; a nav-muted token is a separate ADR-019 exception). Verify: compiled CSS contains `.border-error`/`.text-error` rules (first border-scale use of the token), build exit 0, vitest 849 green, adversarial review 0 REFEREE verdicts. `e2e.yml` now upserts a single marker-matched PR comment (`<!-- e2e-qa-report -->`) after every E2E run — `Playwright QA — PASS/FAIL` + run link + head SHA, edited in place on re-push (no comment spam). `if: always()` so failures report too; skipped on push-to-staging runs (no PR) and on the jobs that path-filter/dependabot-skip out (step never reached). `permissions.pull-requests` read→write (comment-only; `contents` stays read). Closes the W0.1 gap from the MEH-1074 SYNC decision: claude-review (ready-only) + e2e (every PR) already cover the QA-agent intent — this adds the missing concise pass/fail surface without a new workflow or any preview-URL traffic.
- **MEH-1074 W0.1 fix — QA comment skips concurrency-cancelled runs (Refs MEH-1074):** observed live within an hour of the step landing: a rapid second push to PR #1568 concurrency-cancelled the in-flight e2e run, and the `if: always()` comment step reported the cancelled outcome as "Playwright QA — FAIL" — a misleading amplification of the workflow-rule-21 superseded-run false-failure class. The step now adds `steps.e2e-run.outcome != 'cancelled'` to its condition; the fresh run upserts the real verdict, cancelled runs stay silent.
- **MEH-1057 — production gate on /upgrade (Refs MEH-1074 · Closes MEH-1057):** the orphan plan-comparison page ("חינם/פרמיום", price "בהמתנה") promised a premium tier while the monetization model is undecided (MEH-617 frozen); reachable by direct URL despite noindex + zero internal links. Gated with `if (process.env.NODE_ENV === "production") notFound()` in **both** `UpgradePage` and `generateMetadata` (dev/components/page.jsx:42 pattern) — metadata short-circuits too so the gated route emits fallback metadata, not upgrade SEO tags. `UpgradeClient.jsx` + all `upgrade.*`/`seo.upgrade.*` i18n keys preserved — the route returns post-launch by deleting the two guard lines. Verified in a real production `next start` + Chromium: `/en/upgrade` renders the full 404 page ("We couldn't find this page"), generic brand `<title>`. **Honest caveat:** the HTTP status streams as 200 (app-wide: a `loading.js` boundary flushes the shell before `notFound()` runs — the truly-missing-route control behaves identically; Next injects noindex meta for streamed 404s), so the DoD's "returns 404" is satisfied at the rendered-experience level, same as every other notFound() surface in the app. Build exit 0 · vitest 849 passed.
- **MEH-1056 — remove the referral_badge "10% הנחה" promise from /register (Refs MEH-1074 · Closes MEH-1056):** the badge ("הגעת דרך חברה 10% הנחה בהזמנה הראשונה", shown when `?ref=` is present) was a blanket platform promise no business opted into — voided by the MEH-1050 merchant-owned-offers ruling — and its "הזמנה" transactional language contradicts magazine-not-marketplace. Removed the MEH-49 render block from `RegisterClient.jsx` and the `auth.register.consumer.referral_badge` key from both locales (he+en, MEH-978 parity). The `referralCode` state + `/referral/claim` call are untouched — referral *tracking* stays live, only the copy promise is gone (same split MEH-1049 made on the producer page). Verify: build exit 0 · vitest 849 passed · `grep referral_badge` = 0 across frontend.
- **MEH-1074 W0.1 — Playwright QA report comment on PRs (Refs MEH-1074):** `e2e.yml` now upserts a single marker-matched PR comment (`<!-- e2e-qa-report -->`) after every E2E run — `Playwright QA — PASS/FAIL` + run link + head SHA, edited in place on re-push (no comment spam). `if: always()` so failures report too; skipped on push-to-staging runs (no PR) and on the jobs that path-filter/dependabot-skip out (step never reached). `permissions.pull-requests` read→write (comment-only; `contents` stays read). Closes the W0.1 gap from the MEH-1074 SYNC decision: claude-review (ready-only) + e2e (every PR) already cover the QA-agent intent — this adds the missing concise pass/fail surface without a new workflow or any preview-URL traffic.
- **MEH-991 Chunk 3 — Playwright VRT baselines for the 6 parity routes (Refs MEH-1074 · Closes MEH-991):** locks the design-parity sweep (Groups 1-5) against silent drift. New `frontend/e2e/visual/parity.spec.ts` — `toHaveScreenshot` per route (/ · /map · producer detail via first `/producers` card · /about · /login · /register) × both projects (desktop 1440×900 + mobile Pixel 5), `maxDiffPixelRatio 0.02` + `animations: disabled` + `caret: hide` set once in `playwright.config.ts` `expect.toHaveScreenshot`; `testMatch` extended to `e2e/visual/**`. **Determinism strategy (no mocks — MEH-417):** live-data regions are *masked* (producers grid, mini-map/Leaflet, producer h1/images/rating anchor, /about Cloudinary portrait); calendar-dependent banners are *hidden* via injected `parity.css` (`stylePath`) — HolidayBanner + FridayDeliveryStrip got `data-testid`s for it (presence varies by date; a mask can't absorb the layout shift of a section that appears/disappears); data-dependent routes shoot viewport-only (below-fold sections self-hide on empty data → fullPage height unstable), static trio shoots fullPage; cookie consent pre-seeded via `addInitScript`. Producer-detail keeps the flows/03 graceful skip on an empty DB. **Baselines are runner-generated:** new `.github/workflows/vrt-update.yml` (workflow_dispatch, `contents: write`) rebuilds + `next start`s on the runner (mirrors e2e.yml MEH-1044 steps), runs `--update-snapshots`, and commits `*-snapshots` back to the dispatched branch — dev machines/CC sandbox have different font stacks, so locally-generated baselines would red-line every CI comparison; this is also the refresh path after any intentional visual change. Sandbox smoke: 10 passed / 2 graceful-skips (producer detail — no backend egress) via `playwright.local.config.ts`. Also scoped the home hero-search locator to `role="search"` (header mounts a second HeroSearch → strict-mode ambiguity; the duplicate `id="hero-search-input"` is a discovered a11y bug, ticketed separately). `Refs MEH-1074` · `Closes MEH-991`.
- **MEH-1076 — Apply MEH-1044 e2e.yml (E2E → local `next start`) — the RED-tier half #1525 left unapplied (Refs MEH-1044/MEH-1074 · Closes MEH-1076):** MEH-1044 (Done, #1525) shipped only its CC-editable half (dependabot throttle + `playwright.config.ts` baseURL); the actual `e2e.yml` E2E→localhost move was authored only in #1525's PR body for Sapir to apply manually and **was never applied** — live `e2e.yml` still triggered on `deployment_status` and ran Playwright against the live Vercel preview, i.e. the 2026-07-07 Edge-Requests burn (Hobby paused at 319%) was still live. Now that ADR-016 v2 makes `.github/workflows` CC-editable, applied the authored replacement directly: trigger `deployment_status` → `pull_request`+`push:staging`; in-runner `npm run build` + background `next start -p 3000` + hard localhost readiness poll; `PLAYWRIGHT_BASE_URL=http://localhost:3000`; backend unchanged (`NEXT_PUBLIC_API_URL` = Railway staging). Job `name:` fields kept verbatim (`Paths filter`, `Playwright E2E (Vercel preview)`) so required-check identities are unchanged; dependabot skip re-keyed `deployment.ref`→`github.actor`; MEH-424/499/675 paths-filter + MEH-484 flake gate + trace upload preserved. `VERCEL_AUTOMATION_BYPASS_SECRET` no longer used by the workflow. Content byte-for-byte the #1525-authored, locally-verified version (73 passed; its 4 failures were CC-sandbox egress artifacts). `frontend/vercel.json` `ignoreCommand` for `dependabot/*` preview-skip stays a Sapir manual step (deploy-config-protected). Zero `*.vercel.app` traffic in E2E runs going forward.
- **MEH-925 — Skip Claude PR review on draft PRs (Refs MEH-1074 · Closes MEH-925):** `.github/workflows/claude-review.yml` — the `review` job now carries `if: github.event.pull_request.draft == false`, and the `pull_request` trigger adds `ready_for_review` to `types` so a PR opened as a draft still gets reviewed the instant it flips to ready. Every push to an open draft otherwise re-ran the Anthropic `claude-code-action` (~11% of monthly Actions minutes + API$, MEH-925 usage report). Safe because `review` is NOT a required check (`continue-on-error: true`, calibration) → draft-skip can never block a merge. Workflow-YAML only; additive (9 insertions, 1 deletion); no change to the review prompt, permissions, or concurrency key.
- **MEH-1072 — Header pill fixed geometry (DRAFT):** the desktop floating nav pill (`components/Header.jsx`) switched WIDTH on scroll — wide at rest (`gap-14 px-11` end-cap + `gap-11` lead-group intra-gap), snapping compact (`gap-8 px-4` + `gap-9`) at the y=60 threshold (MEH-899). Retired per Sapir 09/07 + NAV-01: geometry is now **fixed compact** (`gap-8 px-4` end-cap, `gap-9` lead-group) at every scroll position — the pill reads one consistent size on every page, at rest and scrolled. The two `!scrolled ? …` width ternaries (`:266`, `:276`) became static classes. **Untouched:** the MEH-947 three-way SURFACE ternary (homepage at-rest `/85`, scrolled `/60`, inner-page solid — surface still varies on scroll, only geometry is frozen); trust strip; logo; NAV_ITEMS; action cluster; mobile branch. `gap`/`px` deliberately stay OUT of the `transition-[…]` allowlist (`:236-238`, MEH-732 guardrail intact — no animated reflow). MEH-899 width comments updated to record the supersession; MEH-890/896/947 surface history kept intact. Grep confirmed no other site branches on `scrolled` for WIDTH (only the two target ternaries; `scrolled` still drives the surface + scroll handler). Test: new non-skipped block in `__tests__/Header.test.jsx` (3 — `gap-8 px-4` present at scrollY=0, still present after simulated scroll >60, lead-group `gap-9` never `gap-11` — geometry scroll-independent). DESIGN-GAP-MATRIX NAV-01 appended. Verify: `npx vitest run __tests__/Header.test.jsx` 3/3, `npm run build` exit 0. Files: `Header.jsx` + `Header.test.jsx` + `DESIGN-GAP-MATRIX.md` (NAV-01) + HANDOFF. `Refs MEH-991` · `Closes MEH-1072`.
- **MEH-1071 — VerifyBanner brand restyle + per-session dismiss (DRAFT):** the email-verify nudge (`components/VerifyBanner.jsx`, first block of `<main>`) used an off-token amber surface (`bg-amber-50`/`border-amber-200`, `text-amber-800`) and had no way to dismiss it. Restyled on-brand: surface → `bg-background-alt` + `border-b border-border`, text → `text-text`, the `EnvelopeSimple` glyph → `text-accent` (gold); the resend link (`text-primary`), success (`text-green-700`) and error (`text-red-600`) states are unchanged. Added a **per-session dismiss** — a Phosphor `X` on the end side (44px tap target, `aria-label="סגירת הודעת אימות"`) writes `sessionStorage["verify-banner-dismissed"]="1"` and hides the banner for the session; it returns in a fresh browser session. SSR-safe: the dismiss state reads sessionStorage only behind a `typeof window` guard (the banner never renders server-side — `user` is null there — so no hydration mismatch). Render gate now `user && email_verified === false && !dismissed`; resend logic + rate-limit UX + all copy unchanged; zero emoji; Phosphor-only (ADR-013); no new tokens (ADR-019); logical RTL props only. New `__tests__/VerifyBanner.test.jsx` (4: renders unverified · hidden verified · hidden after dismiss · sessionStorage set on dismiss). Verify: `npx vitest run` 4/4, `npm run build` exit 0. Files: `VerifyBanner.jsx` + new test only. `Closes MEH-1071`.
- **MEH-1070 — hero CTA row recenter (DRAFT):** the homepage hero CTA row (`HomeHero.jsx:143`, גלו עסקים + near-me + how-it-works) carried `md:justify-start` + `md:px-12` from #1476's start-aligned geometry; with the centered hero content those two classes are dropped so the row is `justify-center` at every breakpoint (mobile was already centered — desktop now matches). `border-primary` on the near-me button is retained (untouched). Adjacent comment updated to record the centering supersedes HOME-06 alignment per Sapir 09/07; DESIGN-GAP-MATRIX HOME-06 Notes appended. Files: `HomeHero.jsx` + `DESIGN-GAP-MATRIX.md` only. Verify: `npm run build` exit 0. `Refs MEH-991` · `Closes MEH-1070`.
- **MEH-1048 — Producer trust strip: rating + review count + one review excerpt next to the h1 (DRAFT):** social proof at the decision point — the header only showed badges; reviews sat deep down the page. YELLOW, chunk-by-chunk, all frontend-only. **Phase 0:** `avg_rating`+`reviews_count` already ship on `ProducerDetailOut` (inherits `ProducerListOut`, `schemas.py:758-759`) → **no backend change**; the review *quote* is the only piece not at page load (reviews are IO-lazy via `useLazyReviews`). **Chunk 1:** the static rating chip in `ProducerHeader.jsx` became an anchor `<a href="#reviews">` reading `★ 4.8 · N ביקורות` (reuses the existing plural `review_count` key — no new copy); rating decimal is `dir="ltr"` + `.numeric` (RTL flip guard); `reviews_count > 0` guard unchanged → zero reviews renders nothing. `ProducerSections.jsx` got `id="reviews" scroll-mt-24` on the always-rendered (IO observation-point) reviews wrapper so the anchor is valid before the lazy mount. **Chunk 2:** new co-located client `ReviewExcerpt.jsx` — eager-fetches page 1 of `/producers/{id}/reviews` (≤10, newest-first) **only when `reviews_count > 0`** (zero-review producers make no call); shows the most-recent review **with non-empty body**, **falling through** rating-only reviews; none-with-text → renders nothing (pill still shows); truncates ≤120 chars + `…`; links to `#reviews`. Confirmed no `limit` param exists on `/reviews` — page-of-10 is the smallest bounded fetch and is required for the fall-through anyway (Option B: a computed `ProducerDetailOut.top_review` field is the documented future optimization if load demands it). i18n `producer.detail.header.review_excerpt_aria` (he+en). **Reviewer follow-ups:** the excerpt's accessible name is the quote itself (nav purpose moved to an `sr-only` suffix — an `aria-label` would override and hide the quote from AT users); fetch error → `Sentry.captureException` (fail-open, not a silent-except MEH-325, and avoids the unused-`no-console`-disable lint error). Tests: `ProducerHeaderTrustStrip.test.jsx` (3) + `ReviewExcerpt.test.jsx` (5 — zero-fetch guard, newest-with-text, fall-through, none-with-text, >120 truncation). `npm run build` exit 0, vitest **832 passed**, 0 physical RTL. Verified 375px + desktop: with-reviews / zero-reviews / with-text-excerpt / all-rating-only. `Closes MEH-1048`.
- **MEH-1062 — SEO audit + structured-data gap-fill (feat, closes on PR #1545):** two PRs under one ticket. **Chunk A (docs, PR #1541):** bootstrapped `.agents/product-marketing-context.md` (derived only from BRAND.md / CONTEXT.md / COPY_BANK.md — 0 emoji, 0 banned brand strings, magazine-not-marketplace, ADR-011 tagline, no invented metrics) + `docs/seo-audits/2026-07-seo-audit.md` (full technical SEO audit — SEO-NN findings table, top-10, coverage of titles/descriptions/canonical/hreflang/OG-Twitter/sitemap/indexability/internal-linking/image-alt/CWV). **Phase 0 headline (STOP condition a):** a full server-rendered structured-data system already existed beyond FAQPage — producer `FoodEstablishment`+`BreadcrumbList`+`WebPage`+`WebSite`+`Organization` via `lib/seo.js`, homepage `Organization`+`WebSite`+`SearchAction`, `Recipe` via `RecipeJsonLd.jsx` — so the ticket premise ("implement missing JSON-LD") was an orchestrator error; B–D collapsed + re-scoped to the genuine gaps. **Chunk B (code, PR #1545):** `lib/seo.js` — extracted shared `buildBreadcrumbList()` (single owner per MEH-271; producer JSON-LD byte-identical, diff-verified both locales), added `buildEventJsonLd()` (`@graph` [Event, BreadcrumbList], `EventOut` fields only, omit-never-invent) + `buildRecipeBreadcrumbJsonLd()` + producer Twitter card; `events/[id]/page.js` server-renders the Event JSON-LD + Twitter card; `[slug]/recipes/[recipe_id]/page.jsx` server-renders a recipe `BreadcrumbList` (RecipeJsonLd.jsx untouched) + Twitter card; `sitemap.js` emits recipe detail URLs per-locale (per-producer, concurrent, fail-open). Verify: `npm run build` exit 0, 63 existing SEO/sitemap tests green, event/recipe payload sanity (JSON.parse + required fields). Out of scope (Sapir): SEO-04 producer dual-URL canonical (needs a canonical-direction decision → own ticket); SEO-08 stays with MEH-918. `Closes MEH-1062`.
- **MEH-1000 — admin notification on recipe submission (DRAFT):** closes the MEH-997 journey-1b gap — the `/admin/recipes` queue was poll-only (recipe create path had zero notification calls) while producer registration already pinged admin. New `notify_admin_new_recipe(producer_name, recipe_title)` in `services/auth_notifications.py` — same channel as `notify_admin_new_producer` (free-text WhatsApp `send_text` to `admin_whatsapp_to` + email to `admin_email`, no new Meta template), Hebrew copy with "בית עסק", deep-link to `/admin/recipes`; whole body try/except-logged (MEH-977 — observable fire-and-forget, never breaks the submission). Wired via `BackgroundTasks` in `producer_recipes.py`: on **create** (after commit, primitives captured pre-task per the auth.py expire_on_commit pattern) and on **update when a content change moves the recipe INTO pending from another status** (resubmit-after-needs_revision; anti-spam guard — editing an already-pending card does not re-ping; REJECTED verdicts never ping). MEH-997 journey-1b probe flipped from documents-the-gap to mock-capture PASS (expected recipient args asserted). New `tests/test_meh1000_recipe_admin_notification.py`: service recipients/content/fail-open + router wiring (create fires, rejected doesn't, resubmit fires once, metadata-only and still-pending edits don't, notify failure never fails the 201). `update_my_recipe` carries an inline `# noqa: PLR0913` (6th arg is FastAPI-injected `BackgroundTasks`). Full pytest green locally. `Refs MEH-1000`.
- **MEH-1047 — Producer hero redesign: photo-first editorial gallery grid (Direction B, DRAFT):** the imaged state of `/producer/[id]` was a single full-width `h-52` banner carousel; redesigned to a photo-first grid (Airbnb-adapted, brand "Editorial Asymmetric" direction — Sapir FINAL-approved in Claude Design). YELLOW, chunk-by-chunk with WAIT gates; all changes in `frontend/components/ImageGallery.jsx` (imaged path only — the MEH-815 Tinted Masthead empty state is **byte-identical**). **Chunk 1 — desktop grid (md+):** hero at inline-start (~62%, `grid-cols-[62%_1fr]`) + tall stacked secondary column, `gap-2`/`rounded-xl`/`border-accent/30` gold hairline/`max-h ~460px`. Densities: **4+** = hero + `images[1]`/`images[2]` with a single **"כל התמונות (N)"** gold `accent` pill (N = total, `Images` glyph) on the bottom stacked cell (Sapir correction: hero+2 stacked = 3 cells, single overlay = pill only, no "עוד N" scrim); **3** = same grid; **2** = hero + one tall companion (no pill); **1** = full-width banner unchanged. Cells open the existing `Lightbox` at their index (`openLightbox(i)`); new i18n `gallery.view_all` (he+en, MEH-978 parity). **Chunk 2 — mobile 375px + FavoriteButton fix:** the `md:hidden` carousel loses dots + prev/next arrows for a counter chip (1/N, `.numeric`-isolated, top-end) + a thin **gold progress bar** (`accent` fill tracking the slide); swipe navigates, tap opens the lightbox. **Fixed a chunk-1 regression** (automated review, PR #1533): FavoriteButton was trapped inside the now-`md:hidden` carousel → invisible on desktop for 2+ images. Hoisted to a **single** shared `relative`-wrapper overlay pinned `top-3 start-3` (`z-20`) across both layouts — one mount, one `/users/me/favorites` fetch (it fetches on mount). **Chunk 3 — LCP + polish:** `images[0]` eager `priority` on both the grid hero and the mobile banner (LCP per breakpoint), every other image lazy (next/image default) — dual-tree double-preload is bounded by `sizes` (documented tradeoff). RTL: removed the physical `right-3`/`left-3` arrows + centered dots → **zero `rtl-ok` markers** left in the file (all logical/symmetric props). Tests: new `__tests__/ImageGalleryGrid.test.jsx` (density structure, pill count, FavoriteButton single-instance regression guard, LCP priority, mobile counter/progress). `npm run build` exit 0, vitest **818 passed**, 0 physical RTL. Screenshots (375px + desktop, Playwright vs the real component) use the branded leaf placeholder — Cloudinary egress is blocked from the CC sandbox (MEH-360 class), so live-photo capture deferred to the Vercel preview. `Closes MEH-1047`.
- **MEH-1027 Chunk B — producer delete: native confirm → modal dialog + AdminRowMenu aria-disabled fold-in (DRAFT):** closes the destructive-action-safeguard pattern on the producers surface. `use-admin-producers.js:151` — the last guard on `DELETE /admin/producers/:id` was a native browser confirm; `deleteProducer(id, name)` (signature unchanged — Chunk A kebab wiring untouched) now only opens a `confirmDelete {id,name}` dialog, and the DELETE fires solely from `confirmDeleteProducer` through the existing `run()`/`delete:${id}` busy key + MEH-747 `delete_error` toast. New `DeleteConfirmDialog` in `page.js` mirrors the MEH-1023 Ch.B contract (`role="dialog"`/`aria-modal`/`aria-labelledby`, Escape-unless-deleting, cancel disabled mid-delete, closes only on success — failure keeps it open with the toast); `deleting` label drives off `isBusy` (no new local flag). **A11y fold-in (auto-review #1524):** `AdminRowMenu.jsx` `disabled={it.disabled}` → `aria-disabled` + click-guard before `setOpen` (WAI-ARIA APG — busy items stay focusable; menu stays open on no-op click); disabled styling moved to conditional classes. **Zero users-page regression:** items without `disabled` render no attribute — locked by test. i18n: only `producers.table.deleting` added (he+en); `confirm_delete` reused verbatim. Tests failing-first: new `AdminProducerDelete.test.jsx` (5 lifecycle cases mirroring `AdminContentCategoryDelete`), aria-disabled updates in `AdminRowMenu.test.jsx` + `AdminProducersTableActions.test.jsx`, `toHaveClass` nit fixed. Verified: build exit 0 · vitest **815 passed**/41 skipped · 0 eslint errors · 0 physical RTL · en-parity 2✓ · `grep -c "confirm(" use-admin-producers.js` = 0 · `grep -rc 'disabled={it.disabled}'` = 0 · adversarial review 0 must-fix (note: focus-trap gap is pre-existing pattern-wide, MEH-1023 follow-up candidate). `Closes MEH-1027`.
- **MEH-1049 — ProducerContactCard CTA hierarchy: single primary + remove "שתפו וקבלו 10%" chip (DRAFT, closes MEH-1049):** two-chunk YELLOW, WAIT-gated. Phase 0 mapped all 17 CTA/share/refer affordances across the 3 zones on `/producer/[id]` (desktop `ContactSidebar`, mobile `ActionRow`, `StickyContactBar`) and found the page was *already* largely single-primary — WhatsApp (`btn-whatsapp` #25D366) the sole filled button everywhere except one spot. **Chunk 1 — `FollowButton.jsx` demoted to secondary weight** (button-className only): its `following` state was a full green fill (`bg-primary text-white`) that read as a competing primary, and its not-following state (`border-primary text-primary`) out-weighed the neutral phone/share tiles. Now `following → bg-primary/10 text-primary border-primary/40` (subtle primary-token tint, not raw palette per Sapir), `not-following → border-border text-fg-muted` (neutral ghost). 44px targets, labels, follow semantics, sticky bar + primary CTA all preserved → WhatsApp is now the single filled primary in the contact card. **Chunk 2 (Sapir-approved 08/07) — removed the platform-wide "שתפו וקבלו 10%" referral chip** from `ActionRow.jsx` (was gated on `user.referral_code`): a platform promise made on businesses' behalf without opt-in (merchant-owned offers → MEH-1050, GBP Offers pattern). Grep proved `user` had zero remaining consumers in ActionRow → dropped the orphaned `user` prop from the destructure + the `ProducerDetail` call site (`user` stays live there via `isOwner`). Cleaned the two now-unused i18n keys `producer.detail.action_row.referral_cta` + `.referral_msg` from **both** locales (he+en, MEH-978 parity); `auth.register.consumer.referral_badge` is a separate feature, left in place. UI-only — referral backend (`/referral/claim`, `referral_code`) and `rating_dispatcher` (MEH-796) untouched. `npm run build` exit 0, vitest **759 passed**, both locale JSONs parse. Screenshots (375px + desktop, token-accurate mocks — live capture not reachable from CC sandbox: FollowButton needs auth+seeded producer) → Sapir mobile QA on Vercel preview. `Closes MEH-1049`.
- **MEH-1045 — bot hardening: catch-all fast-404 + robots.txt + sitemap host fix + localeDetection:false (DRAFT):** vector #2 of the 2026-07-07 Edge Requests incident (MEH-1044 = vector #1). **(1) Fast-404** — the root catch-all (`app/[locale]/[slug]/page.js`) rendered + hit Railway (`revalidate:60` per unique slug) for EVERY scanner probe (`/wp-admin`, `/.env`, `/xmlrpc.php`); new exported `isSlugShaped()` guard rejects non-backend-shaped paths (dot anywhere, `wp-`/`wordpress`/`xmlrpc`/`phpmyadmin`/`cgi-` prefixes, >100 chars, outside the `_slugify` charset `[a-z0-9_\u0590-\u05FF-]`, `producer_import.py:34-41`) BEFORE any backend fetch; RESERVED set intact; legit Latin+Hebrew slugs unaffected. **(2) robots.txt** — full disallow for GPTBot/CCBot/Amazonbot/Bytespider/PetalBot/ClaudeBot/meta-externalagent (search engines stay allowed); **sitemap host bug fixed** — pointed at mehamakor.co.il while the live site is mehamakor.online. **(3) routing.js** — `localeDetection: false`: cookie-less clients (most bots) no longer get a 307 `/`→`/en` on every root hit (hreflang per MEH-476 keeps SEO intact). New `SlugPageBotHardening.test.jsx` (24 cases: guard shapes + page-level "404 without serverFetch" + legit-slug fetch). middleware.js untouched. `Closes MEH-1045`.
- **MEH-1044 — Vercel Edge Requests burn: E2E moves to local `next start` in CI + dependabot burst throttling (DRAFT):** incident 2026-07-07 — Hobby team paused at 319% of the 1M Edge Requests tier; discovery attributed ~1–2M/mo to the E2E suite hitting live preview URLs (fresh browser context per test = zero cache = every `_next/static` chunk re-downloaded, ~50+ req/page × ~80 test executions × ~8 deploys/day). **CC-editable shipped:** `playwright.config.ts` baseURL now `PLAYWRIGHT_BASE_URL || TEST_URL || localhost:3000` (CI targets localhost; manual staging/preview runs via TEST_URL unchanged); `dependabot.yml` limits 5→3 per ecosystem + npm minor/patch grouped into one PR (the 07-06 burst was 8 separate PRs → 8 preview deploys). **RED-tier content authored, NOT committed (Sapir applies):** full replacement `e2e.yml` — trigger `deployment_status`→`pull_request`+`push:staging`, in-runner `npm run build` + background `next start` + readiness poll, `PLAYWRIGHT_BASE_URL=localhost:3000`, backend strategy unchanged (Railway staging via rewrites); job `name:` fields kept verbatim; MEH-547-era cost fixes + MEH-484 flake gate + MEH-424/499/675 paths-filter preserved (dependabot skip re-keyed `deployment.ref`→`github.actor` since the event changed). **Also in PR body for Sapir:** `frontend/vercel.json` `ignoreCommand` to skip preview deploys on `dependabot/*` branches (file is deploy-config-protected in CC). Full suite verified locally against `next start` (summary in PR). `Closes MEH-1044`.
- **MEH-1051 — producer_changes_requested_v1 WhatsApp on request-changes (DRAFT):** the MEH-1011 admin request-changes endpoint notified the producer by email only; now that the template is Meta-approved (Active, UTILITY, he, 2 body params, no buttons) the endpoint also WhatsApps the producer. New `ProducerChangesRequestedV1` in `whatsapp_templates.py` (fields in {{n}} order: `producer_name`, `missing_details` — first 2-param template; Phase 0 verified the base `to_components()` emits N params in declaration order, `whatsapp_templates.py:68-76`). New `notify_producer_changes_requested(name, phone, feedback)` in `auth_notifications.py` — exact mirror of `notify_producer_approved` (preflight kind `changes_requested` + `_normalize_il_phone` + typed class + belt-and-suspenders try/except + same log shape), plus `_sanitize_wa_param`: strips `\r`, `\n`/`\t`→space, collapses 2+ spaces, trims, truncates 550 (Meta rejects newline/tab/4+-space params; precedent `experience_notifications.py:65`). Wired in `admin.py:627` after the producer email block — post-commit + fail-open, WhatsApp can never affect the 200. Template shape LOCKED (no URL button — MEH-509 400 precedent). New `tests/test_changes_requested_whatsapp.py` (5: two-params-in-order+no-button · multiline-sanitized · no-phone-skip+200 · meta-raise-fail-open+200+commit-preserved · 550-truncation); regression `test_meh_509_pr1_hooks`+`test_whatsapp_notify` 12✓. `Closes MEH-1051`.
- **MEH-1027 Chunk A — admin producers row: secondary/destructive actions → AdminRowMenu kebab (DRAFT):** the `/admin/producers` actions column stacked up to 7 inline buttons per row — the same anti-pattern MEH-1023 fixed on users/categories, worse here (destructive delete exposed inline). `ProducerActions` (`AdminProducersTable.jsx:143`) now keeps **inline**: quick-approve + request-changes (pending rows), edit link, view link (spec-silent → untouched, Sapir-approved default); moves **into the MEH-1023 `AdminRowMenu` kebab**: toggle-status (השהה/הפעל — approved/inactive), toggle-ambassador (approved), toggle-story-card (approved+slug — still opens `StoryCardCanvas` below the row, wiring unchanged), delete (`tone="danger"`). Same conditional guards, same handlers, same `isBusy` keys — trigger location only; ambassador + story behavior unchanged (reposition only, thesis-aligned features preserved per issue Phase 0). **`AdminRowMenu` gains an additive per-item `disabled` prop** (Sapir-approved scope extension — mirrors the inline buttons' busy guards; disabled item renders inert, menu stays open; users page unaffected). i18n: new `producers.table.actions.menu_aria` he+en twin (MEH-978). Tests: `AdminProducersTableActions.test.jsx` +8 (menu composition per status · danger tone · story/ambassador wiring · busy-disables-item · inline-stays), `AdminRowMenu.test.jsx` +2 (disabled inert + enabled default) — written failing-first (rule 5). Verified: `npm run build` exit 0 · vitest **765 passed**/41 skipped · 0 eslint errors · 0 physical RTL classes · en-parity 2✓ · adversarial review 0 must-fix (2 QA notes: non-portal menu clips at table edge — same accepted trait as users page; ambassador/story hover-titles don't carry into menu items). Chunk B (producer-delete confirm dialog) NOT started — waits for Sapir. `Refs MEH-1027`.
- **MEH-1046 — /admin/users client-side pagination (DRAFT):** the users table rendered all 500+ fetched rows in one DOM pass (`users/page.js:121` mapped the full array) — heavy DOM, slow mobile scroll (Sapir's density complaint). Added client-side pagination per the Sapir 05/07 scope ruling (slice the already-loaded array; server-side `?page=&limit=` deferred post-launch): `page`+`pageSize` state, default 25 with a 25/50/100 selector, prev/next + "עמוד X מתוך Y", `currentPage` clamped so shrinking result sets never strand past the end. **Phase-0 correction to the ticket's evidence:** the search/role filters are SERVER-side (`load()` query params, `users/page.js:33-36`) — there is no client filter "before :121" — so `users` already IS the filtered set and slicing it directly satisfies the filtered-pagination criterion. Page resets on role change + search submit (new `searchUsers()`), deliberately NOT on every `load()` — block/promote reloads keep the admin on their current page. Fetch, `applyRole`/`toggleBlock`, AdminRowMenu, and the MEH-1023 confirm dialog untouched. Controls hidden on empty set; text prev/next buttons in flex flow (direction-neutral, 0 physical RTL classes). i18n `admin.users.pagination.*` 4 keys (he+en twins). New `AdminUsersPagination.test.jsx` (7 cases: default-25/prev-disabled · next-advance+last-page-bound · page-size-reset · role-filter-reset · search-reset · single-page-bounds · empty-no-controls). `Closes MEH-1046`.
- **MEH-1025 Chunk B — producer-side "נשאר להשלים" banner on the dashboard Overview (DRAFT, closes MEH-1025):** the owner-facing end of the request-changes flow. New co-located `ChangesRequestedBanner.jsx` renders on the Overview when `profile.requested_changes` ≠ null (from `GET /producers/me`, exposed by Chunk A #1518) — **accent** (gold) tint card, `ClipboardText` icon, title "נשאר להשלים", the admin's feedback text as-is (`whitespace-pre-wrap`), `changes_requested_at` wrapped `dir="ltr"` (RTL date-flip guard, MEH-1011 Ch2 precedent), CTA "להשלמת הפרטים" → `/producer/dashboard/edit`; `requested_changes` null → **0 DOM**. **Sapir steers:** (1) title+body use dark `text-text` (not accent-gold) for WCAG on the pale tint — only the icon/CTA carry accent; (2) the generic yellow "ממתין לאישור" banner is **suppressed** when `requested_changes` is set (`page.js:259` guard `status==="pending" && !profile?.requested_changes`) — the specific banner IS the message, the generic one would contradict it (the ball is in the owner's court, MEH-579 subject-test). Read-only display: no edit/dismiss/resubmit, no endpoint/schema (Chunk A shipped the field), no admin-side changes, no 1A–1D internals touched. i18n `dashboard.producer.changes_requested.*` (title/cta/aria, he+en, MEH-978); body is the DB text. New `Dashboard1025Banner.test.jsx` (4 — renders+CTA+LTR-date when set · 0 DOM when null · generic pending suppressed when set · pending shows when null). `npm run build` exit 0, vitest **759 passed**, 0 physical RTL. Draft PR → Sapir mobile QA (Rule 23; confirm accent-tint contrast at 375px). `Closes MEH-1025`.
- **MEH-1030 — registry-path-drift guard (validator + pre-commit) + residual sweep + testing.md required-checks correction (DRAFT):** guard/config registries that list repo file paths silently disable their guard when a refactor moves/deletes a listed file — caught reactively twice (MEH-668 `rtl-allowlist.txt`, MEH-1026 `central-components.json`, both post-`[locale]`). New **`scripts/validate-registry-paths.py`** (stdlib `json`+`pathlib`, mirrors `scripts/check_api_contract.py`): a `REGISTRIES` config maps each registry → a parser; asserts every listed path resolves, exits non-zero + lists `registry:line → missing path` on any miss, exit 0 clean. Covers the **2** path registries with their formats — `central-components.json` (`components[]` JSON array) and `rtl-allowlist.txt` (**only** the `PATH EXCEPTIONS` section between the `====` markers; the `CONTENT PATTERNS` `rtl-ok` marker is not a path). Wired as a `repo: local` **pre-commit** hook (`.pre-commit-config.yaml`, `pass_filenames:false` — always validates the full set) that runs on any change to a guarded registry or the validator. **Residual sweep (1 found):** `rtl-allowlist.txt:10` still listed `…/map/components/DesktopMiniPopup.jsx`, deleted in MEH-1010 — **CC cannot write `.claude/hooks/**` (settings deny), so this one-line removal is a paste-diff for Sapir** (in the PR body); the validator flags it until applied (demonstrating the guard works: exit 1 now, exit 0 after — proven via temp-copy simulation). **testing.md correction (MEH-1013 residual, broadened per Sapir):** the "Required status checks = 6 + MEH-736 docs-only twin jobs" paragraph was doubly wrong — the `protect-staging` ruleset (ID 15240090, verified against the ruleset API 2026-07-04) requires **only 2 aggregator gates** (`CI gate` + `Deploy gate`); the individual named jobs skip on docs/config diffs (expected — skipped deps still let the aggregate report success), and no twin jobs exist or are needed. Rewritten to the verified reality (matches how #1012/#1026/#1485 actually merged). Documented in `.claude/rules/testing.md` ("Guarded registries" — the 2 registries + how to add one). No new deps, no `.github/workflows/**` edits (CC-deny MEH-671 + MEH-787 collision — a CI-gate form is a separate A2 follow-up). `npm run build` green. `Closes MEH-1030`.
- **MEH-1040 — admin reviews delete: native confirm → modal dialog (DRAFT):** review delete (`reviews/page.jsx:49`) was the LAST admin surface still guarded by native `window.confirm` (users/categories fixed in MEH-1023, producers tracked in MEH-1027) — closes the destructive-action-safeguard pattern across the whole admin panel. `handleDelete` now only opens a `confirmDelete` dialog (full review row in state); the `DELETE /reviews/:id` fires solely from `confirmRemove` — call, optimistic row removal, `deletingId` logic and toasts unchanged. Dialog mirrors the MEH-1023 Ch.B markup/a11y contract (`role="dialog"`/`aria-modal`/`aria-labelledby`, Escape closes unless mid-delete, Cancel disabled while deleting, closes only on success — stays open on failure with the existing generic error toast), copy reuses `reviews.confirm_delete` ({user}+{producer}); **zero new i18n keys** (title/confirm/cancel all pre-existing). New `__tests__/AdminReviewDelete.test.jsx` (5 cases mirroring `AdminContentCategoryDelete.test.jsx`: open-shows-names/no-DELETE · cancel · Escape · confirm→DELETE+close+row-removed · failure-keeps-open). `Closes MEH-1040`.
- **MEH-1034 — CategoryOut.producer_count: blast radius בדיאלוג מחיקת קטגוריה + count per row (DRAFT):** the backend follow-up Sapir deferred from MEH-1023 Ch.B. `CategoryOut` (`schemas.py:294`) gains an **optional** `producer_count: int | None` — query-time only, NO column/migration; public consumers (`GET /categories` `producers.py:336`, nested `ProducerOut.categories` `schemas.py:757`) serialize `null`, unaffected. `list_categories_admin` (`admin_extra.py:165`) annotates each row in a single `outerjoin(ProducerCategory) + func.count + group_by` query (no N+1). Frontend: the MEH-1023 Ch.B delete dialog copy becomes "מחיקת '{name}' — {count} בתי עסק משויכים" and each `CategoryRow` shows "{count} בתי עסק" (the categories editor is a `<ul>`, not a table — per-row element instead of a column). i18n: `content.categories.confirm_delete` updated + new `producer_count` (he+en twins, ADR-024). Tests: new `test_categories_producer_count` (0 for empty / N for associated) + `AdminContentCategoryDelete.test.jsx` updated for the new copy (5✓). `Closes MEH-1034`.
- **MEH-995 — /join producer-recruitment landing (positioning-FINAL port, DRAFT):** the info for prospective business owners was scattered across /about/process + /about/for-businesses + the wizard with no single URL to send anyone to. New static SSG route `frontend/app/[locale]/join/page.js` (server component, for-businesses metadata pattern incl. buildAlternates/OG): hero (eyebrow "לבתי עסק מקומיים" gold 0.14em · H1 "העסק שלכם. עמוד משלו." Frank-Ruhl-black · softened editorial subhead — the discovery variant is forbidden) → single CTA "מצטרפים"→/register/producer + trust hint "חינם להצטרף" → "איך זה עובד" 4 numbered spreads (Cormorant `.font-english`, `dir="ltr"` + `leading-none` no-bleed) + link →/about/process → "מה כדאי להכין" checklist (mirrors MEH-994 pre-flight verbatim — keep in sync) → reassurance card (positive framing, MEH-880 tokens) → **self-describing testimonial placeholder** (no invented business; Template-10/MEH-931 verbatim intake swaps it pre-launch, COPY_BANK §8 guardrail) → FAQ teaser "כמה זה עולה?" carrying the no-fees LOCK (Etsy pattern — price sits in FAQ, never the headline) + link →/about/for-businesses. Zero premium mention page-wide (MEH-617). `join.*`+`seo.join.*` he + en twins (surgical text insert — zero reformat, parity guards green); Footer `add_business` repointed /register/producer→/join (single canonical door, label unchanged); **COPY_BANK.md §9** locks all /join strings; MANUAL_TESTING.md § MEH-995. Verify: build exit 0, vitest 740✓/41 skipped, eslint 0 errors, 375px+320px+desktop screenshots, CTA href verified in-browser. `Closes MEH-995`.
- **MEH-999 (B10) — PUT /producers/me license gate grandfathers held categories (license-pending un-brick, DRAFT):** the MEH-530 gate at `producer_me.py` re-ran `ensure_license_for_categories` against the producer's FULL effective category set on every PUT, so a MEH-971 license-pending producer (license-required category + NULL license) got 422 on **any** profile edit — bio, image, contact — until she dropped the category (MEH-999 dogfood blocker #3). Fix (Option B, no schema — verified no `license_pending` column exists on `Producer`): new `_enforce_owner_license_gate` helper (extracted — the inline version tripped C901) validates **NEWLY-ADDED `category_ids` only** (`set(payload) − persisted`); a bodily edit that doesn't add categories skips the gate entirely. **2c clearing guard** (Sapir-approved): explicitly blanking a previously-set `producer_license_number` while license-required categories remain re-runs the gate against the final set → 422 — closes the one hole grandfathering would open, without re-bricking pending producers (their license is already NULL, nothing is cleared). Same posture as the `auth.py:458` register bypass: NOT a security relaxation — licensed-only stays enforced by the admin approval guard + `status=="approved"` publication gate. New `TestOwnerPutGrandfathersLicense` (4 cases: pending+bio-edit→200 · pending+adds-licensed-category→422 · licensed-clears-license→422 · pending+adds-unlicensed-category→200) — first `PUT /producers/me` coverage in `test_producer_license.py`. Verified: license suite **25 passed**, `pytest tests/test_api.py` **202 passed**, ruff clean. `Refs MEH-999` (Linear issue pending — workspace quota).
- **MEH-1025 Chunk A — expose requested_changes on the owner payload (backend, DRAFT):** the producer-side "נשאר להשלים" banner (Chunk B) reads `GET /producers/me`, but `requested_changes`/`changes_requested_at` (columns since MEH-1011 `a1b2c3d4e5f6`) were on `ProducerAdminOut` only — `ProducerOwnerOut` (`schemas.py:950`) deliberately excludes admin-only fields (MEH-767). Added both to `ProducerOwnerOut` (owner-private pattern, same as kosher/license/address `:954-958`); the owner sees her OWN completion feedback, while `risk_score`/`declared_at` stay admin-only. **Pydantic-only — no migration, no model change (`alembic check` clean on a fresh upgraded DB), no endpoint signature change.** Tests: `tests/test_producers_me_schema.py` +2 (field present+correct when set, null when unset). `pytest tests/test_api.py` 202 passed. `Refs MEH-1025` (Chunk B closes). Micro-follow-up flagged: the MEH-1011 line in `docs/DATA.md` could note the owner-payload exposure (left untouched per this chunk's file scope).
- **MEH-999 follow-up — mount the built-but-unrendered ProductsSection into the edit tab (DRAFT):** `ProductsSection` — full producer-self product CRUD (name / price range / diet flags / image, against the live `/producers/me/products` GET·POST·PUT·DELETE at `producer_me.py:879-950`) — was defined at `settings/page.jsx:814` but had **0 mount sites** (`grep '<ProductsSection'` = 0), so producers had no self-service way to manage their catalog. Phase-0 confirmed the component is **self-contained** (no props — self-fetches; only external refs are `EmptyState`, `next/image`, and phosphor icons; zero references to any `settings/page.jsx`-local symbol) and already renders its own card chrome (`bg-white border border-border rounded-[16px]`), matching the MEH-1017 edit-tab editor cards. **Relocated** the component **verbatim** (no internal changes) to `frontend/components/ProductsSection.jsx` (`export default`, with a §14 file header) and **mounted** `<ProductsSection />` as the last card in `producer/dashboard/edit/page.js`, after the categories/images/location cards. Removed the now-dead definition + its 7 orphaned imports (`Plus`/`Package`/`Trash`/`Pencil`/`X`/`Carrot`/`EmptyState`, each verified used only by ProductsSection) from `settings/page.jsx` — pure dead-code deletion (532 lines), `Image` kept (6 other uses). No backend, no schema, no new copy — `settings.products.*` i18n (incl. empty-state `empty.title/description/cta`) already exists (he+en). New `__tests__/EditTabProductsSection.test.jsx` — 2 cases (renders section heading + empty-state copy; renders a fetched product with edit/delete controls). `edit/page.js` is HIGH-RISK (central) → chunk-by-chunk: Phase 0 → WAIT → mount → verify. Verified: `npm run build` exit 0 (113/113 SSG), vitest **737 passed** / 41 skipped, eslint **0 errors** (217 warn-only nits carried verbatim from the original), 0 physical RTL classes added. DRAFT — WAIT for Sapir mobile QA. `Refs MEH-999` (Linear issue pending — workspace quota).
- **MEH-964 chunk 1D — state-driven Overview: empty-state + share-gate + availability-disable + view-public (DRAFT, closes Phase 1):** wires the UX the 1A scaffold's `data-state-*` flags prepared (`page.js:200-206`). **(1) Empty-state** — the 4-KPI strip renders only when `hasActivity` (views‖whatsapp `total>0`, rating excluded per 1A); no activity → warm zero-state ("עוד אין פעילות — שתפי את העמוד כדי להתחיל", `Sparkle`) instead of a wall of zeros. **(2) Share-gate** — `VanityLinkCard` surfaces only when `isComplete && isApproved`; otherwise a why-locked hint ("עוד מעט תוכלי לשתף…", `LockSimple`) takes its place (Sapir fork ①, not silent). **(3) Availability-disable** — the 4 availability radios `disabled` until `isApproved`, with an `aria-describedby`-linked hint ("תוכלי לעדכן זמינות ברגע שהעסק יאושר ויפורסם") — announced, not colour-only. **(4) View-public** — one-tap `LocaleLink` (locale-preserving, MEH-956) to `/{slug}` `target=_blank` ("לצפייה בעמוד שלך", `Eye`). Copy verbatim from Sapir (COPY_BANK subject-test MEH-579 + ADR-024); i18n `dashboard.producer.states.*` he+en (MEH-978). Tests: new `Dashboard1DStates.test.jsx` (8); `DashboardSingleCompletenessWidget.test.jsx` phosphor mock += `Sparkle`/`LockSimple` (MEH-990-class fix). `npm run build` exit 0, vitest **748 passed**, 0 physical RTL. No touch to 1A/1B/1C internals beyond their flags. Draft PR → Sapir mobile QA (Rule 23). `Closes MEH-964`.
- **MEH-1023 Chunk B — admin category delete: native confirm() → modal dialog (DRAFT):** category delete in `/admin` content (`content/page.js:59`) was guarded only by native browser `confirm()` with no context. Replaced with the same modal-dialog pattern as `users/page.js` (fixed overlay + `text-end` card + red confirm / cancel), showing the category name: "למחוק את הקטגוריה '<שם>'?". **Phase 0 STOP recorded + Sapir ruling:** the requested producer-count-per-category is NOT available client-side — `GET /admin/categories` (`admin_extra.py:165-169`) returns a plain `Category` query and `CategoryOut` (`schemas.py:294-299`) is `{id,name,emoji}` only, no count; the page loads no producers client-side. Surfacing it needs a backend change (computed count on the `producer_categories` M2M, `models.py:198-199/397/402-410`, FK `ondelete=CASCADE` → a delete silently unlinks N producers). Sapir chose **name-only, frontend-only** (Chunk B was scoped frontend-only); the blast-radius count is deferred to a **backend-only follow-up ticket**, not opened here. `CategoriesEditor` gains `confirmDelete {id,name}` + `deleting` state; `remove(cat)` opens the dialog, `confirmRemove` fires the DELETE (only on explicit confirm); `CategoryRow` now passes the full `cat` to `onDelete`. The `DELETE /admin/categories/:id` call is unchanged. i18n: `content.categories.confirm_delete` repurposed with a `{name}` param + new `content.categories.deleting` (he+en twins, ADR-024 gender-neutral plural, MEH-978). **Auto-review (PR #1490) follow-ups applied:** dialog gained `role="dialog"`/`aria-modal`/`aria-labelledby` + an Escape handler (closes unless mid-delete) and Cancel is disabled while deleting — aligning it with AdminRowMenu's (Chunk A) a11y bar; new `__tests__/AdminContentCategoryDelete.test.jsx` (4 cases — open-with-name/no-DELETE · cancel · Escape · confirm→DELETE+close+reload) mirrors the Chunk-A test precedent. Skipped: focus-trap (pre-existing pattern-wide gap shared with `users/page.js` — follow-up candidate) and the "drop MEH-XXX comment anchor" note (conflicts with code-execution §15). **Round-3 review (Sapir-approved both):** (a) error handling — `confirmRemove` now catches a failed DELETE, shows `showToast.error(content.categories.delete_error)` (he+en twin) and keeps the dialog open (closes only on success); +1 test case (5 total). (b) RTL — the dialog card `text-end`→`text-start` (Hebrew copy aligns to the start edge in RTL); applied to BOTH this dialog AND the `users/page.js` confirm dialog (Chunk A file, per Sapir "fix both"). Verified: `npm run build` exit 0, vitest **735 passed**, 0 eslint errors, 0 physical RTL classes, `grep -c "confirm(" content/page.js` = 0. `Closes MEH-1023`.
- **MEH-1029 — Map bottom-sheet fidelity: MAP-11 chrome shipped, MAP-16 skeleton STOPPED (DRAFT):** S5 chrome parity on `MapBottomSheet.jsx` (non-central) — **MAP-11 done, all in-file:** sheet top radius `rounded-t-lg` (16px under the token override) → `rounded-t-3xl` (**24px** — verified: the `theme.extend.borderRadius` override only redefines sm/md/lg/xl, so `3xl` stays the Tailwind default 24px; the `3xl:48px` in the tokens file is under `spacing`, not borderRadius); drag handle `w-8 h-1` (32×4) → `w-11 h-[5px]` (**44×5**); result count `text-text` → `text-accent` (gold `#896714` token, styling only — count string/ICU-plural untouched). Handle color `#D4C5A9` kept as literal (tokenization is a separate token-additions issue, per scope); `justify-center` centering is direction-neutral (no physical prop) — 0 physical RTL classes. No he.json keys, no token VALUE change. **MAP-16 (list loading skeleton) STOPPED — declared STOP condition hit:** `useProducersFeed` exposes no loading flag and its sole consumer is `MapClient.jsx` (central/RED). The sheet gets `count` + `children`; on first load `allProducers=[]` → `count=0`, indistinguishable from a genuine empty result, so `MapBottomSheet` cannot self-detect "loading". Any loading signal must be wired through `MapClient.jsx:488` — forbidden by the task's "NEVER touch MapClient/MapComponent". **Sapir decision (2026-07-04): MAP-16 deferred to its own ticket** alongside the central-map cluster (MapClient touch + adversarial review); no MapClient touch this session. `npm run build` exit 0; `en-parity-guard` 2✓ (no i18n change). Draft PR → mobile QA gate before merge. `Refs MEH-1029` (MAP-11 only; MAP-16 tracked separately).
- **MEH-1028 — ProducerCard mobile density variant (CARD-27, DRAFT):** the card rendered its desktop scale on mobile — same 20px name, 44px heart, 12px heart inset, always-visible one-line description — so narrow screens felt cramped. Added a mobile-first density variant to `ProducerCard.jsx` via Tailwind breakpoint prefixes (default = mobile <640px, `sm:` = desktop-up), desktop byte-identical: name `text-[20px]`→`text-[16px] sm:text-[20px]`; card heart box `w-11 h-11`→`w-[34px] h-[34px] sm:w-11 sm:h-11` and its inset `top-3 start-3`→`top-2 start-2 sm:top-3 sm:start-3` (8px mobile / 12px desktop); one-line description gains `hidden sm:block` (hidden on mobile, unchanged on desktop). No token VALUE change, no prop/config toggle (pure responsive classes), no he.json keys (description hides, copy unchanged); shell/media-aspect/badge-row/hover/focus/pressed untouched. **Interpretation flag for review:** the "insets" acceptance row is applied to the heart inset — the only 12px (`-3`) element that isn't the badge row (which must stay unchanged) and the only reading consistent with "desktop keeps 12px" + "no desktop layout shift". **A11y trade-off flag:** the 34px mobile heart is below the 44px WCAG touch-target floor the prior comment guarded — implemented per the v4 design lock and annotated inline. No test asserts sizes so `ProducerCard.test.jsx` is untouched (42✓); `hidden` doesn't drop the node in jsdom, so `card-description` assertions still pass. `npm run build` exit 0; 0 physical RTL classes in diff. Draft PR → mobile QA gate (Rule 23) before merge. `Closes MEH-1028`.
- **MEH-999 — vacation date reveal: break the on_vacation chicken-and-egg (DRAFT):** the producer dashboard availability card (`producer/dashboard/page.js`) gated the return-date input on `producer.availability_state === "on_vacation"` (`:353`), but the backend rejects `on_vacation` without a `vacation_until` (`availability_validation.resolve_vacation_until:85-86`, 422) — so selecting vacation fired an immediate dateless POST, 422'd, and the date field was never reachable. **Frontend-only fix (backend contract untouched):** selecting "בהפסקה" now sets a local `vacationSelected` flag that reveals the date field + a "שמרו" confirm button BEFORE any POST (combined mini-form); the `on_vacation` POST is deferred to that confirm and always carries the date. New **client guard** in `setAvailabilityState` — `state === "on_vacation" && !vacationUntil` → sets an inline error (`availability.vacation_date_required`, `role="alert"` + `aria-describedby`) and returns before the POST, so no dateless 422 round-trip. Radio `active` logic updated so the group never lights up two pills while vacation is pending (`vacationSelected` suppresses the saved-state pill), and `vacationSelected` is reset to `false` on a successful POST so the confirmed form is driven purely by saved server state (a failed POST intentionally keeps it open so the producer can correct the date). Other states (accepting_orders / available_today / full_this_week) POST immediately with no date — unchanged. The old blur-to-save + the X-button's dateless re-POST (which would now trip the guard) were replaced by the explicit confirm; X now only clears the local date. New i18n `availability.vacation_confirm` + `availability.vacation_date_required` (he + en twins, MEH-978) — **copy-gated, surfaced in the PR body for Sapir approval (ADR-024 functional-UI plural voice)**. New `__tests__/DashboardVacationDateReveal.test.jsx` — 4 cases (reveal-before-POST · confirm-without-date blocked client-side + inline error · confirm-with-date POSTs `{state, vacation_until}` · already-on_vacation-on-load renders the field + confirms the edited date). Verified: `npm run build` exit 0 (113/113 SSG), vitest **735 passed** / 41 skipped, JSON valid, 0 physical RTL classes added. `Refs MEH-999` (Linear issue pending — workspace quota).
- **MEH-1001 chunk 1 — events.py cross-owner 403 → 404 (existence-leak, DRAFT):** `events.py` diverged from the recipes existence-leak convention (`producer_recipes.py:203-206`) — a foreign producer hitting `PUT`/`DELETE /events/{id}` on someone else's event got `403 "Not the owner"` / `"אין הרשאה"`, confirming the event id exists. This was pinned as CURRENT behavior by the MEH-997 probe (`test_meh997_e2e_journeys.py:681`, "if a fix lands, flip these to 404"). Now both return **404 "Event not found"** for a stranger. **`update_event` (`events.py:189`)** stays **owner-only** — NOT adding admin-override (that's a separate security.md:19-21 audit, no `admin_events` router exists). **`delete_event` (`events.py:219`)** keeps its **admin-override intact** — a non-owner-non-admin gets 404, but an admin still deletes (→ 200); the preserved override is asserted by a new test so a future change can't silently strip it. Corrected convention per Sapir (the earlier "strip admin-override" reading was withdrawn — it violated the security.md IDOR-with-admin invariant and would have deleted admins' only path to moderate events). New `tests/test_events_cross_owner.py` — 5 tests (cross-owner PUT → 404 · cross-owner DELETE → 404 · owner PUT → 200 · owner DELETE → 200 · admin DELETE → 200). MEH-997 probe flipped to assert 404 + renamed `..._returns_404`. `pytest tests/test_api.py` **202 passed** (no regression); new + journeys suites green. Scope: `events.py` ONLY — RED tier, chunk-by-chunk, Sapir merges. Chunk 2 (experiences #3/#4 + reviews #7, same corrected convention) separate; home_products excluded (MEH-796 decommission). `Refs MEH-1001`.
- **MEH-1023 Chunk A — admin users: role actions → overflow menu (DRAFT):** privilege-escalation ("העלי לאדמין" / "הסירי הרשאות") was rendered inline on every one of ~500 `/admin/users` rows, adjacent to "חסום" — the most sensitive panel action sitting as a primary button while the admin count meant to grow through the table is 0. Moved both into a per-row overflow (kebab) menu; routine "חסום/בטל חסימה" stays inline. New reusable `frontend/components/admin/AdminRowMenu.jsx` — Phosphor `DotsThreeVertical` trigger (`aria-haspopup="menu"` + `aria-expanded`), `role="menu"` dropdown of `role="menuitem"` buttons, closes on outside `mousedown` + `Escape` (Escape refocuses the trigger), anchored `end-0` so it opens toward the start in RTL. Dismissal idiom mirrors `ui/Popover.jsx:56-73` (`REUSES` anchor). Promote item shown when `u.role !== "admin"`; demote hidden for super-admin + self — the SAME `isSuperAdmin`/`isMe` guards, reused not reimplemented; each item opens the existing `setConfirm` dialog — `applyRole`, `toggleBlock`, and the confirm modal are untouched. Empty menu (protected super-admin + self) → renders nothing, no crash. Super-admin row keeps its `Lock` tooltip in the role cell. i18n `admin.users.actions.menu_aria` he+en twin (MEH-978). New `__tests__/AdminRowMenu.test.jsx` — 6 cases (empty→null · open/close toggle + aria-expanded · outside-mousedown close · Escape close + refocus · item onSelect fires + closes · both items render). Verified: `npm run build` exit 0, vitest **726 passed**, 0 eslint errors, 0 physical RTL classes. Chunk B (category-delete confirm dialog) waits for Sapir's staging QA of Chunk A. `Refs MEH-1023`.
- **MEH-1026 — central-components.json: correct 4 stale pre-`[locale]` guard paths (DRAFT):** the MEH-128 pre-edit-guard registry (`.claude/central-components.json`) still listed 4 central components at their pre-i18n-migration locations, so the guard (`pre-edit-guard.js:20`, `filePath.endsWith(c) || filePath.includes(c)`) matched none of them — the `[locale]/` path insertion defeats both `endsWith` and `includes`. Surfaced in PR #1475 (MEH-1019), where all the /map work on `MapClient.jsx` ran with no pre-edit guard firing (human relay was the only guard). Same class as **MEH-668** (sibling `rtl-allowlist.txt` `[locale]` drift, already fixed — that registry was kept in sync, this one wasn't). Phase-0 verified each stale entry resolves to exactly one real `[locale]` file (no root/locale ambiguity): `frontend/app/map/MapClient.jsx` → `frontend/app/[locale]/map/MapClient.jsx`; `frontend/app/producers/page.jsx` → `frontend/app/[locale]/producers/page.jsx`; `frontend/app/page.js` → `frontend/app/[locale]/page.js`; `frontend/app/layout.js` → `frontend/app/[locale]/layout.js`. The other 9 entries already resolved — untouched. Post-fix: JSON valid, all 13 entries resolve on disk, and a guard-match simulation confirms all 4 corrected files now FIRE. Registry-only (writable — deny list covers only `settings.json` + `.claude/hooks/**`); no guard-logic, no component, no app code touched. `npm run build` green (sanity). `Closes MEH-1026`.
- **MEH-991 — Design Parity Epic: Chunk 1 gap-matrix + Chunk 2 GREEN sweep (DRAFT, 5+1 PRs):** closed the code→design drift direction. **Chunk 1 (#1464):** imported the 2026-07-03 Claude Design agent-export into `design-reference/` (s2-logo · ds-components · dashboard · join; 16 stale #1410 HTMLs archived to `_archive-2026-06/`), produced `docs/DESIGN-GAP-MATRIX.md` — 249 rows / 14 surfaces (84 GREEN / 83 YELLOW / 24 RED / 52 COLLISION / 6 parity) with file:line evidence, token-drift + raw-hex + RTL-authorship sections, and 16 live-staging screenshots. Sapir approved the tiers + sweep order (amendment: verify the accent token exists before the gold swaps — it does, `tailwind.tokens.json:13`). **Chunk 2 GREEN sweep, one PR per group:** **G1 #1468** (`parity-tokens-residue`) 11 stale-hex sites → tokens (`#8B6914`→accent `#896714`, `#e8e0d0`→border `#e5dfd3`) across StarSelector/ReviewsSection/admin-users/Skeleton/WhatsAppQuestionChips/events-new + the `.ds-sync-css` mirror; central map sites deliberately excluded. **G2 #1472** (`parity-producer-card`, central + adversarial-review, 1 REAL finding fixed) CARD-03/05/09/21/22/23/26 — +N overflow chip (+2 tests), pressed state, focus-ring (ring-inset idiom), placeholder per Populated frame, cream heart, skeleton radius-0 + opacity-pulse. **G3 #1476** (`parity-home`) HOME-02/05/06/08/15/16/18/23/25/31 — FRL-900 hero, S14 seam-search, no category-glyph hover-scale + gold underline, corrected produce glyph, featured 4:5 + caption chip, gold eyebrow rule, Cormorant `ital` axis. **G4 #1477** (`parity-nav-footer`, both central, reviewed inline after the Fable-limit killed the review agent) NAV-14 account name-label (+2 tests) + NAV-18 footer graded-cream mark (retired the `brightness-0 invert` flatten). **G5 first pass #1479** (`parity-static-pages`) BIZ-15 OpeningHours red `#A32D2D`→fg-muted + LOGIN-04 email placeholder; remaining G5 items (ABOUT/PROC/EVENT/BADGE/COMPL + producer-detail biz family) queued as a flagged follow-up (staging drift needs per-item re-verification; several are copy-gated). All PRs: `npm run build` + vitest green; copy-gated strings surfaced in PR bodies, not shipped. `Refs MEH-991` (Chunk 1 & sweep); VRT baselines (Chunk 3) pending sweep completion.
- **MEH-1005 — upload image-cap copy (SUPERSEDED by MEH-1008, PR #1473 recommended-close):** independently neutralized the same `upload.py:92` premium promise; MEH-1008 landed the fix on staging first with preferable "לבית עסק" wording, so #1473 is a duplicate. Flagged on the PR for closure; the pin/lock drift it surfaced (`python-multipart` pyproject 0.0.32 vs uv.lock 0.0.26) matches the MEH-1008 finding. `Refs MEH-1005`.
- **MEH-1006 — retire stale docs conventions (DRAFT #1474):** Phase-0 grep confirmed the docs-only-direct-commit convention was already corrected by MEH-1012; the one live false claim (`HANDOFF.md:127` "staging merges don't auto-close") corrected in place with the MEH-1004/#1451 auto-close evidence; pending #1452 status line landed. `Closes MEH-1006`.
- **MEH-1011 Chunk 2 — producer request-changes admin UI (DRAFT):** the frontend for the Chunk-1 endpoint. `/admin/producers` pending rows gain a **"בקשת השלמה"** button → `RequestChangesModal` (new co-located component mirroring the recipes feedback modal): title "בקשת השלמה מבית העסק", `<textarea>`, 2 quick-fill chips ("חסרה תמונה — יש להעלות לפחות תמונה אחת" / "חסר מספר רישיון יצרן"), submit "שלחו בקשה" → `POST /admin/producers/{id}/request-changes`; empty feedback → error toast, 409 → "רק לבית עסק בהמתנה לאישור". **Approve-422 auto-open:** `quickApprove` now passes the full producer and, on the MEH-799 (photo) / MEH-971 (license) 422 gate, auto-opens the modal with the gate-matched chip prefilled (`approveGateReason(detail)` — "רישיון" ∈ detail → license, else photo) + a verbatim info toast ("לא ניתן לאשר עדיין — פתחנו עבורכם בקשת השלמה עם הפרט החסר. אפשר לערוך ולשלוח."). **Badge:** rows with `requested_changes` ≠ null show an amber "ממתין להשלמה" + `changes_requested_at` date wrapped `dir="ltr"` (RTL date-flip guard). `approve_producer`/`reject_producer` clear the trail (Chunk 1) so the badge auto-clears. i18n `admin.producers.request_changes.*` + `.table.actions.request_changes` + `.table.awaiting_completion` (he + en twins, MEH-978). Tests: `AdminProducersTableActions.test.jsx` extended — request-changes button (pending-only) · approve passes producer · `approveGateReason` photo/license/default mapping · `AwaitingCompletionBadge` render/null (12 in file). Verified: `npm run build` exit 0, `vitest` 720 passed. Draft PR → mobile QA gate (Rule 23) before merge. `Closes MEH-1011`.
- **MEH-1019 — /map mobile shell: top-banner height reservation (mirror of MEH-1009) + chips/blank-card investigations (DRAFT):** the mobile `/map` shell (`MapClient.jsx:406`, `lg:hidden`) hardcoded `height: calc(100dvh - 64px)` — the identical `64` that overflowed the desktop shell before MEH-1009 — so it reserved only the MEH-945 bottom cookie-banner footprint, never a TOP in-flow banner (email-verification `VerifyBanner`, first block of `<main>`). **Diagnosis reproduced** (Playwright 375px, injected 41px banner): shell bottom spilled from 822/812 (the pre-existing ~10px MEH-933 drift) to **863/812 = 51px below the fold**, page `docScrollable` 90→131. **Fix = literal mobile mirror of the desktop MEH-1009 reservation:** new `mobileShellRef` + `mobileTopOffset` state + a SEPARATE measurement effect (window resize + `<main>` childList MutationObserver for late banner mount) that measures the shell's real document-top offset (`getBoundingClientRect().top + scrollY`, ceil'd) and subtracts THAT; the MEH-1009 desktop effect is byte-identical (kept separate — the two shells never coexist, display flips at `lg`, so a hidden ref measures 0 and keeps its 64 fallback). Post-fix: shell bottom 812/812 with AND without banner (overflow 0), `docScrollable` with-banner 131→80 = identical to baseline (the residual 80px is a pre-existing below-map element, not banner-related); the ~10px MEH-933 drift is absorbed too. Desktop 1440px re-verified byte-identical (900/900 both cases, legend visible). Build exit 0, vitest 702✓. **Investigation A (chips "sliver") — WORKS-AS-DESIGNED:** at narrowed sidebar width both `ChipScrollRow` rows overflow and clip a partial chip under the 32px inline-end fade ("חלב וגבינות"/"גראס פד") — the intended "scroll for more" affordance; fades render correctly (w-3 start / w-8 end, correct gradient directions, RTL spacer present). Minor P3 polish only: `FilterChipsBar` uses the default `fadeBg="#ffffff"` while page bg is `#FFFEFB` and inactive chips are white, so the white→transparent gradient has near-zero contrast — the partial chip reads as an abrupt clip rather than a soft fade (fix would pass `fadeBg` = real container bg; not done here). **Investigation B (blank sidebar cards) — NOT a rendering bug:** `MapProducerCard` (`MapProducerCard.jsx:94-104`) renders `next/image` with the correct `f_auto,q_auto`-injected cloudinary src when a producer HAS an image (DOM-confirmed on a stub producer); photo-less producers show the intended `Leaf` fallback in the 88px `bg-green-50` box (`thumbHasLeafFallback: true`), cards 108–151px with no large blank area. The screenshot "blank areas" are photo-less producers (data/P3 density), not a code defect. Caveat: real staging pixels aren't fetchable from the CC sandbox (egress block, MEH-360) — a producer known to HAVE a photo rendering blank on-device would be a separate real bug, flagged for on-device confirmation. `Closes MEH-1019`.
- **MEH-1011 Chunk 1 — producer request-changes (completion request) backend (DRAFT):** the non-terminal twin of reject — an admin can send a pending producer a "please complete" request (missing photo / license) instead of dead-ending on a blocked approval. New migration `a1b2c3d4e5f6` adds `producers.requested_changes` (TEXT, nullable) + `producers.changes_requested_at` (TIMESTAMPTZ, nullable, tz-aware) — Expand-only, no backfill; ORM columns added to `models.py` (alembic check clean). New `POST /admin/producers/{id}/request-changes` (`RequestChangesIn.feedback`, empty/whitespace → 400 mirroring `admin_recipes.py:123`; pending-only — 409 if status ∉ pending/pending_whatsapp per MEH-769 precedent, since the op leaves status unchanged and would otherwise leave an incoherent approved+trail state) records the feedback, KEEPS status pending, emails the producer (Hebrew copy, env-aware `{settings.frontend_url}/producer/dashboard` link — mirrors `auth_emails.py:159`, so staging emails point at staging not prod) via `_send_notification_email`, and WhatsApps admin ("📝 נשלחה בקשת השלמה ל-{name}"); `approve_producer` **and** `reject_producer` now clear both columns on success (a rejected/approved producer must not carry a stale "ממתין להשלמה" trail). `ProducerAdminOut` += `requested_changes` + `changes_requested_at` (admin-only). New `tests/test_producer_request_changes.py` — 12 tests (happy path · trim · empty/whitespace 400 · 404 · 401/403 auth guards · 409 non-pending · pending_whatsapp allowed · email-to-producer mock-capture · approve-clears · reject-clears). Verified: `alembic upgrade head` + `alembic check` clean; new suite 12✓; `pytest tests/test_api.py` 202 passed (no regression). Docs: DATA.md + `.ai/diagrams/db-schema.md` + `.ai/diagrams/api-routes.md`. Chunk 2 (admin frontend) follows after merge. `Refs MEH-1011`.
- **MEH-1008 — upload image-cap error → neutral, no premium promise (DRAFT):** `upload.py:92`'s free-plan 403 promised a paid tier ("שדרגי לפרמיום להעלאה ללא הגבלה") — the last premium/upgrade promise in `backend/` after MEH-1003 — and broke ADR-024 voice (feminine-singular "שדרגי"). Sapir-approved replacement (incl. dropping "חשבון החינם" — tier framing is itself a promise while MEH-617 is undecided): "אפשר להעלות עד 3 תמונות לבית עסק. כדי להוסיף תמונה חדשה, הסירו קודם תמונה קיימת." Cap number + `plan=="free"` logic untouched; no i18n twin (raw HTTPException detail via `detailToMessage`). Test `test_freemium_limit_returns_hebrew` updated: its "חינם" marker was incidental to the old copy — now asserts the stable cap phrase + explicitly asserts no פרמיום/שדרג (regression guard). pytest test_api.py **202 passed**; post-fix grep: zero premium promises in backend. **Rule-5a CVE check surfaced a real out-of-scope finding:** `uv.lock` resolves `python-multipart==0.0.26` while pyproject pins `0.0.32` (lock drift — exactly MEH-787's gate class, PR #1391 still draft); 0.0.26 is vulnerable to CVE-2026-53539 (quadratic querystring DoS, fixed 0.0.30) and likely CVE-2026-42561 (unbounded part-header DoS) — needs its own lock-bump ticket. `Closes MEH-1008`.
- **MEH-1014 — BottomNav: minimize-on-scroll replaces hide-on-scroll (mobile pill no longer flickers, DRAFT):** the MEH-789 hide-on-scroll slid the whole pill off-screen (`translate-y-[120%]`) on a single-pixel downward delta, and iOS rubber-band overscroll manufactured false direction changes → the pill flickered on/off. Replaced with **minimize**: the pill stays on screen and only compacts (`BottomNav.jsx` — `hidden` state → `compact`; the `translate-y` toggle deleted, no vertical slide remains). New rAF listener (same structure, `[]` deps) **clamps** `y = max(0, min(scrollY, scrollHeight − innerHeight))` to kill overscroll phantoms, then runs **asymmetric hysteresis** via two accumulator refs: `downAcc ≥ 24px → compact`, `upAcc ≥ 8px → expand`, a direction change zeroes the opposite accumulator; `y < 60 → always expanded + reset both`. Compact render: nav `h-14→h-12` (56→48px) + `p-1.5→p-1`; labels wrapped in a collapse element animating `opacity 0 + max-h 0` (`overflow-hidden`) while icons stay `size 22` (weight logic unchanged); `tabCls` min-height `44→40px`. Only height/opacity/max-height transition (`duration-base ease-quart motion-reduce:transition-none`) — the `nav-pill-glass` **backdrop-filter is never animated**. Sheet-open renders the pill expanded (`isCompact = compact && !sheetOpen`, replaces the old `hidden && !sheetOpen` guard) so the account control never collapses under its own sheet; focus within the nav (`onFocusCapture` on the shell) forces expansion, mirroring the MEH-734 focus guard removed from Header in MEH-884. MEH-852 indicator re-measures correctly on toggle — its `ResizeObserver` observes the `<nav>` box (`BottomNav.jsx:166`) which changes size on `h-14↔h-12`, re-running `measure()`. Thresholds are module-top constants (`HIDE_DELTA`/`REVEAL_DELTA`/`TOP_THRESHOLD`) for post-QA tuning. Header untouched (its hide detached in MEH-884); the intentional Header inline-copy note stands. Build exit 0, vitest 702✓, `grep translate-y-[120%]` = 0. `Closes MEH-1014`.
- **MEH-1012 — correct stale "docs-only → direct commit to staging" rule (DRAFT):** Regression rule 7 (`.claude/rules/workflow.md:275`) and the PR-approval guide (`:442`) both claimed docs-only files could be committed straight to `staging` with no PR. Factually wrong since the `protect-staging` ruleset landed — it blocks *all* direct pushes (`push declined due to repository rule violations`, hit on 2026-07-03 pushing a HANDOFF entry, which then went via PR #1461). Both spots rewritten: docs-only still branches + opens a PR; the MEH-736 docs-only twin checks satisfy the required gates so no admin override is needed; merge stays Sapir-only. Rule numbering preserved (still rule 7) so cross-refs resolve. CLAUDE.md carries no copy of the claim (grep-verified); the CHANGELOG:2630 "docs-only" hit is a historical DoD-exception log line, not a rule — untouched. Text-only; `npm run build` green (sanity, no code touched). **Out of scope (MEH-1013):** the docs-only twin jobs `.claude/rules/testing.md` promises are not actually present in `pr-checks.yml`/`deploy.yml` — a mechanical `.github/workflows/**` gap (CC-deny), not this text fix. `Closes MEH-1012`.
- **MEH-1009 — /map desktop shell: banner-height reservation (bottom controls no longer clipped below the fold, DRAFT):** with a top in-flow banner present (email-verification `VerifyBanner`, first block of `<main>` per `layout.js:221`) the desktop `/map` shell — hardcoded `height: calc(100vh - 64px)` — overflowed the fold by exactly the banner height, clipping bottom-anchored map controls (legend toggle `bottom-4`; the ex-DesktopMiniPopup CTA in Sapir's 03/07 screenshot). **MEH-979 exonerated** (its diff only touched the 768–1023px band; full diagnosis in the MEH-1009 Linear thread). Fix = TOP-edge twin of the MEH-945 mobile cookie-banner reservation: measure the shell's real document-top offset (`getBoundingClientRect().top + scrollY`, ceil'd) on mount + resize and subtract THAT instead of 64. Repro quantified on dev (Playwright, 41px injected banner): pre-fix shell bottom 951/900 and legend clipped 35px below the fold → post-fix bottom exactly 900, legend fully visible and opening; no-banner layout also lands exactly at the fold — absorbing the pre-existing ~10px header drift (real header ≈74px vs the hardcoded 64, the MEH-933 spill note). Mobile shell untouched (its own top-banner overflow = known sibling, follow-up candidate); cookie banner overlay = transient-by-design, out of scope. Docs: `MANUAL_TESTING.md` legend line corrected — it was misdocumented as "bottom-right" since #136; it has always been physical bottom-LEFT. Build exit 0, vitest 702✓. `Closes MEH-1009`.
- **MEH-1010 — /map desktop marker click = sidebar card-sync (retire DesktopMiniPopup) + dual-mount ref fixes (DRAFT):** desktop marker click opened `DesktopMiniPopup` pinned to the map's bottom corner — clipped below the fold with a top banner present (MEH-1009 mechanism), CTA unreachable, duplicating the sidebar (documented Airbnb bottom-popup anti-pattern). Now marker click scrolls the matching sidebar card into view + highlights it (existing active border + new desktop-only `lg:ring-2 lg:ring-primary`); popup deleted (import+render+file; sole consumer verified by grep). **Root-cause fix — dual-mount refs:** `cardList`/`mapPane` render twice (desktop + mobile shells), so (a) `cardRefs` (id→node Map) let the hidden mobile node overwrite the desktop one → `scrollIntoView` silently no-oped on desktop — now id→**Set** of nodes, visible one (`offsetParent !== null`) resolved at click time (registerMapApi's visible-instance discipline); (b) `legendRef` pointed at the hidden instance → click-outside closed the desktop legend panel on any inside mousedown, swallowing category clicks — now a callback-ref Set with contains-check across connected instances (canvas click still closes). **Enter-on-marker actually implemented:** Leaflet 1.9 maps Enter→action only via `bindPopup`'s keypress handler and MEH-30 #8 binds no popups, so MEH-765's `keyboard: true` assumption never fired — a document-level Enter delegate re-dispatches a bubbling click on the focused `.leaflet-marker-icon` (same Leaflet routing as mouse; works for clusters too; MapComponent untouched — MEH-948 lock respected). Mobile (<lg) byte-identical: sheet flow, `selectedProducer` ownership, legend hidden. Verified: build exit 0, vitest 702✓, 18/18 Playwright smoke vs stubbed `/producers` (marker/cluster-child/Enter → correct card; highlight survives zoom; legend deselect/reselect with panel staying open; 375px sheet unchanged). `Closes MEH-1010`.
- **MEH-994b — registration-duration copy aligned to "10 דקות" everywhere (DRAFT):** Sapir decision closing the 5-vs-10 mismatch flagged in MEH-994. Four value-only locale edits: `auth.register.producer.subtitle` ("5 דקות…"→"10 דקות…") + `auth.register.producer.preflight.duration` ("בערך 5"→"בערך 10"), he + en twins; plus the now-stale `RegisterPreflight.jsx` duration comment updated. `/about/for-businesses` FAQ (`about_business.faq.time.a` = "בערך 10 דקות") untouched — it was the approved precedent. Excluded (different meanings, verified): completeness nudge "זה לוקח 5 דקות" (add-products task), guides 5/10-minute exercises, "15 דקות בטלפון". Post-sweep grep: zero registration 5-minute strings. Build exit 0; register vitest 13✓ (suite asserts keys, not values — no churn). `Refs MEH-994`.
- **MEH-964 1C — anonymous activity pulse on the Overview (§5 FINAL SPEC, DRAFT):** new `ActivityPulse` section in `producer/dashboard/page.js` below the 1B KPI strip — aggregate-only pulse from `/producers/me/analytics` (`producer_me.py:488`), no names / message text / per-row city / per-row CTA (the conversation lives off-platform in WhatsApp). **Sapir rulings recorded (03/07, deltas-precedent format):** (1) **reviews row DROPPED** — payload has only lifetime `total_reviews`, no windowed count → "ביקורת חדשה" would be a false recency claim (§5 honesty clause) and a lifetime row duplicates the rating KPI (FLAG-1); returns with MEH-966. Pulse = 2 event types, each gated on its own `last_7d > 0`. (2) **fixed locked order** (וואטסאפ → צפייה), NO per-row relative times ("אתמול" never rendered — no per-event timestamps in payload), uniform 7-day frame. (3) **hero binds to `whatsapp_clicks.last_7d`, NOT `.total`** — "חדשות" must be truthful; `last_7d == 0` → no hero; both metrics 0 → zero-state "עוד אין פעילות — שתפי את העמוד כדי להתחיל". ONE section CTA "פתחי וואטסאפ לענות" → `wa.me` (gated with the hero — "לענות" with zero inquiries is the same false claim). Card sizes to `rows.length` (0/1/2 collapse). i18n `dashboard.producer.pulse.*` he+en (hero uses ICU one/two/other — grammatical derivations of the locked string, flagged for verbatim approval); `data-testid` hooks (`activity-pulse`, `-hero`, `-row-whatsapp`, `-row-view`, `-cta`, `-empty`). Tests: dedicated `DashboardActivityPulse.test.jsx` (4 cases — hero+CTA gating on whatsapp `last_7d`, views-only row, zero-state with non-zero lifetime totals, no-reviews-row); test-mock fix: `DashboardSingleCompletenessWidget` phosphor mock += `WhatsappLogo`/`Eye` (MEH-990 class). Build exit 0; vitest **702 passed**; 0 eslint errors; 0 physical RTL classes. Pre-work: stale PR #1377 (superseded 1B) closed per Phase-0 gate. `Refs MEH-964`.
- **MEH-1003 — chat FAQ "האם האתר בחינם" premium-promise → neutral (DRAFT):** `chat.py:91` (SYSTEM_PROMPT knowledge base) promised a future premium tier ("בתי עסק יכולים לבחור לשדרג לתוכנית פרמיום בהמשך") while the pricing model is undecided (MEH-617). Replaced with Sapir-approved neutral copy: "אין עמלות על עסקאות ואין דמי מנוי — הלקוחות פונים ישירות לבית העסק, והתשלום עובר ביניהם בלבד" — keeps the no-transaction-fees LOCK (BRAND.md), gender-neutral plural (ADR-024), zero future-tier promise. String-only; no i18n twin exists (the "האם האתר בחינם?" suggested prompt has no `HARDCODED_ANSWERS` entry in `ChatWidget.jsx` — it flows through `POST /chat`, so this fix covers the widget path). Post-fix grep: zero פרמיום/שדרוג left in chat.py. pytest tests/test_api.py **202 passed** locally. Out-of-scope siblings surfaced for separate tickets: `upload.py:92` ("שדרגי לפרמיום") + the entire `upgrade.*` locale surface. `Closes MEH-1003`.
- **MEH-994 — /register/producer pre-flight intro screen ("לפני שמתחילים", DRAFT):** the producer wizard opened cold on the ACCOUNT form — no prep expectations, no duration, no what-happens-after. New `RegisterPreflight.jsx` (co-located) renders BEFORE frame 01 on both auth states via a `showPreflight` early-return in `RegisterProducerClient.jsx` — the wizard tree below stays byte-identical (functional-freeze MEH-132: STEP enum, OAuth, declarations, submit body untouched; stepper still 01–04, pre-flight is NOT a numbered step). Blocks: "מה כדאי להכין" checklist (email/account line — hidden on the upgrade path, סיפור קצר, 2–3 תמונות, רישיון יצרן aligned with the MEH-971 pending-opt-in "אפשר להשלים אחר כך"); duration "בערך 5 דקות" — **matched to the live page subtitle "5 דקות", NOT the spec's "10 דקות"** (copy-guard: the page already carries 5; the cross-page mismatch vs about_business.faq.time "בערך 10 דקות" is flagged for a separate copy decision); "מה קורה אחרי" + link to `/about/process`; single "מתחילים" CTA. `auth.register.producer.preflight.*` he + brief-en twins (MEH-978/840). Tests: suite entry now real-click-through of the CTA (`renderWizard()` helper); new describe — pre-flight renders first + upgrade variant hides the account line (register suite 13✓); Playwright spec 18's three `goto` entries gained the same click-through (entry-setup class). Verify: build exit 0 (113/113), vitest 698✓/41 skipped, 375px screenshot. `Closes MEH-994`.
- **MEH-1004 — design-code truth: template 01 v2.1 (`code_truth` block + data-bindings) + `.ai/design-inventory.md` bootstrap (DRAFT):** closes the prevention direction of design-code drift (incident 2026-07-03: Claude Design designed a notifications bell with no backend, an "images editor" off a stale gap list, and a checklist field absent from the heuristic — MEH-964 / MEH-1002). **Template 01 → v2.1:** new mandatory `<code_truth>` block after `<brand_lock>` (orchestrator-generated per session, live-verified file:line, with an explicit NOT-available list) + filled producer-dashboard example; new deliverable #7 — every dynamic mockup element annotated with its data binding ("צפיות ← analytics.profile_views.last_7d"), unbound elements labeled NEEDS-BACKEND; stale gold `#8B6914` → `#896714` (AA, post-fix grep = 0). **NEW `.ai/design-inventory.md`:** per-surface capability sheet with the governance rule in the header (updated in the same PR that changes a surface — a standalone doc that rots is the anti-pattern); first surface = producer dashboard, every claim verified file:line against staging (components incl. the 3 edit-tab editor chunks #1436/#1440/#1446; full `/producers/me` + `/dashboard` + `/analytics` payloads from `producer_me.py:61/406/488`; NOT-available: producer notifications system, per-view viewer identity, per-KPI numeric deltas). **Phase-0 correction vs ticket:** "self-serve gallery editor" is no longer NOT-available — it shipped in #1440 and is inventoried as available (per the ticket's own no-claims-on-faith rule). Docs-only. `Closes MEH-1004`.
- **MEH-1002 — completeness heuristic: "תיאור קצר" added as 6th field (design-code parity for the MEH-964 checklist, DRAFT):** `producerCompleteness()` (`frontend/lib/producer-completeness.js`) checked only city / coords⊕delivery / contact / category / image — no description field at all, while the approved MEH-964 dashboard checklist shows "תיאור קצר". **Phase-0 field mapping:** BOTH description fields render publicly — the tagline `short_description` (`ProducerCard.jsx:208`, `ProducerHeader.jsx:81`) and the long story `description` (`ProducerSections.jsx:77`) — so per the issue's OR clause the new check flags "תיאור קצר" only when **both** are empty/whitespace (a producer whose AI-bio panel filled `description` clears the item). Payload verified: `GET /producers/me` → `ProducerOwnerOut` inherits both from `ProducerListOut` (`schemas.py:695-696`); the admin list payload carries them too. **Yellow-tier only** — redCondition (`producer-completeness.js` red block) untouched. `ProfileCompletenessCard`: `TOTAL_FIELDS` 5→6 (percent steps now ~16.7% — still never lands on the 70 boundary), checklist gains the 6th row, FIELD_KEY inversion (MEH-831) picks the slug up automatically; CTA for `short_desc` routes to `/producer/dashboard/edit` (default non-city branch). i18n: `dashboard.producer.completeness.fields.short_desc` he ("תיאור קצר") + en twin ("Short description") — MEH-978 parity + MEH-840 gates green. Tests: heuristic node suite +5 cases (16✓ — OR semantics, whitespace, yellow-not-red), card vitest updated to 6-field percentages (83/67) + new missing-description case; full vitest **696 passed**, build exit 0, 0 eslint errors. **Known side-effect (accepted per issue):** the shared heuristic also drives the admin producers list — producers with no description now show a yellow completeness dot with "תיאור קצר" in the tooltip. **Gap flagged (not in scope):** the owner edit hub has no `short_description` input yet (only the AI-bio panel writing `description`) — the MEH-964 skin lands separately. `Closes MEH-1002`.
- **MEH-903 A (Expand) — unify the delivery write-path on `delivery_areas` (DRAFT):** Phase 0 (03/07, @6c46b434) proved the `producers.delivery_cities` **column** is written by admin only and read only by the admin `ProducerForm` (public frontend migrated to the `delivery_areas` relation via MEH-902/904; owner PATCH never wrote it). This chunk makes `delivery_areas` the single store; the column stays declared (drop = Chunk C). **admin.py:** dropped `delivery_cities=data.delivery_cities` from create, and popped `delivery_cities` out of the update payload before the setattr loop (renamed the local areas var `delivery_cities`→`delivery_area_cities` for clarity) — column now written by **nobody**. **schemas.py:** both `_validate_location_mode` XOR validators (Create + Update) re-pointed from `delivery_cities` → `delivery_area_cities`; exact nationwide-XOR-cities semantic preserved, only the field source changed (input `delivery_cities` fields stay declared but inert — removal = Chunk C). **ProducerForm.jsx:** ONE cities input — the `CitiesAutocomplete` (location-mode block) now feeds `delivery_area_cities` on submit; the redundant comma `delivery_area_cities` `<Field>` removed; initial populate reads `initial.delivery_areas[].city` instead of `initial.delivery_cities`; client gates already keyed on the unified `delivery_cities` array. **Grep proof:** zero `delivery_cities=` write sites in `backend/app/` (only `models.py:161` column def); zero `form.delivery_area_cities` reads left. **Tests:** `test_producer_location_types.py` — 3 cases re-pointed (XOR reject, nationwide-only, city-list round-trip now asserts `delivery_areas[].city`). `seed_data.py`/`test_api.py`/`test_analytics.py` untouched (already go through `delivery_areas`). No Alembic / `models.py` / `ProducerListOut` change (all Chunk C). ⚠️ **Flagged for Sapir (MEH-272 CHECK interaction):** the DB `delivery_nationwide_xor_cities` CHECK still references the column — flipping a **pre-existing** producer whose column is non-empty to nationwide would now violate it (column no longer cleared). No current test hits this (fixtures start column=`[]`); real-data exposure is Sapir's DB truth. pytest + `npm run build` green. `Refs MEH-903`.
- **MEH-997 — functional E2E audit (critical user journeys) + `/admin/recipes` seed fix + permanent journey suite (DRAFT):** the complement to MEH-996's static audit — every critical journey got an **executed** verdict (test run, not code-reading). **Seed CONFIRMED + FIXED:** the recipes backend (`admin_recipes.py`, MEH-589) shipped with NO admin frontend — zero `admin/recipes` grep hits across `frontend/`, no `NAV_HREFS` entry (`layout.js:41-57`), no i18n keys, and `pending_moderation_count` summed 5 queues WITHOUT recipes (`admin_extra.py:549-555`) — so a producer's recipe sat `pending/unpublished` with no surface showing it (Sapir's manual QA repro). **Fix (pre-authorized in the issue):** new `/admin/recipes` page mirroring `/admin/experiences` 1:1 (5-tab queue on `moderation_status` = pending/needs_revision/approved/rejected/all, approve/request-changes/reject wired to the live endpoints, feedback modal, expandable rows with ingredients/instructions/moderation notes, `data-testid` per E2E-LOCATORS), sidebar nav entry (`Bread` icon, he+en), `admin.recipes.*` i18n block (he + brief en twins), and `pending_recipes` (pending+needs_revision, mirroring the experiences pair) added to the dashboard sum + stats — failing-test-now-passing in `tests/test_meh997_admin_recipes_badge.py`. One deliberate parity deviation: the table card is `overflow-x-auto` (not experiences' `overflow-hidden`) so the actions column stays reachable at 375px. **Permanent suites:** `tests/test_meh997_e2e_journeys.py` — 19 executed API+DB journey tests (recipe pipeline create→queue→approve→public, registration→approve→public with **notification mock-capture** (`notify_admin_new_producer`/`notify_producer_registered`, MEH-977 class), group-buy join/count, event w/ image, favorites, review incl. WA-click gate, auth session login/me/refresh/logout, experience pipeline, map data, and the OWASP role×action **authorization matrix**); `frontend/e2e/flows/20-admin-recipes-queue.spec.ts` — browser regression tripwire for the missing-admin-surface class (env-gated like spec 19) + `playwright.local.config.ts` (sandbox-only chromium pin). **Executed locally:** pytest full suite green, build + vitest 695✓, Playwright 11 specs green incl. MEH-216 spec 19 re-run. **Report-only (executed probes, no edits):** recipe submit fires NO admin notification (queue is poll-only — probe test documents it); events cross-producer PUT/DELETE returns **403 not 404** (existence leak, diverges from `producer_recipes.py:203-206` convention — permissions logic, HIGH-RISK). `Refs MEH-997`.
- **MEH-272 — producer CHECK constraints → ORM + hand-written revision (SHOW, NOT applied; DRAFT):** two `producers` CHECK constraints have lived on prod/staging since the removed `_migrate_columns` (MEH-267 era) but were never in the ORM or the alembic baseline (`ef8fb1858f5b`), so fresh bootstrapped DBs (local/CI/new env) silently lacked them — a defense-in-depth regression. **Added to `Producer.__table_args__`** (`models.py`, `CheckConstraint` import added): `producer_location_mode` (`has_physical_location OR offers_delivery`) + `delivery_nationwide_xor_cities` (`NOT (delivery_nationwide AND array_length(delivery_cities, 1) > 0)`) — verbatim from MEH-272. **Hand-written revision** `f9a2c7d41b83` (down_revision = current head `c3f8a1d27e94`; no autogenerate) adds each constraint inside a `DO $$ ... IF NOT EXISTS (pg_constraint) ... $$` guard (scoped `AND conrelid = 'producers'::regclass` — pg_constraint uniqueness is `(conrelid, conname)`, not global, so an unscoped conname match could see a same-named constraint on another table and silently skip the ADD, reintroducing the exact regression; PR review Must-Fix) so it's a **no-op on prod/staging** (already present) and a **create on fresh DBs**; downgrade drops both `IF EXISTS`. Pydantic `model_validator` still guards the API layer; these protect direct-SQL paths (seeds, imports, psql). **Not applied** — `alembic upgrade` is Sapir's manual RED step. `py_compile` + `ruff` clean; alembic chain intact (single head → `f9a2c7d41b83`); existing `test_producer_location_types.py` direct inserts satisfy both CHECKs (verified); **pytest → CI** (no local Postgres). `.ai/diagrams/db-schema.md` documented. `Refs MEH-272` — **DO NOT MERGE before Sapir reviews the revision; apply is manual.**
- **MEH-996 — Fable 5 full-codebase correctness audit (proof-gated find & fix) — 3 fixes MERGED to staging:** one-shot whole-repo audit (11 Pass-A families + Pass-B free hunt, run inline — audit subagents hit the session usage cap). Evidence bar held: every CONFIRMED finding shipped with a failing-test-now-passing. **Merged:** (1) **#1441** i18n misnested namespaces — `DeliveryBlock`/`FollowButton`/`HolidayBanner` read `producer.delivery/follow/holiday_banner`, namespaces that exist in NEITHER locale file (keys live under `group_buys.*`, he.json:923-944) → users saw raw key paths (e.g. follow-button label `producer.follow.follow_aria` for every signed-in user on producer pages); repointed to `group_buys.*` (FridayDeliveryStrip precedent), `error_generic`→shared `error.generic` (MEH-848); proof test uses the REAL NextIntlClientProvider + real he.json (the suite's key-echo next-intl mock masks exactly this class); sibling sweep: all **150** translator namespaces now resolve — these 3 were the only misses. (2) **#1444** contradictory UI states (PRE-FOUND, Sapir screenshot) — group-buys dashboard rendered the empty state TOGETHER with the open create form (`page.js:297-318`, independent branches); sibling `recipes/page.js:90-112` identical; both now gate EmptyState on `!showForm` (settings/page.jsx `&& !adding` precedent); MEH-964 collision check: no open PR touched either file. (3) **#1445** silent fetch failures (MEH-977 class) — `FavoritesClient` fetch failure rendered "אין מועדפים עדיין" (indistinguishable from real empty → user's favorites look deleted) and `admin/analytics` failure showed loading forever; both now render existing error copy (`error.generic` / `admin.common.error_loading`) — zero new keys, zero parity impact. **Report-only (no edits):** `ProducersClient.jsx:114-132` filter race (no stale-response guard, HOT-018 class — central surface → own ticket + adversarial review); `CitiesAutocomplete:26` `Math.random` listbox id → `useId` (Sev 1); `KashrutBadgeStrip:20-24` UTC/local day-count skew (Sev 1); **Sentry-not-wired gap re-confirmed** (finding 3 is exactly its class). **Verified NOT-BUGS:** the 35 he-only en.json gaps are the frozen MEH-978 BASELINE (MEH-472); all 40 FKs carry ondelete; `err.detail` consumers all guarded (MEH-959 held); no dead routes; hot mutation forms double-submit-guarded; Leaflet cleanup present. All 3 PRs merged green (build/vitest/lint/contract gates + Playwright E2E on preview). `Refs MEH-996`.
- **MEH-273 — alembic advisory lock in env.py (DRAFT):** `run_migrations_online` had no mutual exclusion, so two overlapping runners (e.g. concurrent Railway deploys both booting the Dockerfile migrate step) could race on `alembic upgrade head` — the loser observing a half-applied head and partially re-applying a revision. Added a **transaction-scoped Postgres advisory lock** (`SELECT pg_advisory_xact_lock(:key)`, fixed key `273273273`) as the first statement inside `context.begin_transaction()`, before `context.run_migrations()`. The second invocation **blocks** there until the first commits, then re-reads the now-current head and no-ops. `pg_advisory_xact_lock` **auto-releases on commit/rollback** (no leak, no explicit unlock); a single runner acquires it uncontended → **no behavior change**. Relies on the single-transaction default (env.py does NOT set `transaction_per_migration`, so all revisions share one txn — verified). **Offline mode (`run_migrations_offline` / `--sql`) untouched** — it emits SQL with no live session to lock. `py_compile` + `ruff` clean. **Concurrency test approach (no local Postgres in sandbox → CI/manual):** open two `psql`/alembic sessions; session A acquires and holds inside a long migration; session B's `pg_advisory_xact_lock` blocks until A commits — observable via `pg_locks WHERE locktype='advisory'`. `Closes MEH-273`.
- **MEH-616 — reconcile pre-commit ESLint with warnings-as-feedback (DRAFT):** the `eslint` pre-commit hook (`.pre-commit-config.yaml:16`) ran `npx eslint --max-warnings=0`, so a single warning blocked the commit — directly contradicting the MEH-443 policy (`.claude/rules/code-execution.md`: frontend lint rules ship at `"warn"` and are **feedback, not a gate**, until promoted to `"error"` after MEH-437/438/439/440 + 30-day soak). Dropped `--max-warnings=0`; eslint still exits non-zero on genuine errors, so **errors block, warnings don't**. `.pre-commit-config.yaml` only (`eslint.config.mjs` is deny-listed and untouched). **Verified locally** with the exact hook command: a warning-only file (`no-magic-numbers`) → **exit 0** (passes) under the new command but **exit 1** under the old `--max-warnings=0` (proving the conflict was real); a `no-undef` file → **exit 1** (still blocks). Policy was unambiguous → no STOP. `Closes MEH-616`.
- **MEH-965 — dead-key sweep: `map.sheet.badge.organic` + `.kosher` (DRAFT):** Phase-0 grep across `app/`+`components/`+`lib/` (literal `map.sheet.badge` AND dynamic `sheet.badge.${...}`/`[var]`) found the **only** reader is `MobileSheetSelectedCard.jsx:81` → `t("map.sheet.badge.verified")`. `.organic` + `.kosher` have **zero** consumers (confirms the MEH-943 finding that MEH-826 dropped the dietary badges from that card's render). Removed both keys from **he.json** (`אורגני`/`כשר`) **and en.json** (`🌿 Organic`/`✡️ Kosher`) — parity preserved, `verified` retained (the live key). `npm run build` green; vitest **686 passed / 41 skipped** (en-parity-guard he→en + MEH-840 en-locale guard both green). JSON valid both locales. (Live-staging visual reconfirm deferred to Sapir — CC sandbox `*.vercel.app` egress block; static grep is authoritative.) `Closes MEH-965`.
- **i18n(location) — neutralize `modals.location.geo_failure` (MEH-985 fold-in, DRAFT):** the last functional map/location string carrying the singular-feminine possessive `שלך`. `he.json:2235` `modals.location.geo_failure` "לא הצלחנו לקבל את המיקום שלך" → "לא הצלחנו לקבל את המיקום" (Sapir-LOCKED — verbatim mirror of `map.component.geo_failure:1040`, shipped in #1405). The MEH-985/981 batch explicitly excluded `modals.location.*` (believed already-neutral, #1385) so this residual survived; now closed. **he.json value-only** — `en.json` untouched (key unchanged, both parity guards green). Post-fix grep over `map.*`/`location.*`/`modals.location.*`: **zero** functional strings contain `שלך`. `npm run build` green.
- **chore — union-merge strategy for CHANGELOG (DRAFT):** `docs/CHANGELOG.md ## Unreleased` was a chronic conflict magnet on staging churn — every batch PR this week (MEH-985/986/959) needed a manual Accept-Both resync because two branches each prepend a top entry. Added `docs/CHANGELOG.md merge=union` to `.gitattributes` (which already exists for `*.sh`), so git's built-in `union` driver keeps **both** sides of a conflicting hunk instead of erroring. **Caveat (documented in `.gitattributes` + PR):** union may interleave line-adjacent edits and never resolves an in-place edit of an existing entry — acceptable for an append-only log where entries are only added at the top. `git check-attr merge docs/CHANGELOG.md` → `union`. Infra-only, no ticket.
- **MEH-990 follow-up — Emoji-LOCK residue: raw arrow/dingbat DOM affordances → Phosphor (DRAFT):** the merged MEH-990 sweep (#1419) claimed "Post-fix DOM emoji grep = clean", but its pictographic-emoji grep missed **raw arrow/geometric dingbats** rendered in JSX. A fresh frontend-wide scan surfaced 7 genuine DOM affordance sites — all now Phosphor, matching the MEH-938 `→`→`ArrowRight rtl:rotate-180` precedent (`aria-hidden`, tokens): `events/EventsClient.jsx` CTA `→`→`ArrowRight`; `map/components/MobileSheetSelectedCard.jsx` `→`→`ArrowRight`; `components/ProfileCompletenessCard.jsx` CTA `→`→`ArrowRight`; `components/OnboardingTip.jsx` CTA `→`→`ArrowRight`; `components/GuideArticle.jsx` back `←`→`ArrowRight` (SSR import — server component, `@phosphor-icons/react/ssr`); `settings/page.jsx` grace `⏳`→`HourglassSimple`; `producer/dashboard/events/new/page.js` remove-image `✕`→`X`. Test-mock fix: `DashboardSingleCompletenessWidget.test.jsx` phosphor mock += `Check`/`ArrowRight` (real card now instantiates them). `npm run build` green (exit 0); vitest **686 passed**; 0 eslint errors; no physical RTL classes added. **Left intentionally (documented, NOT breaches):** `ImageGallery`/`Lightbox` carousel `←→` (physical-by-design, rtl.md allowlist, `aria-hidden`), `ChatWidget ●` typing dots (decorative), `admin/reviews <option> ⭐` (option can't hold icons), `admin/settings` diff `→` separator, `MapComponent:106 ✓` (central — Leaflet `divIcon` HTML-string, needs SVG + adversarial review), `StoryCardCanvas`/`ShareButton 🌿👉` (canvas/share-text, not DOM), `lib/holidays.js` emoji data (design call), he.json copy `✓/→` (copy-approval-gated, rule 22; `en.json` untouched). `Closes MEH-990`.
- **MEH-992 — group-buy dashboard form clarity (DRAFT):** the producer group-buy create form (`producer/dashboard/group-buys/page.js`) gave no upfront guidance, so a `group ≥ regular` price only failed as a raw **400** (`group_buys.py:192` "מחיר קבוצתי חייב להיות נמוך מהמחיר הרגיל") after submit. Three clarity adds (he.json copy only, functional/neutral per ADR-024): (1) **₪ adornment** on both price inputs — `start-3` span (right in RTL) + `pe-7 text-end` on the `dir=ltr` input so the shekel sits right and digits align to it; the now-redundant `(₪)` was dropped from `price_regular_label`/`price_group_label`. (2) **Pre-submit price helper** ("המחיר הקבוצתי חייב להיות נמוך מהמחיר הרגיל.") — always shown muted, turns red when a client-side `priceInvalid` mirror of the backend rule trips, and the submit button disables while invalid → the raw 400 is prevented, not merely surfaced. (3) **Deadline helper** ("המועד האחרון להצטרפות, לפי שעון ישראל.") + a **one-line concept intro** under the form heading (participants unlock a wholesale price). 3 new he-only keys added to the MEH-978 en-parity-guard `BASELINE` (en wave under MEH-472); `en.json` untouched. No backend change; bio/custom-questions cards + `dashboard/page.js` left to MEH-964. `npm run build` green; parity + he.json valid; 0 physical RTL classes. Verified via Chromium harness screenshot (valid + `group≥regular` red-helper/disabled states). `Closes MEH-992`.
- **MEH-816 — remove redundant OTP pre-deletes on producer delete (DRAFT):** the MEH-755 explicit `db.query(PhoneOtpToken)...delete()` calls in both producer-delete paths (`auth.py::delete_account`, `admin.py::admin_delete_producer`) became redundant once MEH-773 Chunk B (PR #1154) added `passive_deletes=True` to the `Producer.otp_tokens` backref (`models.py:1060`) over the FK `ondelete="CASCADE"` (`models.py:1046`). MEH-773 explicitly deferred the cleanup to a follow-up (this ticket). **Phase-0 confirmed** `passive_deletes=True` + FK cascade exist before removing anything. Removed both bulk pre-deletes + their now-stale MEH-755 comments (which claimed "the ORM relationship has no delete cascade" — false since MEH-773), dropped the now-unused `PhoneOtpToken` import from both routers, and added a `DO NOT re-add` sentinel note in each. Deletion behavior is byte-identical: on delete the DB FK cascade removes the OTP rows (the ORM no longer nullifies `producer_id`). **Regression coverage already exists** — `tests/test_account_deletion_cascade.py::test_delete_account_with_otp_tokens` (:172) + `::test_admin_delete_producer_with_otp_tokens` (:196) both insert OTP rows then assert `count == 0` after each delete path; refreshed their stale MEH-755 header comment. Test DB is Postgres (enforces `ON DELETE CASCADE`), so the tests hold via the cascade. `py_compile` + `ruff check` clean; **pytest deferred to CI** (no local Postgres in sandbox — MEH-360 class). No schema/Alembic change (models unchanged). Auth-path touch is a delete-path simplification only — no JWT/session/authz/input-validation change, so no new CVE surface. `Closes MEH-816`.
- **MEH-939 — stale center-coord comments post-MEH-932 (docs-only, DRAFT):** MEH-932 moved the `/map` default center from `[31.7683, 35.2137]` (Jerusalem) to `[32.4, 34.95]` (producer band); the adversarial review flagged 3 comments citing the old center. **#1** (`MapComponent.jsx:452`) was fixed in-scope by MEH-932. **#2** (`MapClient.jsx`, was `:85-86` "initial view must stay anchored at [31.7683, 35.2137]… country-wide") — **already corrected** by the MEH-970 refactor: the current comment (`MapClient.jsx:116-117`) reads "the MEH-932 producer-band default ([32.4, 34.95] zoom 8, set in MapComponent.jsx:297-305)". No change needed. **#3** (`HomepageMiniMap.jsx:41`) — assessed as **valid history**, not drift: the comment documents why `HomepageMiniMap` chose its own Tel-Aviv fallback (`[32.0853, 34.7818]`) relative to `/map`'s center *at the time*. Kept the historical rationale and appended a one-line clarification that `/map`'s default has since moved to the MEH-932 producer band and this pre-fit fallback is independent of that change. **Comment-only** — `git diff` confirms zero code lines; the 4 unrelated test-fixture coord hits (`distance.test.js`, `MapProducerCard.test.jsx`, `ProducerCard.test.jsx`) were left untouched. `npm run build` green. `Closes MEH-939`.
- **MEH-959 — adopt `detailToMessage()` across remaining 422 consumers (DRAFT):** the MEH-957 helper (`lib/errors.js` — collapses a FastAPI 422 `detail` ARRAY to one `" · "`-joined string, strips the Pydantic `"Value error, "` prefix, returns `null` for shapes it can't render) was only wired into 3 sites (GroupBuyDetailClient ×2, RegisterProducerClient, + MEH-989 dashboard). Phase-0 grep (`data?.detail` frontend-wide) found ~40 consumers rendering the raw `detail` directly into `setError`/`showToast`/`alert`/`onError` — any of which white-screens (React #31 "Objects are not valid as a React child") or shows `[object Object]` when the endpoint returns a 422 validation array. **Wired 25 files** through `detailToMessage(...) || <fallback>` — pure no-op on string/absent `detail` (identical happy-path + string-error behavior), only the array case changes (was unsafe → now one sentence): central `errorMessage()` (400/409/422 + 403 + 404 branches, no import — same module) + ReportButton + PhoneVerifyCard ×2 (the 3 named in the issue) + Google/Apple/ProducerOAuthButtons, RecipeForm, StoryCardCanvas, Footer, ReviewsSection, admin/ProducerForm ×2, VerifyEmailClient, ContactClient, settings/page ×5, LoginClient, AboutClient, events/new ×2, dashboard/group-buys, rate/[token], recipes, admin/{experiences ×3, kashrut ×2, group-buys ×2, page}, use-admin-producers ×2. **Left as-is (already crash-safe — documented census):** sites with an explicit `typeof detail === "string"` guard + custom object-shape handling — RegisterClient/ResetPasswordClient/settings (`{failures:[...]}` MEH-306), NewExperienceClient (`detail.reason`), RecipeForm submit (`detail.error==="recipe_rejected"`), admin/outreach (`detail.error==="duplicate_lead"`), producer/dashboard/edit (`typeof`-guarded); already-wired GroupBuy/RegisterProducer. `npm run build` green; vitest **686 passed / 41 skipped**. Frontend-only; `en.json`/`he.json` untouched. `Closes MEH-959`.
- **events image control i18n extraction (DRAFT):** MEH-988 inlined the event-form (`producer/dashboard/events/new/page.js`) image-control Hebrew directly in JSX under a single-file scope constraint; this extracts the 6 strings to `t()` keys under the existing `sweep_tail.event_new` namespace (`image_label`, `image_upload_hint`, `image_uploading`, `image_remove`, `image_remove_aria`, `image_upload_error`) and removes the orphaned `field_image_label`/`field_image_placeholder` left behind when the raw Cloudinary URL input was replaced. **Both locales edited** — the MEH-978 `en-parity-guard` (he→en key presence) requires en twins for the new keys, so `en.json` gets the 6 English strings and the orphans are removed there too (keeping the reverse MEH-840 guard green). No behavior change; `npm run build` + vitest green.
- **MEH-989 — availability toggle surfaces backend error detail (DRAFT):** the producer dashboard availability toggle's `setAvailabilityState` catch was bare (`catch {`), collapsing every failure into the generic `t("error_availability_update")` alert — so the two real cases (picking "on vacation" without a date → 422, and the 20/hour rate limit → 429) showed a useless "try again later". Now binds `catch (err)` and routes `err?.response?.data?.detail` through `detailToMessage()` (MEH-957, `lib/errors.js`) so the backend's Hebrew message shows, and a FastAPI 422 detail-array never renders as "[object Object]"; falls back to the generic string only when no usable detail exists. Optimistic update + refetch-on-failure unchanged; still `alert()` (the showToast migration is MEH-964's scope). Single file (`producer/dashboard/page.js`), no backend/i18n/env/migration. `npm run build` green. `Closes MEH-989`.
- **MEH-986 chunk 3a (REAL) — ProducerHeader kosher chip → verified-only (P0 legal, DRAFT):** the producer-detail highlights strip (`ProducerHeader.jsx:145`) rendered a `StarOfDavid` "כשרות" chip from the **free-text** `producer.kosher` field (any non-empty string) — a live חוק איסור הונאה בכשרות exposure the earlier "3a" missed and chunk-2 explicitly flagged as a Known residual. Re-keyed the chip condition (`:145`) **and** the strip-wrapper visibility (`:128`) from `producer.kosher` → `!!producer.kashrut_verified_at`, mirroring `badges.js:167` exactly (the admin-verification timestamp stamped by `admin_kashrut.py:75`). Now free-text kosher + no verification → **no chip**; `kashrut_verified_at` present → chip shows. Field already on the object (`ProducerListOut/DetailOut:758`; ProducerHeader already reads it at `:158`). Label (`producer.detail.header.attr.kosher`, en `:686`/he) + `StarOfDavid` icon kept. Frontend-only, 1 component. `npm run build` green. **Unblocks 3b** (backend filter re-key + public-output `kosher` removal). `Refs MEH-986` (multi-chunk — NOT Closes). STOP after chunk 3a.
- **MEH-990 — Emoji-LOCK sweep (rest): stray UI emoji + dingbats → Phosphor across 40 files (DRAFT):** cleared the scope-guard backlog flagged by MEH-987 — every emoji/dingbat that renders in the **DOM** (JSX text, button/label strings) is now a Phosphor icon (size/weight/color tokens, `aria-hidden`, RTL-safe). Pictographs: verify-email ✉️✅❌→`Envelope`/`CheckCircle`/`XCircle`, favorites 🌿👇→`Leaf`/`ArrowDown`, privacy/terms/accessibility/contact 📧📞→`Envelope`/`Phone`, contact ⏱️→`Timer`, messages 💬→`ChatCircle`, upgrade ⭐→`Star`, HomepageMiniMap 🌿 removed (decorative), ProducersClient 📍🔍🌱🌿→`MapPin`/`MagnifyingGlass`/`Plant`/`Leaf`, StarRating ⭐→`Star`, FavoriteButton ❤️🤍→`HeartStraight`, WhatsAppShareButton/FridayDeliveryStrip/InstallPrompt/AlertPrefsPanel/RecipeDetail/SearchClient/ExperienceDetail/NewExperience/group-buys/rate/StoryCardCanvas(⬇️) + admin (reports 💡, group-buys 🛒, experiences 💰💡, users 🔒❤️, page 🆕, producers 🔴🟡🟢✡️, import-preview ✓), dashboard (insights stat-cards, followers 🌱, page 🌟✕, settings ⚠️). Dingbats (per Sapir "everything incl. dingbats"): PasswordStrength/PasswordInput ✓○…→`Check`/`Circle`/`CircleNotch`, settings ✓→`CheckCircle`, ProducerForm ✕→`X`, ProfileCompletenessCard ✓→`Check`, ProducerCard ★→`Star` (central — adversarial review), ProducerHeader ✡️→`StarOfDavid`. All icons confirmed in `@phosphor-icons/react`; Phosphor-only (no Lucide). `npm run build` green, **0 eslint errors**. Post-fix DOM emoji grep = **clean**. **Deferred (reported, NOT in this PR):** (1) **he.json copy emoji — 81 lines** → copy-approval-gated (workflow rule 22), `en.json` untouched; (2) outbound **share-text 👉** (`ShareButton:18`, `StoryCardCanvas:217`) — WhatsApp/share message body, not DOM; (3) **canvas-drawn 🌿** (`StoryCardCanvas:130`) — generated story-image art, not DOM; (4) **`MapComponent:106` ✓** — central component, ✓ lives inside an HTML-string Leaflet `divIcon` (needs SVG-string + adversarial review; glyph-LOCK follow-up); (5) **admin/reviews `<option>` ⭐ ×5** — `<option>` can't hold icon children (select-control limitation, copy-side); (6) **holidays.js holiday glyphs ×6** (render at `HolidayBanner:71` + interpolated into copy) — per-holiday menorah/pita have no clean Phosphor map; design + copy call. No automated Emoji-LOCK guard exists (manual convention). `Closes MEH-990`.
- **MEH-986 chunk 2 — kosher badge → verified-only (P0 legal, DRAFT):** the public kosher badge (`lib/badges.js`) was derived from the **free-text** `producer.kosher` field (`:161`, any non-empty string) — an unverified kosher assertion rendered on every raw-data surface (the `/producers` list cards via `producers/page.jsx:31` `res.json()`, and the producer detail page via `serverFetch`) = live חוק איסור הונאה בכשרות violation. **Re-keyed the `earnsBadge` "kosher" case to `!!producer.kashrut_verified_at`** — the admin-verification timestamp stamped by `admin_kashrut.py:75` on cert approval (alongside `kashrut_badges`), the same "verified signal" shape the verified badge uses (`badges.js:136`). Now: free-text kosher + no verification → **NO** kosher badge; `kashrut_verified_at` present → badge shows. Both fields already exposed on `ProducerListOut` (`schemas.py:755-756`) + `ProducerDetailOut`. **Phase-0 note:** the home grid + map feed parse producers through `ProducerSchema` (`lib/schemas.js:7`), a `z.object` that strips undeclared keys — `kosher`/`kashrut_verified_at` are both stripped there, so the badge was already absent on those surfaces and stays absent (no regression; verified producers won't get the badge on home/map cards until `kashrut_verified_at` is declared in `ProducerSchema` — a separate, optional enhancement, not required for the legal fix). **en.json parity:** mirrored the chunk-1 descriptor fixes — `popular_descs.meat` "Fresh, kosher, from the farm" → "Fresh, local, quality", `artisan_dairy.tooltip` dropped ", kosher". 3 `badges.test.js` cases updated to the verified-gated contract (free-text kosher → no badge; `kashrut_verified_at` → badge); vitest 33/33 green. **Reviewed, left as-is (each drives something not removed this chunk):** en `:414` onboarding filter hint (the kosher *filter* still exists — separate chunk), en `:686` ProducerHeader ✡️ badge label + en `:1022` dead map-sheet badge (display surfaces, out of scope). **Known residual (flagged):** `ProducerHeader.jsx:145` ✡️ badge + en `:686` still render from free-text `producer.kosher` — same legal class, a *component* display badge (scope says don't edit components); needs its own fix. **No backend / no filter / no verified-badge change** (filter + public output = chunk 3). `npm run build` green; en.json valid; `badges.js` 0 eslint errors. `Refs MEH-986` (multi-chunk — NOT Closes). STOP after chunk 2.
- **MEH-987 — Emoji-LOCK sweep: 4 EmptyState `emoji=` → Phosphor `icon=` (DRAFT):** converted the 4 remaining `EmptyState emoji=` props (flagged by MEH-982/MEH-979b) to the Emoji-LOCK-safe `icon=` prop: `settings/page.jsx` 🥕→`Carrot`, `producer/dashboard/recipes/page.js` 🍞→`Bread`, `producer/dashboard/followers/page.js` 🌱→`Plant`, `components/ReviewsSection.jsx` ⭐→`Star` (already imported). All 4 icons verified present in `@phosphor-icons/react`; Phosphor-only (no Lucide); tokens for size/color (the shared `EmptyState` renders `size=56 text-primary`). `grep emoji=` over `app/`+`components/` → **0 remaining**. `npm run build` green, 0 eslint errors. No automated emoji-lock guard exists (manual convention). **Scope-guard finding (LISTed, not fixed):** the frontend has many more stray UI emoji literals beyond the 4 `emoji=` props (verify-email ✉️✅❌, favorites 🌿👇, privacy/terms 📧, messages 💬, upgrade ⭐, HomepageMiniMap 🌿, ProducersClient 📍🔍🌱🌿, StarRating ⭐, FavoriteButton ❤️🤍, WhatsAppShareButton 💬, StoryCardCanvas 🌿⬇️👉, ShareButton 👉, lib/holidays data) + dingbats ✓★○♥ (likely intentional glyph-LOCK) — recommend a dedicated follow-up rather than expanding this PR. `Closes MEH-987`.
- **MEH-986 chunk 1 — remove unverified kosher claims from category descriptors (P0 legal, DRAFT):** Israeli law (חוק איסור הונאה בכשרות) forbids presenting a business as kosher *in writing* without a valid certificate, so two blanket category descriptors that asserted "כשר" for entire categories were made kashrut-neutral. **`he.json:2389`** `popular_descs.meat` "טרי, כשר, מהחווה" → "טרי, מקומי, איכותי" (Sapir-LOCKED). **`he.json:2321`** `artisan_dairy.tooltip` "חלב ומוצריו ישירות מהחווה, כשר" → "חלב ומוצריו ישירות מהחווה" (dropped ", כשר" only). **he.json only** — `en.json` left untouched because neither CI guard requires it for a value edit: `i18n-icu-parity.yml` checks ICU plural-forms only (these have none), and the MEH-978 `en-parity-guard.test.js` checks key *presence* (both keys still exist in both locales). **Verified cert-gated kashrut badge labels (`:1586`/`:2292`/`:2308`), disclaimers (`:2945`/`:2954`), and the admin kashrut UI (`:1551-1555`) are UNTOUCHED** (confirmed via grep). Descriptors-only chunk: the consumer kosher **filter is NOT touched** here — it returns as MEH-986 **chunk 2 = verified-only** (re-keyed off `kashrut_verified_at`, not removal); backend is chunk 3. **Known residual (out of scope, flagged):** the English `en.json` meat/dairy descriptors still read "kosher" (same legal claim on the /en locale) — fold into a later chunk or follow-up; and the `ProducerHeader.jsx`/`lib/badges.js` free-text `producer.kosher` display badges remain (separate display-badge surface). Supersedes the broad PR #1411 (MEH-971), closed unmerged in favor of this chunked series. `npm run build` green; he.json valid. `Refs MEH-986` (multi-chunk — NOT Closes). STOP after chunk 1.
- **MEH-982 follow-up — Emoji-LOCK fix: group-buys dashboard EmptyState (DRAFT):** `producer/dashboard/group-buys/page.js:288` passed `emoji="🛒"` to `EmptyState` — an Emoji-LOCK v2 breach (the public group-buys list already moved off this emoji to a Phosphor `ShoppingCart` in MEH-862). Added an optional `icon` prop to `components/ui/EmptyState.jsx` (backwards-compatible — renders a Phosphor icon when given, else the legacy `emoji`; existing consumers untouched) and switched the dashboard empty state to `icon={ShoppingCart}` (`size=56 text-primary`, mirrors the public list). File now emoji-free. **Out of scope (siblings flagged for follow-up):** `settings/page.jsx:1003` 🥕, `recipes/page.js:105` 🍞, `followers/page.js:71` 🌱, `ReviewsSection.jsx:301` ⭐ still pass `emoji=` — same one-line `icon=` migration each. No automated Emoji-LOCK guard exists (manual convention). `npm run build` green, 0 eslint errors. `Refs MEH-982` (the Emoji-LOCK quick-win it flagged; discovery issue stays open for the sibling follow-ups).
- **MEH-979 — /map legend breakpoint collision fix (DRAFT):** on the `/map` mobile shell (`<lg`, `MapClient.jsx:336`), the desktop category legend was gated `hidden md:block` (`MapPane.jsx:146`), so in the **768–1023px band** it rendered (bottom-left, z-800) alongside the `lg:hidden` NearMePill — two map controls live at once. Fix: `md:block` → **`lg:block`** so the legend shows only on the true desktop shell (matching the desktop GPS button `lg:flex` + the `lg:grid` desktop layout). One-class change; only the two breakpoint usages in `MapPane.jsx` (`lg:flex` GPS + this legend) exist, so nothing else assumed `md`. Verified via Chromium repro at 800px (legend + toggle gone, pill clear) and 1024px (legend present, pill hidden). `npm run build` green, 0 eslint errors, RTL clean. `Closes MEH-979`.
- **MEH-977 — Vercel→Railway server-fetch resilience (DRAFT):** the App Router server-side `fetch(${API_URL}…)` data loads had no timeout or retry, so an intermittent `TypeError: fetch failed` (Railway cold-start dropping the first connection / a stale undici keep-alive socket → `cause.code` `ECONNRESET`/`UND_ERR_SOCKET`) surfaced as a missing producer/event/etc. New server-only `lib/server-fetch.js` `serverFetch()` adds an explicit per-attempt timeout (AbortController, 8s) + a bounded backoff retry (2 retries, 250ms→500ms ≈2× backoff) **only** on those two transient socket codes — a non-transient error and any HTTP response (incl. 4xx/5xx) are returned/thrown unchanged (no retry on a 404). Routed the server call-sites through it: `[slug]` (producer by-slug), `producer/[id]`, `map` (producers SSR), `events/[id]`, `experiences/[id]`, `group-buys/[id]`, `[slug]/recipes/[recipe_id]`, and `sitemap.js` (producers + events). **home/dashboard are client axios (`lib/api.js`) — out of scope by design.** No new env var, no infra change. Unit test `__tests__/server-fetch.test.js` (5 cases: transient retry-to-success ×2, no-retry on non-transient, exhausts retries, no-retry on non-ok). `npm run build` + vitest green. `Closes MEH-977`.
- **MEH-980 — home.cta copy → variant B (DRAFT, Sapir veto at merge):** updated `he.json` `home.cta` body lines to variant B (verbatim) — `body_l1` "מהמקור הוא הבית של בתי עסק מקומיים בישראל — כל עסק נבחר אישית.", `body_l2` "תקבלו עמוד משלכם: תמונות, סיפור, וקו ישיר ללקוחות.", `body_l3` "חינם, בלי עמלות. נשמח להכיר." Leads with what the business gets + the free/no-commission reassurance. `heading`/`button` unchanged; `en.json` untouched. `COPY_BANK.md` Homepage-bottom-CTA entry updated in the same PR (brand-before-code). No component/layout change — `HomeStaticBlocks.jsx:HomeCTA` already renders 3 independent `<p>` lines (no fixed height); verified via Chromium screenshots at 360px + desktop (clean wrap, button intact). `npm run build` + he.json valid. `Closes MEH-980`.
...
- **MEH-976 — newsletter footer bidi fix (DRAFT):** the global footer newsletter input (`Footer.jsx`) had a Hebrew placeholder colliding with the submit arrow. Root cause: MEH-968's `dir="ltr"` pinned the Hebrew placeholder to the start of an LTR run — the **same** physical side as the arrow (`end-0` → inline-end of the RTL form) — so the `ps-11` padding couldn't keep it clear. Fix: dropped `dir="ltr"`, set `unicode-bidi:plaintext` so direction resolves per content (Hebrew placeholder → RTL, opposite the arrow → clear gap; typed Latin email → LTR), and switched `ps-11`→`pe-11` so the 44px reserves the arrow side for the LTR email value too. Verified via Chromium screenshots at 360px + desktop in RTL (placeholder + typed-email states). Touches only this input/button (+ file-header History). `npm run build` green; 0 RTL violations. `Closes MEH-976`.
- **MEH-975 — group-buys commit 422 + React #31 crash fix (DRAFT):** committing to a group buy could white-screen. Root cause (frontend bad-request): `GroupBuyDetailClient.jsx` `onChange` set `quantity` to `Number(e.target.value)`, so clearing the number field sent `0` (or a >100 value when `max_participants` widened the input `max`), violating `GroupBuyCommitRequest.quantity` (`schemas.py:1602`, `int ge=1 le=100`) → **422**. The 422 `detail` ARRAY was then set straight into `error` state and rendered as a React child (`:312`) → **React #31** ("Objects are not valid as a React child"). Fix: (a) both commit/cancel catch branches now route through `detailToMessage()` (`lib/errors.js`, MEH-957) so a `detail` array collapses to a single string, never crashing; (b) `clampQuantity()` bounds the POST quantity to 1–100 (and the input `max`/onBlur) so a schema-invalid value never leaves the client. Frontend-only; new regression test `__tests__/GroupBuyCommit422.test.jsx` forces a 422 and asserts a plain-string alert (no crash). `npm run build` + vitest green; backend untouched (`test_group_buys_api.py` → CI). `Closes MEH-975`.

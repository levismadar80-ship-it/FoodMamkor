...
- **feature/mobile-layout-fixes** — Mobile layout audit + fixes: (1) `/producer/:id` sidebar — FavoriteButton + ShareButton row was mismatched (FavoriteButton had a bordered wrapper with `py-2`, ShareButton had its own compact `px-3 py-2 rounded-[8px]` styling and hid its label on small screens); fixed by passing `className` prop to both components so ProducerDetail gives them identical `w-full py-3 border rounded-[10px]` treatment. "Show on map" button bumped from `py-2.5` to `py-3` for consistent 48px touch targets across all sidebar actions. (2) `/register` + `/register/producer` form inputs — all inputs were `px-3 py-2` while `/login` used `px-4 py-3`, making the register forms feel cramped on mobile; bumped to match login. ShareButton and FavoriteButton gained optional `className` / `labelClassName` props (defaults to existing compact style so other consumers are unaffected).

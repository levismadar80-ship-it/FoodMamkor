...
## Unreleased
- **MEH-986 chunk 3b (FINAL) — kosher filter verified-only + free-text kosher off public output (P0 legal, DRAFT):** the last חוק איסור הונאה בכשרות exposure in the backend. **(1) Filter re-key** — the public `?kosher` filter (`services/producer_listing.py:183-192`, applied by `_apply_scalar_filters`) matched the **free-text** `Producer.kosher` column (any non-empty string = "kosher"), so an unverified self-asserted producer was returned by `?kosher=true`. Re-keyed to the admin-verification timestamp: `kosher=true` → `Producer.kashrut_verified_at.isnot(None)`, `kosher=false` → `.is_(None)` (same signal `admin_kashrut.py:75` stamps on cert approval; mirrors chunk-2 `badges.js:167` + chunk-3a `ProducerHeader.jsx:148`). **(2) Public output** — removed `kosher: str | None` from `ProducerListOut` (`schemas.py:720`), which also strips it from `ProducerDetailOut` (inherits ListOut) → the free-text field no longer serializes on any public surface. **Re-declared** it on `ProducerAdminOut` + `ProducerOwnerOut` (both inherit `ProducerDetailOut` and would silently lose it) so admin queue + owner's own dashboard still see the raw input. `ProducerAdminCreate`/`ProducerUpdate` kosher **input** fields untouched (owners still submit it); `kashrut_verified_at` stays public (`:758`). **No Alembic** — the column stays in the model, only its serialization + filter semantics change. Tests: `tests/test_meh986_kosher_verified.py` (4) — 2 schema-level (`model_fields` presence, no DB) + 2 DB (verified producer returned by `?kosher=true`, free-text-only producer excluded from true / returned by false). `ruff check` + `format` green; DB tests deferred to CI (no local Postgres). `docs/DATA.md:381` filter description updated to verified-only. `Refs MEH-986` (final chunk — NOT Closes; Sapir owns merge). STOP after chunk 3b.

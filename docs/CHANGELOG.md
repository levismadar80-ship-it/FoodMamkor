...
## Unreleased
- **MEH-971 — remove unverified kosher claims from consumer surfaces (P0 legal, DRAFT):** Israeli law (חוק איסור הונאה בכשרות) forbids presenting a business as kosher *in writing* without a valid certificate, so the blanket/self-declared kosher framing was removed from the consumer UI. **Five strings + one filter.** (1) `he.json:2389`/`en.json:2368` meat category descriptor "טרי, כשר, מהחווה"/"Fresh, kosher, from the farm" → "טרי, מקומי, איכותי"/"Fresh, local, quality" (LOCKED copy). (2) `he.json:2321`/`en.json:2300` artisan_dairy tooltip dropped the trailing ", כשר"/", kosher". (3) `he.json:429`/`en.json:414` home onboarding hint dropped "כשר"/"kosher" from the filter list. (4) **Consumer kosher FILTER removed end-to-end:** the chip is a **hardcoded** `{key:"kosher",label:"כשר"}` in `lib/producer-filters.js:3` (NOT an i18n key — the spec's `:682`/`:1018` premise was a mis-locator) rendered via `CHIPS_CONFIG` by **both** `HomeProducersGrid.jsx:67` and `ProducersClient.jsx`; removed it from `CHIPS_CONFIG` + `CHIPS_DEFAULT` + `buildChipParams`, plus the dormant `kosher` chip state/URL read/write in `lib/use-home-page.js` (delete-not-disable, workflow.md smell #1). Backend `producers.py:61` `kosher` filter param left **dormant** (no consumer caller) — no backend change this PR. **en.json edited for legal parity** (the ICU-plural-only guard didn't force it, but English is a live consumer locale carrying the identical claim). **UNTOUCHED:** verified cert-gated kashrut badges (`he.json:2292+`, `KashrutBadgeStrip`), admin kashrut approval UI (`:1552-1555`, `ProducerForm` options), legal disclaimers (`:2945`/`:2954`). **Known residual (out of this spec's scope, follow-up needed):** the producer-detail header ✡️ badge (`ProducerHeader.jsx:145`, key `:682`) and the `lib/badges.js` ProducerCard kosher pill still render free-text `producer.kosher` — same legal class, display-badge surface, deliberately not in scope here. `map.sheet.badge.kosher` (`:1018`) is already dead (MEH-826 removed its binding; only `badge.verified` renders). `npm run build` green; he/en JSON valid; no test asserts the kosher chip (`ProducersClientSearch` mocks `CHIPS_CONFIG:[]`). `Refs MEH-971` (legal sub-task; epic stays open).

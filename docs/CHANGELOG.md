...
## Unreleased
- **MEH-870 — letter-floor on `ProducerRegister.short_description` + `address` (public registration parity, DRAFT #1267):** the public producer-registration path collected the `short_description` (tagline) and `address` fields with only the bleach sanitize strip, so a **punctuation-only** value (e.g. `"???"`) was accepted — unlike `ProducerCreate.name` / `HomeProductCreate.title`, which already enforce the ≥3-letter floor via `_min_letters_validator` (regex `[א-תa-zA-Z]` — counts Hebrew letters, no risk to legit Hebrew copy; same MEH-555 pattern). **`backend/app/schemas/schemas.py` only:** two stacked `field_validator`s on `ProducerRegister` reusing `_min_letters_validator` (no new regex), running **after** the existing sanitize validators (bleach first → count letters; mirrors `HomeProductCreate._sanitize_title → _validate_title_letters`). Both fields are **optional**, so an absent value (`None`, incl. bleach-emptied input like `"<b></b>"`) stays valid — only a *provided* value must clear the floor. **Scope note / honest framing:** this closes the gap on the **public registration path only**. The `ProducerUpdate` / `ProducerAdminCreate` twins do **not** currently letter-validate these fields (and `AdminCreate` has no `address` field), so true "parity with the twins" is a separate follow-up — the validation was deferred from MEH-829 (#1233). **Tests** (`tests/test_validation_hardening.py`, +4): punctuation-only `short_description`/`address` → 422; regression — short Hebrew tagline (`"אוכל ביתי"`) + address with digits (`"רח' הרצל 5"`) → 200 (endpoint returns 200, not 201). Local pytest green (8 in-file; 267 across the registration/schema regression sweep incl. `test_api.py`); ruff clean. No Alembic (no column change), no frontend, **en.json untouched (freeze)**. Closes MEH-870.

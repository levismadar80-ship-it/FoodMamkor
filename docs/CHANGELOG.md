...
## Unreleased
- **MEH-897 — ProfileCompletenessCard yellow>70 checklist state (✅ merged #1298, `adcb5ac`):** dashboard card, `ProfileCompletenessCard.jsx` + `he.json` + test only. At **>70% complete** ("almost there") the single next-step line is replaced by a **5-row checklist** (completed + remaining); the top remaining field stays emphasized in a next-step **box** + CTA below. **red / yellow≤70 / green branches byte-identical** (preserved in the `else`). Completed rows = `text-primary` ✓ (reuses the green-branch precedent) + label; remaining = `text-fg-muted`, **no marker / no status-dots / no state palette** (ADR-019); per-row done/remaining state exposed to AT via `sr-only`; rendered as `<ul>`/`<li>` with `aria-label`. Next-step **box = faint primary tint via opacity** (`bg-primary/5 border-primary/20`, REUSES `KashrutBadgeStrip.jsx:53`) — no grey, no new hex. **Applicable-5 derived in-component** mirroring the heuristic's `coords` XOR `delivery` split (`lib/producer-completeness.js:25 isDeliveryOnly` — `COMPLETENESS_FIELDS` is 6 keys, exactly 5 apply per producer); `lib/` untouched. Row labels **reuse existing `fields.*` keys**; only 3 checklist-specific a11y keys added (`checklist_aria`/`done`/`todo`), `he.json` only (en deferred to MEH-472). **Automated PR review caught + fixed 1:** `/en` is an active locale that reaches yellow-high → the he-only a11y keys would surface raw key paths; **gated `isYellowHigh` on `locale === "he"`** so `/en` falls back to the localized inline next-step (mirrors the MEH-884 trust-strip locale gate). vitest **8/8**; build green; ESLint 0 errors; grep gate clean (0 physical RTL / Lucide / emoji / `bg-gray-*` / new hex). All 6 required checks green (backend correctly skipped). `Refs MEH-897` — Sapir merged after the review-fix push (Rule 23 mobile QA pending on staging). Complements MEH-288 (origin); coordinate anchor copy before MEH-290 (onboarding tour) builds.

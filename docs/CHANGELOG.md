...
## Unreleased
- **MEH-1120 (MEH-1074 Task B) — unify verification badge: TrustBadge recognition-only (Refs MEH-1074):** producer Detail + Card rendered two parallel badge systems for the same verification signal — `BadgeRow` (verification_tier gold "מאומת" seal, ADR-022) **and** `TrustBadge` tier-3 "✅ עסק מאומת" (green, MEH-51) — plus tier-2 "✓ מספר מאומת" (phone) reading as business verification. `TrustBadge.jsx` now self-gates to **tier ≥ 4** (renders only the recognition tiers 4 "⭐ מובילת קהילה" + 5 "🏅 שגרירת מהמקור"); the verification tiers 2/3 are dropped from every reader surface — verification is owned solely by BadgeRow/verification_tier (ADR-022), and "מספר מאומת" no longer appears on the public header (ADR-022 gate 1). `ProducerCard.jsx:291` gate raised `>= 3` → `>= 4` (central component — adversarial pass). `ProducerHeader.jsx` comment documents the single-affordance contract. `lib/badges.js` untouched; `is_verified`/schema untouched (MEH-766). Tooltip: TrustBadge (tier 4/5) keeps ui/Tooltip, already touch-capable (MEH-792) — Popover unification deferred (over-engineering guard). `TrustBadge.test.jsx` rewritten (tiers 2/3 → null, 4/5 render; 68 badge/card/header tests pass). Verify: build exit 0 · 0 physical RTL.

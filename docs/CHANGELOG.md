...
- **feat/forgot-password-placeholder (tasks_for_claude_code.md PR 7 — task 6)** — Add `"שכחתי סיסמא ←"` link directly below the password input on `/login`, right-aligned (Hebrew start-of-line). The task spec has a conditional: *"If [endpoint] exists: implement full flow. If no: show toast message and add TODO comment"*. Verified against `docs/DATA.md` §Auth — the backend exposes `/auth/register`, `/auth/register/producer`, `/auth/login`, `/auth/me`, `/auth/google`, `/auth/apple`, and `DELETE /users/me` — but NO `/auth/forgot-password`. Fell through to the placeholder branch: click fires an info toast (`"נשלח לך מייל לאיפוס סיסמא בקרוב"`) via the existing `showToast` utility at `lib/toast.js`, no network request. Implementation uses `type="button"` so it doesn't accidentally submit the login form, and an inline `TODO:` comment block above the link documents the code the follow-up PR must write once the backend endpoint ships (replace the toast with a modal → email input → POST → success state). First PR of the 🟠 UX tier — all 8 🔴 red-tier tasks (1, 2, 3, 4, 5, 16, 17, 18) merged into staging before this one. `MANUAL_TESTING.md` gains a "Forgot password link on /login" section with 9 test cases including a "zero network requests on click" assertion so a future tester can catch the regression where someone forgets to remove the placeholder after wiring up the real endpoint.

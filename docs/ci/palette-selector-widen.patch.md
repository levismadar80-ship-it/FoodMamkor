...
| findings without the prefix (the paste) | 248 | **283** in 71 files |
...
**The one non-class hit, and why this PR neutralises it rather than STOPping.**
`RequiredMarkerParity.test.jsx:109` holds the literal
`'label={<>{t("title_label")} <span className="text-red-500">*</span></>}'` — a
JSX-*source* fixture (the exact line that shipped a regression, kept as the
scan's control). It contains a shade but is not a class string, so the card's
own STOP fired on drain 25. The card's rule message names the escape hatch —
`// token-ok` + `eslint-disable-next-line` — and `frontend/__tests__/**` is a
CC-writable path, so this PR applies exactly that to the one line. With it, the
widened selector's **283 hits are all class strings**: 266 in components / app
(`className=`, lookup objects, ternaries, template literals) and 17 in tests that
are class-string assertions or fixture tables (`toHaveClass("text-red-600")`,
...
strings (rendered by `HolidayBanner.jsx`). 102 additional findings, all `warn`.

## Post-paste expectation

```
npx eslint . --format json | <count messages starting "Raw Tailwind palette">
# expected: 283 · 0 errors · severity warn
```

A count of 181 after pasting means the prefix is still there. A count far above
283 means the paste widened more than this one entry.

## Optional follow-up, Sapir's call (not in this PR)

17 of the 283 are in `__tests__/**`. If the rule should not read test

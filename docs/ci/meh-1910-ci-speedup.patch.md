...
> ## 🔄 תוקן 05/08 — strict **דלוק עכשיו**. הסעיף שמתחת לזה היה נכון לחלון שנמדד ואינו נכון להיום.
>
> הפסקה למעלה סייגה את עצמה נכון ("as-of 03/08 16:53"), והסייג הזה נפרע. שלוש
> ראיות שנאספו ב-05/08, אחרי שהקובץ הזה כבר נמזג:
>
> 1. **`mergeable_state: "behind"`** על PR #2623 החי. GitHub מדווח `behind` כאשר
>    ה-base התקדם **ויש דרישת up-to-date**; בלי הדרישה, PR מפגר-אך-מיזוגי מדווח
>    `clean`.
> 2. **12 מתוך 12 ה-merges האחרונים ל-`staging` הם up-to-date** — לעומת 55%
>    שאינם, בחלון ההיסטורי.
> 3. **הראיה המכריעה — GitHub עדכן ענף בעצמו לפני מיזוג.** commit `123141f6`,
>    *"Merge branch 'staging' into feature/meh-1910-ci-baseline-and-staged-patches"*,
>    מחבר `sapirschnapp`, נוצר **15:14:16** — שתי דקות לפני שה-auto-merge מיזג את
>    PR #2622 ב-15:16. **CC לא יצר את ה-commit הזה.** עדכון-ענף-אוטומטי-לפני-מיזוג
>    הוא בדיוק ההתנהגות של auto-merge כאשר "Require branches to be up to date"
>    דלוק.
>
> **המשמעות — ההנחה של §5 בכרטיס נכונה היום, וההמלצה שמתחת שגויה בסעיף 1 שלה.**
> הכאב של "Update branch" **כן** נגרם ע"י ה-ruleset, ו**כיבוי strict הוא לבר
> אמיתי** — בניגוד למה שכתוב מיד מתחת. סעיפים 2–4 (merge queue) **לא הושפעו**:
> הנימוק שם הוא ש-merge queue מוסיף זמן CI לסוויטה איטית, וזה נכון ללא תלות ב-strict.
>
> **מה כן נשאר נכון בטקסט המקורי:** המדידה ההיסטורית (91/164 מיזוגים מאחור עד
> 03/08 16:53). strict לא היה דלוק אז. מתי בדיוק הוא נדלק — לא ניתן לקבוע מכאן,
> וה-403 על ה-ruleset עדיין בתוקף. **עדיין דורש אישור מול המסך.**

### ההמלצה

> ⚠️ **סעיף 1 שלהלן שגוי — ראו התיקון מ-05/08 למעלה.** נשמר כלשונו כדי שהתיעוד
> יראה מה נטען ועל סמך מה, ולא כדי שיפעלו לפיו.
...
3. **זמינות:** merge queue דורש repo ציבורי, או Team/Enterprise ב-repo פרטי.
   `FoodMamkor` פרטי, ואת רמת ה-plan לא הצלחתי לקרוא (אותו חסם 403).
   **צריך אימות מולך** לפני שהאפשרות בכלל על השולחן.

...
> ## ✅ עדכון (26/07, MEH-1601) — תנאי מוקדם A כבר מתקיים
>
> **ה-paths-filter ב-`e2e.yml` כן מדלג docs-only היום.** בלוק ה-`filters` החי
> (`e2e.yml:62-70`) הוא **חיובי בלבד**, בלי דפוסי-שלילה ובלי `predicate-quantifier`
> — כלומר **בדיוק הבלוק ש"תנאי מוקדם A" למטה מבקש**. ראיה אמפירית: run
> `30220080416` (הדחיפה ה-docs-only `80d5c62` ל-staging) — ה-job
> `Playwright E2E (Vercel preview)` דיווח `skipped`. הסעיף "תנאי מוקדם A" נשמר
> כתיעוד היסטורי של הבאג ושל התיקון, ואין לבצע אותו שוב.
>
> **התנאי שעדיין חוסם הוא B בלבד — הסוויטה אינה ירוקה:** 2 כשלי VRT
> ב-`parity.spec.ts` (`map` דסקטופ, `home` מובייל) נכון ל-run `30220096957`.
> **אין להוסיף את ה-context ל-ruleset עד ש-B מתקיים.**
>
...
**וזה כבר בוצע** (26/07, MEH-1601; ראו העדכון בראש הקובץ).
...
> * **תנאי B — ❌ פתוח, אבל עם סט כשלים אחר לגמרי.** ארבעת הכשלים המקוריים
>   (parity `/producer/[id]` ×2 + a11y `/producer/[id]` ×2) **אינם מופיעים יותר**.
>   בריצה `30221482924` (סוויטה מלאה, 146 passed / 3.9m): **1 failed** —
>   `e2e/flows/25-role-reachability.spec.ts:133` (צרכנית לא מנותבת מ-
>   `/producer/dashboard`; קיבלה `http://localhost:3000/` במקום `/login`) —
>   ו-**1 flaky**, `e2e/flows/02-search-producer.spec.ts:4`
>   (strict-mode: `[data-testid="hero-search"]` מתאים לשני inputs, ה-double-mount
>   המוכר). ה-job מריץ `--fail-on-flaky-tests`, אז flaky = אדום.
...
>
> **המשמעות המצטברת:** החסם היחיד שנותר הוא **B**, והוא שני specs פונקציונליים
> ב-`e2e/flows/` — לא VRT. לשניהם אין ticket (ה-workspace ב-Linear הגיע לתקרת
> ה-issues בתוכנית החינמית), ולכן הם מדווחים כאן ולספיר ישירות.

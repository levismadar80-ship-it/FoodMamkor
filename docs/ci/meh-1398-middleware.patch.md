# MEH-1398 — final `frontend/middleware.js` (human-apply)

`frontend/middleware.js` is in the CC Edit-deny list (`.claude/settings.json`),
same class as `next.config.js` / `main.py` / `.github/workflows`. CC cannot land
this file; apply it as-is (Sapir), or unlock the file for CC to finalize.

This is the **hardened** version of the measured spike (branch
`feature/meh-1398-hard-404-matched-route`, PR #2001): it imports the guarded
`lib/static-routes.json` registry instead of an inline list, and drops the
"spike" framing. Behaviour is identical to the committed spike (already measured:
misses → real 404, static routes → 200, valid → 200, hreflang intact, +1 lookup
only on cold valid producer pages).

...
**הכלל בשורה אחת:** הכותרת אומרת מה המצב. התיאור אומר למה זה שווה. הפעולה יושבת בתוך המצב הריק.

**הבעיה:** עד היום §4 הכריע רק את הטון של ריקנות (`"אין תוצאות"` מול `"לא מצאנו בינתיים"`), לא את המבנה — ולכן כל כלי בדשבורד המציא אחד. בקבוצות רכש הכותרת היא הצעת ערך, `"קבוצות רכש = מחיר סיטונאי ללקוחות שלך"`; באירועים היא הצהרת מצב, `"עדיין אין אירועים"`. שתי תבניות לאותו מצב, בשני כלים באותו דשבורד (audit 28/07).

**המבנה:**

1. **כותרת = המצב**, בעברית פשוטה וחמה: `"עדיין אין אירועים"`, `"עדיין אין קבוצות רכש"`. לא הצעת ערך, לא סלוגן, לא שאלה.
2. **תיאור = ההסבר או התועלת** — משפט או שניים, בגובה העיניים. זה המקום של הצעת הערך.
3. **ה-CTA הראשי יושב בתוך המצב הריק** (Polaris), לא ככפתור בראש העמוד. כשהמצב הריק נושא CTA, הכפתור המקביל בראש העמוד לא מרונדר: במסך ריק יש כפתור אחד, לא שניים.
4. **פעולה חסומה: בלי כפתור בכלל** — לא מעומעם ולא פעיל. התיאור מסביר למה הפעולה נעולה ומוביל למה שפותח אותה. כפתור disabled נראה לחיץ ואינו מגיב, וזה בדיוק הבלבול ש-NN/g מתעדת.
5. **Emoji LOCK v2 חל על כל מחרוזת** של מצב ריק. האייקון עובר כקומפוננטת Phosphor ב-prop `icon`, לעולם לא כתו בתוך המחרוזת.

**do / don't** — שתי המחרוזות האמיתיות מה-audit:

**אסור** — הצעת ערך בתפקיד הכותרת:

- `title: "קבוצות רכש = מחיר סיטונאי ללקוחות שלך"`

**מותר** — אותו משפט עצמו, בתפקיד התיאור, מתחת לכותרת שמדווחת על המצב:

- `title: "עדיין אין קבוצות רכש"`
- `description: "קבוצות רכש = מחיר סיטונאי ללקוחות שלך."`

**נימוק להכרעת מיקום ה-CTA:** ב-מהמקור אין action bar קבוע בעמודי הדשבורד — הכפתור העליון קיים בחלק מהעמודים בלבד. ה"עקביות במיקום" ש-Carbon מגן עליה לא קיימת כאן ממילא, ולכן נשאר רק החיסרון שלה: בעלת עסק מול מסך ריק בלי מוצא.

**תקדים חיצוני:** Shopify Polaris מחזיק את ה-CTA בתוך המצב הריק, ותת-הכותרת מתפקדת אצלו כטקסט גוף שמסביר את הכותרת. IBM Carbon (experimental) מחזיק אותו מחוץ למצב הריק כדי לשמר מיקום קבוע בין מלא לריק. NN/g: פעולה חסומה אינה מוצגת ככפתור מושבת — מסבירים למה.

**הערה לקוד:** הקומפוננטה היא `frontend/components/ui/EmptyState.jsx` (props `title` / `description` / `ctaLabel` / `icon`). ה-CTA מרונדר רק כאשר `ctaLabel` קיים וגם אחד מ-`ctaHref` / `ctaOnClick` — כך שמצב נעול מקבל מצב ריק בלי כפתור, בלי prop חדש. MEH-1709 מיישם את המקרה הנעול (קבוצות רכש לפני אישור העסק), MEH-1630 מאחד את שאר המצבים הריקים ומחיל עליהם את הכלל.

...
| `safeParse` + `notFound()` | 404 | כלום | ❌ **לא — כתשובה לכשל סכימה.** ראו התיקון של MEH-2101 מתחת לסעיף: הנימוק המקורי («404 מסכן אינדוקס») נכון לכשל *זמני* ושגוי כתיאור של ישות חסרה. |
...
### ⚖️ תיקון MEH-2101 — הברירה הבינארית הוחלפה בהפרדה תלת-דרכית

**מה שנשאר בתוקף:** ההמלצה למעלה — `safeParse` + לוג + render raw — היא על
**כשל סכימה**, והיא לא משתנה. payload פגום עדיין לא מפיל עמוד.

**מה שהוחלף:** השורה `safeParse` + `notFound()` נקראה כאיסור גורף על
`notFound()` בארבעת מסלולי §7, והיא הפכה שם ל-`return null` על **כל** כשל
fetch. כלומר 404, 500, 429 ו-timeout קיבלו את אותה תשובה — 200 עם מטא-דאטה
חלופית, כלומר soft-404 על כל אחד מהם. הפגם הוא במסגור: §6 הציג ברירה בינארית
(404 מסכן אינדוקס / 200 בטוח) בזמן שיש **שלושה** מצבים, והם על ציר אחר לגמרי
מזה של כשל סכימה.

| המצב | הקוד הנכון | מה שהזחלן לומד |
| -- | -- | -- |
| הישות באמת לא קיימת | **404** | להסיר מהאינדקס. נכון ורצוי. |
| ה-backend נפל / timeout | **5xx** | זמני, לחזור מאוחר יותר. ה-URL נשמר. |
| rate limit | **429** | הלקוח שלח יותר מדי — לא «השרת לא זמין» |

החשש של MEH-1754 מוצדק ובעינו עומד: 404 בזמן תקלה זמנית באמת נקרא כהסרה
קבועה. אבל התרופה שלו — 200 לכולם — פתרה אותו בכלי הלא נכון. הפתרון הוא
**להפריד את המקרים**, לא להשוות ביניהם כלפי מטה.

**איך זה ממומש, ולמה דווקא ב-`generateMetadata`:** `notFound()` בגוף העמוד
מזרים 200 + UI של 404, כי גבול הסטרימינג המשותף (`app/[locale]/loading.js`)
מרוקן את ה-shell קודם (vercel/next.js #77235 · #76474 · #75543).
`generateMetadata` רץ **לפני** הסטרימינג, ולכן `notFound()` שם מחזיר סטטוס 404
אמיתי — התקדים הוא MEH-1045 על מסלול ה-slug. הקובץ המשותף לא נגוע.

**האסרציות הן על סטטוס נמדד**, לא על נוכחות `notFound()` בקוד — בדיוק בגלל
הבאג הזה. בקרת ה-discrimination חיה ב-`frontend/__tests__/EntityResolverErrorSeparation.test.jsx`:
מול ה-resolver הישן 24 מ-29 האסרציות נכשלות, ואחרי התיקון כולן עוברות. חמש
האסרציות שעוברות בשני העולמות הן מסלול ה-200 ושומר הטבלה — התנהגות שלא
אמורה הייתה להשתנות.

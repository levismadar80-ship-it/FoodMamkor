...
## אמנדמנט 27/07/2026 — E2E נשאר אינפורמטיבי: דחינו את ה-scope, לא את המנגנון

**ההכרעה.** ה-aggregator `E2E gate (required)` הוחל ב-`e2e.yml` (ספיר, ידנית,
27/07 — job `e2e-gate`, `if: always()`, `needs: [filter, e2e]`), אבל **לא נוסף
ל-required status checks של `protect-staging` (15240090) ולא ייווסף**. שני
ה-required checks נשארים `CI gate` ו-`Deploy gate` בלבד. ה-E2E הוא **סיגנל
אינפורמטיבי**.

**המנגנון עצמו תקני — הוא לא מה שנדחה.** דפוס ה-aggregator הוא הפרקטיקה המקובלת,
ולא מעקף:

* ההנחיה של GitHub עצמה אוסרת path-filtering על workflow שהוא required. הסיבה
  אסימטרית ולא אינטואיטיבית: workflow שדולג ע"י **paths-filter** נשאר **Pending**
  וחוסם לנצח, בעוד job שדולג ע"י **`if:`** מדווח **Success**. זה בדיוק כשל
  MEH-892 שתועד למעלה, והוא הסיבה שהתשובה היא job-aggregator ולא הוספת ה-job
  הישיר.
* אותו דפוס בשימוש רחב: Mixpanel, Pants, Ansible DevTools, ו-`ci-gate` של
  Mergify עצמה.
* `alls-green` הוא הצורה הארוזה של אותו דפוס, וה-README שלו הוא המקור לכך
  ש-`if: always()` **יחד עם** בדיקה מפורשת של `needs.*.result` היא חובה — בלי
  שתיהן ה-aggregator מדווח ירוק על dependency שנכשל.

הבית שלנו עקבי לזה: `ci-gate` ו-`deploy-gate` בנויים כך כבר, וה-`e2e-gate` החדש
מעתיק את אותו `ok()`/`check()`.

**מה כן נדחה — ה-scope.** E2E דפדפני מול סביבת preview חיצונית הוא בדיוק
הקטגוריה שמקורות התעשייה משאירים ב-**informational set** ולא ב-required set:
כל required check נוסף הוא עוד flake שיכול לחסום את כל הרפו, וה-blast radius של
E2E דפדפני (רשת, קרה-הפעלה, תלות-נתונים, VRT) גדול מזה של build או unit tests.

**והראיה המקומית חזקה מהטיעון הכללי:** השער נשאר לא-מופעל **שבועיים**, על פני
מספר ניסיונות, וכל פעם נחסם ע"י אדום flaky או תלוי-נתונים — לא ע"י באג אמיתי
במוצר. מערכת שלא הצלחנו להפעיל שבועיים בגלל רעש היא בדיוק המערכת שאסור שתחזיק
את מפתח ה-merge. ADR-016 כבר קובע שהתדירות נגזרת מ-risk tier; זו אותה לוגיקה
בשכבת ה-gates.

**מה זה משנה בפועל:**

* Residual-risk #5 ("VRT אינו חוסם merge") **נשאר פתוח בכוונה** — הוא לא ייסגר
  ע"י הוספה ל-ruleset. הוא מנוהל ע"י קריאת התוצאה, לא ע"י חסימה.
* תנאים מוקדמים A ו-B (למעלה) הושלמו ואינם חוסמים עוד דבר; הם נשארים בתיעוד
  כרשומה של למה ה-filter נראה כפי שהוא.
* `docs/ci/e2e-gate.patch.md` הפך למסמך היסטורי; סעיף "סדר ההחלה" בו נכתב מחדש
  כך שאף צעד אינו מורה על הוספה ל-ruleset.

**⚠️ תלות פתוחה — MEH-1582.** ה-`e2e-gate` מתייחס ל-`skipped` כאל מעבר, בדיוק
כמו `ci-gate`. זו הסמנטיקה ש-MEH-1582 (In Progress, חוסם-לאנץ') בוחן. אם 1582
ישנה את הכלל הזה לרוחב הרפו, **גם `e2e-gate` צריך את אותו טיפול** — אחרת ייווצר
פער בין ה-gates. לא מטופל כאן.

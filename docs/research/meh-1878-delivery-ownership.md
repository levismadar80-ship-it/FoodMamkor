...
# נספח — האם `offers_delivery = false` הוא מצב לגיטימי? (03/08, אחרי המיזוג)

הדוח למעלה השאיר את השאלה כ"ייתכן שזו הצהרה מכוונת". הנספח הזה בודק את הטענה
הזו ישירות מול **main** — הקוד שפרודקשן מריצה. המסקנה מתחזקת ומתחדדת, ולא
משתנה: **אי אפשר להכריע מהנתונים.**

## 1. הוולידטורים של main **מתירים** את המצב באופן פוזיטיבי — MEASURED

לא "לא דוחים" — **מתירים במפורש**, בשני מסלולי כתיבה:

| anchor (`origin/main`) | מה יש שם |
|---|---|
| `backend/app/schemas/schemas.py:713-714` | ב-`ProducerAdminCreate`: `has_physical_location: bool = True` ואחריו `offers_delivery: bool = False` |
| `schemas.py:761-763` | `_validate_location_mode` — `if not self.has_physical_location and not self.offers_delivery: raise ValueError("חייב לפחות אחד: חנות פיזית או משלוחים")` |
| `schemas.py:860` | ב-`ProducerUpdate` (המחלקה ב-`:800`): `offers_delivery: bool \| None = None` |
| `schemas.py:962-967` | אותו ולידטור, עם `# Only validate when both are explicitly set (partial updates allowed)` |

הכלל הוא **"לפחות אחד מהשניים"**. לכן `offers_delivery=false` יחד עם
`has_physical_location=true` **עובר ולידציה** — הוא אינו ברירת מחדל שנשארה
בטעות ושהמערכת הייתה דוחה.

**וארבע שורות הפרודקשן עומדות בדיוק בתנאי הזה:** `has_physical_location=True`
בכולן (נמדד ב-Q1, אותה תשובת API). כלומר ארבעתן יושבות במצב שמסלול הכתיבה
**מאשר**, לא במצב שהוא סובל.

זה מחזק את ההימנעות מ-backfill: הפעלת `offers_delivery` עלולה להדליק יכולת
שבעלת עסק כיבתה במכוון דרך מסלול שנועד לאפשר לה בדיוק את זה.

## 2. אבל הוולידטור עיוור ל-`delivery_areas` — שתי שכבות, פער אחד

הוולידטור בודק שני בוליאנים על טבלת `producers`. **הוא לעולם אינו רואה את
טבלת `delivery_areas`.** לכן הוא אינו מסוגל להפריד בין שני מצבים שנראים לו
זהים לחלוטין:

| | תרחיש | מה הוולידטור רואה |
|---|---|---|
| א | עסק שבאמת אינו מספק משלוח, עם 3 שורות `delivery_areas` שנשארו מתצורה ישנה | `physical=true` → תקין |
| ב | עסק שכן מספק משלוח, שהדגל שלו מעולם לא עודכן, וה-`physical=true` לבדו סיפק את הוולידטור | `physical=true` → תקין |

שניהם עוברים. **הוולידטור עיוור בדיוק לאות שמעורר את החשד** — שלוש שורות
`delivery_areas`.

זהו **אותו פער** ש-MEH-1849 כבר תיעד כבלתי-אכיף ברמת CHECK:
*"a CHECK cannot span tables … the sibling contradiction 'rows in
`delivery_areas` while `offers_delivery = false`' lives across two tables and
is therefore NOT expressible here"*
(`alembic/versions/20260803_1200_d8c3f1a75e29:53-58`).

**שתי שכבות הגנה, אותו חור בדיוק:** ה-CHECK אינו יכול לחצות טבלאות, והוולידטור
אינו טורח. הפער אינו במימוש של אף אחת מהן — הוא בכך ששתיהן מוגדרות על טבלה
אחת בעוד העובדה מתפרסת על שתיים.

## 3. אין חותמות זמן על `delivery_areas` — ולכן אין מבחן הכרעה — MEASURED

`DeliveryArea` (`backend/app/models/models.py:699-722`) מחזיקה שש עמודות בלבד:
`id` · `producer_id` · `city` · `min_order` · `delivery_day` · `delivery_fee`.
**`grep -c "created_at\|updated_at"` על גוף המחלקה = 0.**

המשמעות: **הפרובננס של שלוש השורות בכל עסק אינו ניתן לשחזור.** אילו היה
`created_at`, ההשוואה מול מועד עדכון העסק הייתה מבחן ההכרעה — שורות ישנות
בהרבה מהצהרת הבעלים היו מצביעות על תרחיש א׳, שורות חדשות על תרחיש ב׳. **אין
דיסקרימינטור כזה, ואי אפשר לייצר אותו בדיעבד.**

### ממצא צדדי: הפניה מוצלבת שהתיישנה

ה-docstring של `ProducerLocation` (`models.py:725`, השורות `:733-735`) כותב:

> `id / producer_id FK CASCADE / created_at / updated_at follow ProducerRecipe (models.py:1328-1365) and DeliveryArea (models.py:511).`

שתי טעויות במשפט אחד, שתיהן MEASURED:

1. **`models.py:511` אינו `DeliveryArea`** — שם יושב מודל ה-`User`
   (`is_producer`, `avatar_url`). `DeliveryArea` נמצאת ב-`:699`.
2. **`DeliveryArea` אינה נושאת `created_at`/`updated_at` כלל**, ולכן אינה
   יכולה לשמש תקדים להן. `ProducerLocation` עצמה **כן** נושאת אותן
   (`:783`, `:786`) — האסימטריה היא בדיוק הפוכה למה שה-docstring טוען.

זו בדיוק המחלקה שכלל ה-anchor שנוסף היום לתבניות 06/07 (PR #2563) קיים כדי
למנוע: הפניה שנכתבה פעם, נכונה אז או לא, ואיש לא אימת אותה מאז. **נרשם
כתצפית — לא תוקן כאן** (הנספח הזה docs-only, וה-docstring הוא קוד).

## מסקנה

**השאלה אינה ניתנת להכרעה מהנתונים.** שלוש דרכי ההכרעה שנבדקו סגורות:

* הדגל עצמו — `false`, אבל זהו מצב **תקין** ולכן אינו ראיה לטעות (§1).
* הצלבה עם `delivery_areas` — הוולידטור וה-CHECK שניהם עיוורים לה (§2).
* חותמות זמן — **אינן קיימות** על הטבלה (§3).

**הדרך היחידה שנותרה היא לשאול את ארבעת בתי העסק ישירות.** אין backfill עד
שספיר מאמתת מולם.

...
## Origin
Method authored 2026-06-12 (Drive doc 128EMaVy…). Committed to repo 2026-06-15 as versioned SoT. staging = source of truth (prod stale).

## Method
Heuristic evaluation per section. Two passes (flow → detail) + a fresh-eyes pass. Single evaluator finds ~35% of issues; multi-pass compensates but does not replace real user testing.
...
- performance (LCP)
- brand + voice (vs BRAND.md / DESIGN.md + anti-patterns; incl. Emoji LOCK v2, ADR-014, "בית עסק" not "יצרן")
...
## Capture (reconciled to our infra)
Widths 375 / 768 / 1280 / 1440, fold + full. staging is SSO/Vercel-protected → CC-web env: Network=Custom + staging.mehamakor.online; Playwright headers x-vercel-protection-bypass=$VERCEL_AUTOMATION_BYPASS_SECRET + x-vercel-set-bypass-cookie=true; target /he. Prefer DOM/text probe over PNGs. Real entity for dynamic routes; skip "twt".
(Original doc said web_fetch live page — superseded: prod stale, staging gated.)

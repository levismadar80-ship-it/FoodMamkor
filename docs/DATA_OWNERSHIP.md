...
| **What it costs** | `products.price_min` / `price_max` | owner — product form | `products.price_range` free text · *(the `producers.starting_price_label` alias that used to sit here — owner path closed MEH-1851 — was DROPPED by MEH-1855 chunk 2, revision `9849fab1637a`, after 97669fe803f5 backfilled it into `producers.price_range`; its registry entry left with the column)* | MEH-2064 · MEH-1855 ✅ |

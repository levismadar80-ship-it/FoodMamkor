...
| 437 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **שורת פעולות שקטה כאורחת** — בלי להתחבר, הקישי שמירה או מעקב — **תוצאה מצופה:** מודל התחברות; אחרי התחברות הפ… |
| 441 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **שאלה שנפתחה בוואטסאפ נספרת (MEH-1886)** — התחברי כצרכנית, פתחי עמוד עסק והקישי על **קישור שאלה** (לא על תשוב… |
| 449 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **זום inline** — בסקשן "הגעה ומיקום" הקישי + / − על המפה — **תוצאה מצופה:** המפה מתקרבת/מתרחקת במקום; הפינים נ… |
| 450 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **אין מלכודת גלילה** — במובייל החליקי אצבע מעל המפה כלפי מעלה — **תוצאה מצופה:** ה**עמוד** נגלל, המפה לא זזה ו… |
| 451 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **הקשה על המפה פותחת מסך מלא** — הקישי על רקע המפה, ואז (בבדיקה שנייה) על הפין עצמו — **תוצאה מצופה:** בשני המ… |
| 452 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **אינטראקציה מלאה במסך מלא** — במסך המלא: גררי, צבטי (pinch), הקישי פעמיים, גלגלי עכבר בדסקטופ — **תוצאה מצופה… |
| 453 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **סגירה** — הקישי X (פינה עליונה) או Esc במקלדת — **תוצאה מצופה:** המפה נסגרת, הפוקוס חוזר לכפתור ההגדלה, והעמ… |
| 454 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **אותו דבר באירועים וחוויות** — פתחי `/events/[id]` ו-`/experiences/[id]` עם מיקום — **תוצאה מצופה:** אותה התנ… |
| 455 | MEH-1659 — זום במיני-מפה + הגדלה למסך מלא (27/07) | **מאחורי העסק** — עסק עם contact_name — **תוצאה מצופה:** כרטיס עם אווטאר אות אחת + שם + עיר; עסק בלי contact_n… |
| 464 | MEH-1682 — tooltip של פין במיני-מפה צמוד לפין ב-RTL (27/07) | **אירועים וחוויות ללא רגרסיה** — `/events/[id]` ו-`/experiences/[id]` עם מיקום — **תוצאה מצופה:** אלה מקבלים m… |
| 468 | MEH-1325 — לב עמוד בית העסק: סנכרון favorites-cache + ירוק (… | **סנכרון שמירה עמוד עסק → כרטיסים** — התחברי, פתחי `/producer/[id]`, לחצי על הלב (שמירה) → חזרי אחורה ל-`/prod… |
| 469 | MEH-1325 — לב עמוד בית העסק: סנכרון favorites-cache + ירוק (… | **סנכרון הפוך כרטיס → עמוד עסק** — שמרי עסק מהכרטיס ב-`/producers` → כנסי לעמוד העסק — **תוצאה מצופה:** הלב בע… |
| 470 | MEH-1325 — לב עמוד בית העסק: סנכרון favorites-cache + ירוק (… | **לב שמור = ירוק (לא אדום)** — שמרי עסק בעמוד העסק בכל 3 המופעים (הלב על הגלריה, הלב ליד הכותרת "שמור", והלב ב… |
| 471 | MEH-1325 — לב עמוד בית העסק: סנכרון favorites-cache + ירוק (… | **אורחת → מודל התחברות** — בלי להתחבר, לחצי על הלב בעמוד העסק — **תוצאה מצופה:** נפתח LoginPromptModal (בלי קר… |
| 476 | MEH-1310 — שורת "מועדפים" בתפריט האווטאר הדסקטופי (18/07) | **UserMenu דסקטופ — כלקוחה רגילה** — התחברי כלקוחה, בדסקטופ (≥768px) לחצי על עיגול האווטאר בפינת ה-Header — **… |
| 477 | MEH-1310 — שורת "מועדפים" בתפריט האווטאר הדסקטופי (18/07) | **UserMenu דסקטופ — כבעלת עסק (producer)** — התחברי כבעלת עסק, פתחי את תפריט האווטאר — **תוצאה מצופה:** הסדר *… |
| 495 | MEH-1291 — "עודכן לאחרונה" בעמוד העסק (18/07) | **עסק שלא נערך — אין שורה** — `/producer/[id]` של עסק ותיק שלא עודכן מאז ה-migration — **תוצאה מצופה:** אין שו… |
| 496 | MEH-1291 — "עודכן לאחרונה" בעמוד העסק (18/07) | **עריכת בעלים → מופיעה שורה** — היכנסי כבעלת-עסק → `/producer/dashboard/edit` → שני משהו (למשל תיאור) ושמרי → … |
...
| 631 | MEH-1256 — בחירה מהירה לפי אזור בשדה ערי משלוח (17/07) | **קליק על אזור מוסיף את כל עריו** — לחצי "השרון" — **תוצאה מצופה:** כל ערי השרון נוספות כ-chips בשדה; שמירה עו… |
| 641 | MEH-1259 — הסתרת badge "אורגני" מכל משטח ציבורי (17/07) | **אין badge בעמוד העסק** — `/producer/[id]` של עסק אורגני — **תוצאה מצופה:** רצועת ההיילייטים לא כוללת "אורגני… |
| 642 | MEH-1259 — הסתרת badge "אורגני" מכל משטח ציבורי (17/07) | **אין צ'יפ סינון "אורגני"** — דף הבית + `/producers` + `/map` (FilterSheet) — **תוצאה מצופה:** אין צ'יפ "אורגנ… |
| 651 | MEH-1471 — שדה "מאיפה שמעת עלינו?" בהרשמת בית עסק (22/07) | **הדרופדאון מופיע בשלב האחרון מעל התנאים** — `/register/producer` → עברי עד השלב האחרון (STORY) — **תוצאה מצופ… |
| 654 | MEH-1471 — שדה "מאיפה שמעת עלינו?" בהרשמת בית עסק (22/07) | **נשמר ומופיע לאדמין** — השלימי הרשמה עם ערך (למשל "אינסטגרם", או "אחר" + טקסט) → כאדמין `/admin` טאב בתי עסק … |
| 683 | MEH sweep 16/07 — דשבורד בעלת עסק (1234/1236/1237/1238/1239) | **"סיימתי להשלים — שלחו לבדיקה" (MEH-1236)** — עסק בהמתנה עם בקשת השלמה מהאדמין → לחצי על הכפתור בבאנר — **תוצ… |
...
| 722 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1363 פעולת עניין אחת בעמוד עסק** — פתחי עמוד בית עסק @375 ו-@1440 — **תוצאה מצופה:** שורת הפעולות השקטה … |
| 723 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1364 מונה עוקבות מ-favorites** — כבעלת עסק עם לבבות שמורים, פתחי `/producer/dashboard/followers` @375px … |
| 724 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1361 התראת מתכון חדש** — כלקוחה ששמרה עסק: פתחי את פאנל ההתראות ב-/favorites — **תוצאה מצופה:** 4 שורות … |
| 725 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1360 targeting התראת משלוחים לפי עיר** — חשבון צרכנית עם עיר X בפרופיל + התראת "אזורי משלוח חדשים" דלוקה… |
| 726 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1391 חיצי גלילה ברצועות כרטיסים (דסקטופ)** — בדסקטופ עם עכבר: רצועת שוק-שישי בעמוד הבית (בחלון שישי, או … |
| 727 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1383 חיצי גלילה בשורת הצ'יפים (דסקטופ)** — פתחי `/map` בדסקטופ עם עכבר (1440px) — **תוצאה מצופה:** בקצה … |
| 728 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1192 LocationModal** — ב-`/` (באנר "בחרו עיר"), `/map`, `/producers` @375px — פתחי את המודל, הקלידי "תל"… |
| 729 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1193 ניקוי /map** — פתחי `/map` @375px — **תוצאה מצופה:** אין כפתור "הצג מפה" ב-header של ה-bottom-sheet… |
| 730 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1194 כפתור קרוב-אליי** — `/map` @360/375/390 — **תוצאה מצופה:** "קרוב אליי" הוא כפתור-אייקון עגול (44×44… |
| 731 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1196 שורת שפה ב-AccountSheet** — פתחי את תפריט החשבון (טאב חשבון ב-BottomNav) @375px ב-/he וב-/en — **תו… |
| 732 | MEH-1190…1196 — sweep (טלפון בהגדרות · opt-in וואטסאפ · Loca… | **MEH-1228 שורת לוח הבקרה ב-AccountSheet** — פתחי את תפריט החשבון (טאב חשבון ב-BottomNav) @375px כבעלת עסק (ro… |
| 740 | MEH-1209 — כניסה לעריכת העסק מעמוד העסק הציבורי (owner-bar) | **בעלת העסק רואה את הבר** — התחברי כבעלת עסק ופתחי את עמוד העסק שלך — **תוצאה מצופה:** מעל שם העסק (ה-h1) מופי… |
| 741 | MEH-1209 — כניסה לעריכת העסק מעמוד העסק הציבורי (owner-bar) | **לחיצה נוחתת בעריכה** — לחצי על "ערכו את העסק" — **תוצאה מצופה:** מגיעים ל-`/producer/dashboard/edit` (טאב הע… |
| 742 | MEH-1209 — כניסה לעריכת העסק מעמוד העסק הציבורי (owner-bar) | **אורח לא רואה בר** — פתחי את עמוד העסק בלי להתחבר — **תוצאה מצופה:** אין בר, אין מקום שמור, אין קפיצת פריסה (… |
| 743 | MEH-1209 — כניסה לעריכת העסק מעמוד העסק הציבורי (owner-bar) | **צרכן/בעל עסק אחר לא רואים בר** — התחברי כצרכן, או כבעל עסק אחר, ופתחי עמוד עסק שאינו שלך — **תוצאה מצופה:** … |
| 744 | MEH-1209 — כניסה לעריכת העסק מעמוד העסק הציבורי (owner-bar) | **קישור /settings נוחת בעריכה** — ב-`/settings` → "העסק שלי" לחצי על "ערכו פרופיל עסק" — **תוצאה מצופה:** מגיע… |
| 751 | MEH-1186 — היררכיה ויזואלית ב-/producers (שפה אחת לכל התנהגו… | **אין רצועת "ביקרת לאחרונה" ב-`/producers`** (MEH-450) — פתחי `/producers` — **תוצאה מצופה:** אין רצועת "ביקרת… |
| 752 | MEH-1186 — היררכיה ויזואלית ב-/producers (שפה אחת לכל התנהגו… | **שורת בקרה אחת (בלי הרצועה הירוקה)** — בחרי קטגוריה + צ'יפ סינון + חיפוש — **תוצאה מצופה:** אין רצועה ירוקה מ… |
| 754 | MEH-1186 — היררכיה ויזואלית ב-/producers (שפה אחת לכל התנהגו… | **רוחב תיבת החיפוש** — השווי את רוחב תיבת החיפוש לשורות הצ'יפים — **תוצאה מצופה:** התיבה ברוחב מלא, מיושרת לשו… |
...
| 811 | map-quality batch PR 1 — כרטיס עסק אחיד ב-/map | **פס משלוח נשאר** — משתמשת עם עיר שמורה שעסק עושה אליה משלוח — **תוצאה מצופה:** פס "משלוח ל..." עדיין מופיע (ה… |
| 824 | MEH-1145 Wave E3 — אימוץ ui/Input בטפסים כבדים (הגל האחרון) | **הגדרות → פרופיל (`/settings`)** — התחברי ופתחי `/settings` בנייד (375px) — **תוצאה מצופה:** שדות "שם מלא" (מ… |
| 826 | MEH-1145 Wave E3 — אימוץ ui/Input בטפסים כבדים (הגל האחרון) | **הגשת חוויה חדשה (`/experiences/new`)** — התחברי ופתחי בנייד — **תוצאה מצופה:** כל השדות החד-שורתיים (כותרת, … |
| 833 | MEH-1145 Wave E2 — אימוץ ui/Input במשטחים ציבוריים | **שדרוג (/upgrade, dev בלבד)** — הדף חסום ב-production (404) — בסביבת dev: שדה האימייל בשורת ההרשמה נראה קנוני… |
...
| 871 | MEH-1103 — כיול גדלים: טקסט אינטראקטיבי ומטרות מגע (6 PRs) | **Header — קישורי ניווט** — דף הבית + דף פנימי, דסקטופ — **תוצאה מצופה:** קישורי הניווט וכניסה לחשבון ב-16px (… |
| 875 | MEH-1103 — כיול גדלים: טקסט אינטראקטיבי ומטרות מגע (6 PRs) | **אימות טלפון** — דשבורד בעלת עסק — **תוצאה מצופה:** כפתורי שליחת קוד ואישור נלחצים בקלות (44px). |
...
| 1040 | MEH-964 chunk 1A — producer dashboard nested-route shell | **שער הזדהות** — משתמשת לא-producer (או לא מחוברת) על כל `/producer/dashboard/*` → הפניה ל-`/login` |
| 1053 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **רצועת KPI (סקירה)** — 4 קלפים ב-2×2, סדר RTL ימין→שמאל: פניות בוואטסאפ → צרי קשר → דירוג → צפיות; **זהה בניי… |
| 1055 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **שורת המרה** — מתחת לרצועה, שקטה/מוצללת (לא קלף): "X% מהצופות פנו אלייך" (מונה = וואטסאפ בלבד) |
| 1057 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **תובנות** (`/producer/dashboard/insights`) — הטאב הרביעי מופיע; מציג את הקלפים החלוניים (צפיות/חיפושים/וואטסא… |
| 1060 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **/en** — תווית הטאב "Insights"; ה-KPI באנגלית (WhatsApp leads / Contact clicks / Rating / Views); אין מחרוזות… |
| 1068 | MEH-1134 — סקירה: סדר כרטיסים מותנה-מצב (completeness מעל av… | **מופע יחיד** — כרטיס ההשלמה מופיע פעם אחת בלבד בכל מצב; שאר הכרטיסים (אימות טלפון / לינק / באנרים) לא זזו |
| 1117 | MEH-1157 — טאב עריכה: 401 מפנה להתחברות + שגיאות ביו לפי סיב… | **סשן שפג → הפניה** — כניסה לטאב עריכה עם טוקן שפג (או מחיקת הטוקן ב-devtools ורענון) — **תוצאה מצופה:** הפניה… |
| 1118 | MEH-1157 — טאב עריכה: 401 מפנה להתחברות + שגיאות ביו לפי סיב… | **ביו — מגבלת קצב** — לחיצה על "יצירת ביו" 6 פעמים בשעה — **תוצאה מצופה:** "נשלחו יותר מדי בקשות. נסו שוב בעוד… |
| 1119 | MEH-1157 — טאב עריכה: 401 מפנה להתחברות + שגיאות ביו לפי סיב… | **ביו — AI לא זמין** — כשהשרת מחזיר ביו ריק (fail-open) — **תוצאה מצופה:** "השירות לא זמין כרגע. אפשר לכתוב תי… |
| 1120 | MEH-1157 — טאב עריכה: 401 מפנה להתחברות + שגיאות ביו לפי סיב… | **ביו — שגיאת רשת** — ניתוק רשת ולחיצה על "יצירת ביו" — **תוצאה מצופה:** השגיאה הכללית "שגיאה ביצירת הביו — נס… |
...
| 1273 | MEH-731 — navbar homepage-state (locale-path) + verify-banne… | עמוד פנימי (`/about`, `/map`) — cream pill כברירת מחדל — תוצאה: pill כהה-דיו |
| 1274 | MEH-731 — navbar homepage-state (locale-path) + verify-banne… | קישור `גלי` בעמוד הבית — קו תחתון זהב פעיל — תוצאה: underline מופיע (היה שבור) |
| 1275 | MEH-731 — navbar homepage-state (locale-path) + verify-banne… | BottomNav (מובייל) בעמוד הבית — tab הבית מודגש — תוצאה: highlight מופיע (היה שבור) |
| 1277 | MEH-731 — navbar homepage-state (locale-path) + verify-banne… | verify-banner בעמוד פנימי + בגלילה — עדיין מוצג — תוצאה: נוכח בכל עמוד |
| 1286 | Desktop (≥768px) | עמוד פנימי (`/about`, `/map`) — cream pill כברירת מחדל (לא שקוף) — תוצאה: pill כהה-דיו על cream |
| 1287 | Desktop (≥768px) | קישור פעיל = קו תחתון זהב (`גלי` ב-`/`, `מפה` ב-`/map`) — תוצאה: underline זהב בלבד |
| 1293 | Desktop (≥768px) | search icon + `/` shortcut + LanguageToggle עובדים — תוצאה: search נפתח, שפה מתחלפת |
| 1300 | Mobile (375px) | search button במובייל עובד — תוצאה: נפתח `/search` |
| 1314 | MEH-671 — Producer-signup smoke (now automated) | `POST /auth/register/producer` with a fresh `smoke+{id}@mehamakor.online` → **200** |
| 1315 | MEH-671 — Producer-signup smoke (now automated) | New row appears in `/admin/producers` (admin login) |
| 1316 | MEH-671 — Producer-signup smoke (now automated) | Railway log shows `[WHATSAPP] Producer welcome template sent` (not `… FAILED` / `… send failed`) |
| 1327 | MEH-1232 — Admin pending queue: photo thumbnails before appr… | **תמונות מרונדרות בשורה ממתינה** — פתחי את תור האישורים עם בית עסק בסטטוס `pending` שיש לו תמונות — **תוצאה מצ… |
| 1328 | MEH-1232 — Admin pending queue: photo thumbnails before appr… | **URL שבור מסומן ⚠** — בית עסק ממתין עם תמונה שבורה (למשל `https://bread.jpg`) — **תוצאה מצופה:** ה-thumbnail … |
| 1329 | MEH-1232 — Admin pending queue: photo thumbnails before appr… | **קליק פותח את התמונה המלאה** — לחצי על thumbnail תקין — **תוצאה מצופה:** התמונה המקורית (לא החתוכה) נפתחת בטא… |
| 1330 | MEH-1232 — Admin pending queue: photo thumbnails before appr… | **רק לממתינים** — בית עסק `approved` — **תוצאה מצופה:** אין רצועת thumbnails (הרצועה מיועדת לתור האישורים בלבד… |
| 1331 | MEH-1232 — Admin pending queue: photo thumbnails before appr… | **בלי תמונות = בלי רצועה** — בית עסק ממתין ללא תמונות — **תוצאה מצופה:** אין רצועה ריקה (שער התמונה של MEH-799… |
| 1341 | Test 1 — Admin blocked on password upgrade path | Log in as admin (`sint12345@gmail.com` or any account with `role='admin'`) |
| 1342 | Test 1 — Admin blocked on password upgrade path | Type `/register/producer` into the address bar → **expected:** automatic redirect to `/admin` (no form ever re… |
| 1343 | Test 1 — Admin blocked on password upgrade path | In DevTools console, send a direct POST: `fetch('/auth/register/producer', {method:'POST', headers:{'Content-T… |
| 1344 | Test 1 — Admin blocked on password upgrade path | After both checks: refresh `/admin` → **expected:** admin dashboard loads normally; role still `"admin"` in `/… |
| 1348 | Test 2 — Admin blocked on OAuth Step 0 | (Only if admin has linked Google) Click "Sign up as producer with Google" from `/register/producer` step 0 → *… |
| 1353 | Test 3 — Regression: consumer can still upgrade to producer | Navigate to `/register/producer` → **expected:** form renders normally, no redirect |
...
| 2532 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Footer** — price truncates at `max-w-[120px]`; primary-method icon switches per `primary_contact_method` (wh… |
...
| 2608 | Chip appearance (both pages) | `/map` — below the city search, same 4 chips |
...
| 2695 | Login page — eye toggle | Toggle shows/hides password |
| 2698 | Modal close buttons (start-3) | Click favorite when logged out → LoginPromptModal opens → ✕ close button appears on the **top-right** of the m… |
...
| 2801 | Google OAuth / CSP (fix #173, 2026-04-19) | /login — click "כניסה עם Google" → Google popup opens and completes without postMessage error |
| 2802 | Google OAuth / CSP (fix #173, 2026-04-19) | /login — Network tab → `accounts.google.com/gsi/style` loads with status 200 (not blocked) |
| 2808 | MEH-287 — Producer registration WhatsApp welcome | `/register/producer` → submit הרשמה תקינה עם טלפון אמיתי → תוך 60 שניות מתקבלת הודעת WhatsApp "ברוכה הבאה למהמ… |
| 2809 | MEH-287 — Producer registration WhatsApp welcome | Response של `POST /auth/register/producer` בDevTools → Network → מכיל `whatsapp_sent: true` (כשTwilio מוגדר) א… |
| 2810 | MEH-287 — Producer registration WhatsApp welcome | Staging ללא `TWILIO_WHATSAPP_FROM` מוגדר → הרשמה חוזרת 200 + `whatsapp_sent: false` → success screen מציג bann… |
| 2811 | MEH-287 — Producer registration WhatsApp welcome | Railway logs בזמן ההרשמה ה-"חסרה" → `[WHATSAPP] Producer welcome SKIPPED — missing: TWILIO_WHATSAPP_FROM` ברמת… |
| 2812 | MEH-287 — Producer registration WhatsApp welcome | הרשמה עם `whatsapp_sent: true` → success screen מציג את הטקסט המקורי "שלחנו לך הודעת WhatsApp..." ללא banner |
| 2819 | Case A — Silent refresh on access expiry | Login on staging, note timestamp |
| 2822 | Case A — Silent refresh on access expiry | EXPECT: Action succeeds. No "פג תוקף ההתחברות" toast. |
| 2826 | Case B — Forced logout when refresh expired / missing | Login on staging |
| 2830 | Case B — Forced logout when refresh expired / missing | EXPECT: "פג תוקף ההתחברות" toast appears. Page redirects to `/login`. |
| 2833 | Case C — logout-all-devices stays authenticated on current d… | Login on staging in browser A; login on staging in browser B (same account) |
| 2837 | Case C — logout-all-devices stays authenticated on current d… | EXPECT browser B: receives 401 on next refresh attempt → "פג תוקף" toast + redirect to `/login` |
| 2852 | ProducerCard badge dot | /map or /producers list, look at a producer with `availability_state='available_today'` — תוצאה מצופה: green d… |
...
| 3160 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **סדר בכרטיס העסק** — אחרי רישום עם ב→א→ג, עמוד העסק + כרטיס העסק + סיכה במפה מציגים **ב** ראשונה |
| 3161 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **יציבות אחרי רענון** — מרעננות את עמוד העסק פעמיים — **תוצאה מצופה:** אותה קטגוריה ראשית (ב), לא אקראי |
| 3172 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **שני השדות** — ממלאות עלות 35 וסף 250, שומרות, פותחות את עמוד העסק — **תוצאה מצופה:** שורה אחת "משלוח: 35₪ · … |
| 3177 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **שני השדות ריקים** — מנקות את שניהם ושומרות — **תוצאה מצופה:** אין שורת עלות בכלל בעמוד העסק (לא שורה ריקה, ל… |
| 3196 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **סטטוס בעמוד העסק בין טווחים** — לעסק עם 09:00–13:00 ו-16:00–20:00, פתחי את עמוד העסק ב-14:30 — **תוצאה מצופה… |
| 3204 | MEH-1871 — פאנל overlay נסגר בגלילה (03/08) | **בועת תג נסגרת בגלילה** — דף הבית / `/producers` → הקישי על תג בכרטיס עסק (למשל "מהדרין" או "גראס פד") → גללי… |
| 3217 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **עמוד העסק** — פתחי את עמוד העסק של אותו עסק → פתחי את רצועת השעות — **תוצאה מצופה:** בשורת שישי מופיע "09:00… |
| 3230 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **פתוח עכשיו** — עסק שהגדיר חלון שכולל את השעה הנוכחית → דף הבית / `/producers` — **תוצאה מצופה:** מתחת לשורת … |
| 3253 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **הטקסט בעמוד העסק** — אחרי שמירה → עמוד העסק — **תוצאה מצופה:** התגית מציגה את המילים שלך **פעם אחת**, עם איי… |
| 3254 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **הצ'יפ בכרטיסייה** — דף הבית / `/producers` — **תוצאה מצופה:** אותן מילים בצ'יפ הקצר (נחתך ב-`truncate` אם אר… |
| 3294 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **עסק עם ערוץ "אתר"** — עמוד עסק → הקישי על מוצר — **תוצאה מצופה:** הכפתור הראשי הוא "להזמנה באתר" עם אייקון ג… |
| 3296 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **הוואטסאפ המשני שומר על שם המוצר** — הקישי על הקישור השקט — **תוצאה מצופה:** וואטסאפ נפתח עם ההודעה שמזכירה א… |
| 3297 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **עסק עם ערוץ וואטסאפ — ללא שינוי** — מוצר של עסק שערוץ הקשר שלו וואטסאפ — **תוצאה מצופה:** בדיוק כמו קודם — כ… |
| 3298 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **עסק עם ערוץ "טלפון"** — **תוצאה מצופה:** כפתור "התקשרו" שפותח חייגן (אותה לשונית), ומתחתיו הקישור השקט לוואט… |
| 3299 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **עסק עם אתר ובלי טלפון** — **תוצאה מצופה:** "להזמנה באתר" לבד. אין קישור וואטסאפ בשום מקום בגיליון (זה ה-dead… |
| 3317 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **הקישור מופיע** — סטייג'ינג עם ה-seed (3 חוויות) → דף הבית בדסקטופ — **תוצאה מצופה:** "חוויות" בשורת הניווט ה… |
| 3335 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | **פתחו את עמוד העסק הציבורי — השם עדיין הישן.** זו הבדיקה המרכזית |
| 3375 | מד ההשלמה — שש שורות, לא חמש (16/08) | הצ'קליסט מציג **שש** שורות: תמונה ראשית · קטגוריות ומיקום · מוצר ראשון · פרטי קשר · **אימות וואטסאפ** · שעות פ… |
| 3376 | מד ההשלמה — שש שורות, לא חמש (16/08) | «אימות וואטסאפ» נושא צ'יפ **חובה**; «שעות פתיחה» היא היחידה עם **מומלץ** |
| 3377 | מד ההשלמה — שש שורות, לא חמש (16/08) | לחיצה על «אימות וואטסאפ» מקפיצה לכרטיס האימות בבאנר (עוגן `#phone-verify`) — **לא** לעמוד העריכה, שאין בו פקד … |
| 3378 | מד ההשלמה — שש שורות, לא חמש (16/08) | **התא ששבר את הגרסה הקודמת:** עסק עם טלפון שמור אבל **בלי** אימות — «פרטי קשר» מסומן ✓ **ובו-זמנית** «אימות וו… |
| 3384 | אימות הוואטסאפ מתוך הטיוטה — הבדיקה הקריטית ביותר ברשימה | אחרי אימות מוצלח — הכרטיס נעלם, ופריט «אימות וואטסאפ» יורד מרשימת החסרים |
| 3385 | אימות הוואטסאפ מתוך הטיוטה — הבדיקה הקריטית ביותר ברשימה | אם כל השאר הושלם — הכפתור נעשה פעיל באותו מסך, בלי רענון ידני |
...
| 431 | MEH-1396 — צ'קליסט בדיקה לפני אישור בית עסק (אדמין, Phase 1) | **ה-gates הקשיחים לא נשברו** — נסי לאשר עסק בלי תמונה / בלי מספר רישיון בקטגוריה שדורשת — **תוצאה מצופה:** התנ… |
| 439 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **פופאובר מאומתת** — הקישי על תג ✓מאומתת — **תוצאה מצופה:** במובייל bottom-sheet (Esc/רקע סוגרים, הפוקוס חוזר … |
| 521 | MEH-1164 Chunk 2A — אכיפת אימות מייל לפני פרסום תוכן (17/07) | יצירת אירוע — `POST /events` כבעלת-עסק לא-מאומתת — **403** עם `{"detail":"יש לאמת את כתובת האימייל תחילה"}`; ל… |
| 522 | MEH-1164 Chunk 2A — אכיפת אימות מייל לפני פרסום תוכן (17/07) | יצירת מתכון — `POST /producers/me/recipes` כבעלת-עסק לא-מאומתת — **403** עם אותו detail; לאחר אימות → נוצר. |
| 523 | MEH-1164 Chunk 2A — אכיפת אימות מייל לפני פרסום תוכן (17/07) | יצירת קבוצת רכש — `POST /group-buys` כבעלת-עסק לא-מאומתת — **403** עם אותו detail; לאחר אימות → נוצר. |
| 524 | MEH-1164 Chunk 2A — אכיפת אימות מייל לפני פרסום תוכן (17/07) | קדימות תפקיד — משתמשת רגילה (consumer) מאומתת על `POST /events` — **403 "Producer access required"** (בדיקת הת… |
...
| 662 | MEH-1258 — כרטיס "רישיון יצרן" בטאב עריכה (17/07) | **רמז קטגוריה דורשת-רישיון** — עסק עם קטגוריה מ-LICENSE_REQUIRED (למשל דבש/לחמים) — **תוצאה מצופה:** שורת רמז … |
| 663 | MEH-1258 — כרטיס "רישיון יצרן" בטאב עריכה (17/07) | **ניקוי בזמן קטגוריה דורשת → 422 עברית** — עסק מאושר עם רישיון + קטגוריה דורשת → רוקני את השדה → שמירה — **תוצ… |
| 664 | MEH-1258 — כרטיס "רישיון יצרן" בטאב עריכה (17/07) | **deep-link מהבאנר** — נווטי אל `/producer/dashboard/edit#license` — **תוצאה מצופה:** אקורדיון הרישיון נפתח וג… |
| 670 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **הכרטיס מופיע** — `/producer/dashboard/edit` → כרטיס "תעודת כשרות" (אחרי "רישיון יצרן") → פותחת אותו — **תוצא… |
| 671 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **שליחת בקשה** — בחרי סוג כשרות → (אופ') העלי צילום תעודה → "שליחת בקשה לאישור" — **תוצאה מצופה:** הודעת הצלחה… |
| 672 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **בקשה כפולה → 409 inline** — שלחי שוב בקשה לאותו סוג כשרות שכבר pending — **תוצאה מצופה:** הודעת השגיאה העברי… |
| 673 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **תעודה לא-תמונה נדחית** — נסי להעלות PDF — **תוצאה מצופה:** "רק תמונות JPG/PNG/WebP/GIF/HEIC מותרות". |
| 674 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **גלריה מלאה לא חוסמת תעודה** — עסק בתוכנית free עם 3 תמונות גלריה — **תוצאה מצופה:** העלאת צילום התעודה מצליח… |
| 675 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **מסלול מלא (עם אדמין)** — שלחי בקשה → `/admin/kashrut` approve → חזרה לכרטיס — **תוצאה מצופה:** ה-badge עובר … |
| 676 | MEH-1167 — כרטיס "תעודת כשרות" בטאב עריכה (21/07) | **דחייה מציגה notes** — אדמין reject עם הערה — **תוצאה מצופה:** צ'יפ "נדחתה" + הערת האדמין מוצגת ליד הבקשה. |
| 809 | map-quality batch PR 1 — כרטיס עסק אחיד ב-/map | **שורת קטגוריה משולבת** — עסק מאומת עם דירוג — **תוצאה מצופה:** צ'יפ קטגוריה + כוכב עם "4.8 (12)" + חותם אימות… |
...
| 3428 | טופס האדמין | `/admin/producers` → עריכת עסק → השדה «תוקף רישיון (מהמסמך)» מופיע ליד מספר הרישיון — איך לבדוק: פותחים עסק עם… |
| 3429 | טופס האדמין | לעסק **בלי** רישיון ובלי תאריך — השדה מוסתר מאחורי «יש לי רישיון יצרן ↓» — תוצאה מצופה: לחיצה על ה-toggle חושפ… |
| 3433 | טופס האדמין | הזנת תאריך **בעבר** — תוצאה מצופה: **נשמר בהצלחה**. זה מכוון: אדמין שסוקרת עסק שהרישיון שלו כבר פג חייבת יכולת… |
...
| 391 | MEH-2245 — חוויות route-as-tab: `/events` + `/experiences` =… | **`/experiences`** — פתחי בנייד — **תוצאה מצופה:** אותו עמוד כמו `/events` עם טאב «חוויות וסדנאות» פעיל, hero … |
| 392 | MEH-2245 — חוויות route-as-tab: `/events` + `/experiences` =… | **לחיצה על טאב** — מ-`/events` לחצי «חוויות וסדנאות» — **תוצאה מצופה:** ה-URL מתחלף ל-`/experiences` (ב-`/en` … |
| 393 | MEH-2245 — חוויות route-as-tab: `/events` + `/experiences` =… | **`?tab=` לא נכתב לעולם** — סנני לפי עיר בכל אחד מהטאבים — **תוצאה מצופה:** ה-URL נושא רק `?city=` / `?categor… |
| 394 | MEH-2245 — חוויות route-as-tab: `/events` + `/experiences` =… | **deep-link ישן** — `/events?tab=experiences` (וגם `/he/…`, `/en/…`) — **תוצאה מצופה:** 308 → `/experiences`; … |
| 395 | MEH-2245 — חוויות route-as-tab: `/events` + `/experiences` =… | **שני הטאבים תמיד** — גם כשאין נתונים באחד מהם — **תוצאה מצופה:** שני הטאבים מרונדרים בשני ה-routes (MEH-1865 … |
| 399 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **רשימת האירועים שלי** — היכנסי כבעלת-עסק → `/producer/dashboard/tools` → כרטיס "האירועים שלי" (או ישירות `/pr… |
| 400 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **עריכת אירוע** — הקישי "עריכה" על אירוע — **תוצאה מצופה:** טופס זהה לטופס היצירה, מלא מראש (כותרת/תיאור/תאריך… |
| 401 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **ביטול/הפעלה (is_active)** — הקישי "ביטול" על אירוע פעיל — **תוצאה מצופה:** התג הופך ל"מבוטל" מיד (אופטימי), … |
| 402 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **מחיקת אירוע** — הקישי "מחיקה" → אשרי בדיאלוג — **תוצאה מצופה:** השורה נעלמת; ביטול הדיאלוג משאיר אותה. |
| 403 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **רשימת החוויות שלי** — `/producer/dashboard/experiences` — **תוצאה מצופה:** רשימת החוויות שלך עם תג סטטוס (מא… |
| 404 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **עריכת חוויה** — "עריכה" על חוויה — **תוצאה מצופה:** טופס היצירה מלא מראש (כולל שדה כתובת + בדיקת מודרציה חיה… |
| 405 | MEH-1405 — ניהול אירועים וחוויות בדשבורד (רשימת "שלי" + עריכ… | **נגישות שדה כתובת** — בטופס אירוע/חוויה, שדה "מיקום ספציפי"/"כתובת" — **תוצאה מצופה:** תווית אחת בלבד (בלי הכ… |
| 408 | MEH-1401 — מייל "ברוכה הבאה" ב-RTL HTML (21/07) | **BiDi תקין בג'ימייל** — הירשמי עם מייל בדיקה (צרכנית) ב-staging → פתחי את מייל ה-welcome בג'ימייל נייד — **תו… |
| 409 | MEH-1401 — מייל "ברוכה הבאה" ב-RTL HTML (21/07) | **ברכה בשם פרטי** — שם מלא ברישום ("Smadar Levi") — **תוצאה מצופה:** הברכה "שלום Smadar," (שם פרטי בלבד, ללא ש… |
| 410 | MEH-1401 — מייל "ברוכה הבאה" ב-RTL HTML (21/07) | **וריאנט יצרנית** — הרשמת עסק → מייל welcome-producer — **תוצאה מצופה:** אותה תבנית RTL, קופי היצרנית, כפתור "… |
| 414 | MEH-1355 — CTA "עריכת דף העסק" בראש טאב "העסק שלי" (21/07) | **CTA ראשי בראש הטאב** — כבעלת-עסק → `/settings` → טאב "העסק שלי" @375 + @1440 — **תוצאה מצופה:** כפתור ראשי י… |
| 415 | MEH-1355 — CTA "עריכת דף העסק" בראש טאב "העסק שלי" (21/07) | **באנר סטטוס נשמר מתחת** — עסק בהמתנה/דחייה/השעיה — **תוצאה מצופה:** ה-CTA למעלה, באנר הסטטוס (וכפתור התמיכה) … |
| 416 | MEH-1355 — CTA "עריכת דף העסק" בראש טאב "העסק שלי" (21/07) | **אין קישור כפול** — **תוצאה מצופה:** הקישור הישן "ערכו פרופיל עסק ←" הוסר; יש מסלול ניווט אחד ברור. |
| 420 | MEH-1403 — כפתור פריסט שעות = טוגל דו-כיווני מתויג | **פריסט → ניקוי** — היכנסי כבעלת-עסק → `/producer/dashboard/edit` → כרטיס "שעות פתיחה" → הקישי על "א׳–ה׳ 9:00–… |
| 421 | MEH-1403 — כפתור פריסט שעות = טוגל דו-כיווני מתויג | **ניקוי → פריסט (חזרה)** — הקישי על "ניקוי השעות" — **תוצאה מצופה:** כל 7 הימים נסגרים ("סגור"); התווית חוזרת … |
| 422 | MEH-1403 — כפתור פריסט שעות = טוגל דו-כיווני מתויג | **שעות נשמרות בניקוי** — פרסי פריסט → נקי → פתחי יום כלשהו מחדש (checkbox) — **תוצאה מצופה:** שעות הפתיחה/סגיר… |
| 428 | MEH-1396 — צ'קליסט בדיקה לפני אישור בית עסק (אדמין, Phase 1) | **סימון + reset בסגירה** — סמני כמה סעיפים (הסעיף מסומן + קו חוצה, המונה עולה) → סגרי את הצ'קליסט → פתחי שוב —… |
| 429 | MEH-1396 — צ'קליסט בדיקה לפני אישור בית עסק (אדמין, Phase 1) | **אזהרה רכה באישור עם סעיפים לא מסומנים** — השאירי לפחות סעיף אחד לא מסומן והקישי "אשר" — **תוצאה מצופה:** דיא… |
| 430 | MEH-1396 — צ'קליסט בדיקה לפני אישור בית עסק (אדמין, Phase 1) | **כל הסעיפים מסומנים → אין דיאלוג** — סמני את כל 7 הסעיפים והקישי "אשר" — **תוצאה מצופה:** האישור ממשיך ישירות… |
| 435 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **סטטוס 3 מצבים בשורת המטא** — פתחי עמוד עסק פתוח / עמוס-השבוע / בחופשה — **תוצאה מצופה:** "פתוח להזמנות" ירוק… |
| 436 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **"חדש" באפס ביקורות** — עסק בלי ביקורות — **תוצאה מצופה:** "חדש" בסלוט הדירוג, בלי כוכב, בלי ציטוט ביקורת. |
| 438 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **שיתוף במובייל = בהירו בלבד** — במובייל: ⤴ עיגול על ההירו, אין שיתוף בשורת הפעולות; בדסקטופ: שיתוף בשורת הפעו… |
| 440 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **שאלות נפוצות — עוד שאלות** — עסק עם 4+ שאלות — **תוצאה מצופה:** 3 גלויות + "עוד שאלות" שפותח את השאר; התשובו… |
| 442 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **תשובה בעמוד אינה נספרת (MEH-1886)** — באותו עסק הקישי על **"אפשר משלוח ל…"/"איך מזמינים?"** כשהן נפתחות כתשו… |
| 443 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **חשיפת טלפון בדסקטופ** — בדסקטופ הקישי על אייקון הטלפון — **תוצאה מצופה:** המספר נחשף inline (ספרות משמאל לימ… |
| 444 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **שעות פעילות** — בסקשן "הגעה ומיקום": שורה מקופלת "היום · 9:00–17:00" (או "היום · סגור") ניטרלית בלי ירוק; הק… |
| 445 | MEH-1334 — ProducerDetail Quiet Direction v3 (18/07) | **כפתורי ניווט** — Waze + מפות Google עם לוגו מותג אמיתי; הקישורים פותחים ניווט ליעד. |
| 461 | MEH-1682 — tooltip של פין במיני-מפה צמוד לפין ב-RTL (27/07) | **פין ראשי (32px)** — בדסקטופ, בסקשן "הגעה ומיקום", העבירי עכבר מעל הפין המלא (הסניף) — **תוצאה מצופה:** ה-too… |
| 462 | MEH-1682 — tooltip של פין במיני-מפה צמוד לפין ב-RTL (27/07) | **פין משני (24px)** — אותו דבר מעל פין חלול (איסוף / דוכן) — **תוצאה מצופה:** אותה התנהגות; ה-tooltip צמוד מעל… |
...
| 622 | MEH-1255 — משלוחים לכל הארץ חוץ מ־ (exclusion mode) (17/07) | **כיבוי "לכל הארץ" מנקה החרגות** — עסק עם החרגות → כבי "משלוחים לכל הארץ" → שמירה — **תוצאה מצופה:** ההחרגות נ… |
| 624 | MEH-1255 — משלוחים לכל הארץ חוץ מ־ (exclusion mode) (17/07) | **אותו pattern באדמין** — טופס עסק באדמין → "משלוחים לכל הארץ" → שדה "חוץ מ:" נחשף ונשמר. |
| 632 | MEH-1256 — בחירה מהירה לפי אזור בשדה ערי משלוח (17/07) | **dedupe** — הוסיפי ידנית "נתניה", ואז לחצי "השרון" — **תוצאה מצופה:** "נתניה" מופיעה פעם אחת בלבד. |
| 633 | MEH-1256 — בחירה מהירה לפי אזור בשדה ערי משלוח (17/07) | **אזור שכולו נבחר → מושבת** — אחרי הוספת כל ערי אזור — **תוצאה מצופה:** ה-chip מציג "· נוסף" ולא לחיץ. |
| 634 | MEH-1256 — בחירה מהירה לפי אזור בשדה ערי משלוח (17/07) | **גם באדמין** — טופס עסק באדמין → "משלוחים" בלי "לכל הארץ" — **תוצאה מצופה:** אותה שורת chips ואותה התנהגות. |
| 643 | MEH-1259 — הסתרת badge "אורגני" מכל משטח ציבורי (17/07) | **הפילטר לא מסנן** — `GET /producers?organic=true` (ידני/URL) — **תוצאה מצופה:** מוחזרים גם עסקים לא-אורגניים … |
| 644 | MEH-1259 — הסתרת badge "אורגני" מכל משטח ציבורי (17/07) | **הטוגל של בעלת העסק נשמר** — דשבורד → סמני/בטלי "אורגני" → שמירה → רענון — **תוצאה מצופה:** הערך נשמר (הדאטה … |
| 645 | MEH-1259 — הסתרת badge "אורגני" מכל משטח ציבורי (17/07) | **צ'ק-בוקס אדמין עובד** — טופס אדמין → "אורגני" — **תוצאה מצופה:** ניתן לסמן/לבטל ולשמור כרגיל. |
| 652 | MEH-1471 — שדה "מאיפה שמעת עלינו?" בהרשמת בית עסק (22/07) | **submit חסום כשריק** — השאירי "בחר אפשרות", סמני את התנאים וההצהרה, לחצי "הצטרפי" — **תוצאה מצופה:** הודעת שג… |
| 653 | MEH-1471 — שדה "מאיפה שמעת עלינו?" בהרשמת בית עסק (22/07) | **"אחר" חושף שדה חופשי** — בחרי "אחר" — **תוצאה מצופה:** נפתח input "ספרי לנו איפה" (עד 120 תווים); מעבר לאופצ… |
| 682 | MEH sweep 16/07 — דשבורד בעלת עסק (1234/1236/1237/1238/1239) | **העלאת תמונה נשמרת בלי "שמור" (MEH-1236)** — עריכה → תמונות → העלי תמונה → חזרי לסקירה — **תוצאה מצופה:** צעד… |
| 684 | MEH sweep 16/07 — דשבורד בעלת עסק (1234/1236/1237/1238/1239) | **באנר שינויים שלא נשמרו מפרט כרטיסים (MEH-1237)** — עריכה → שני שינויים בשני כרטיסים בלי לשמור — **תוצאה מצופ… |
| 685 | MEH sweep 16/07 — דשבורד בעלת עסק (1234/1236/1237/1238/1239) | **עסק עם מוצר אחד מגיע ל-100% (MEH-1238)** — פרופיל מלא + מוצר אחד בקטלוג — **תוצאה מצופה:** הצ'קליסט 4/4 = "ה… |
| 686 | MEH sweep 16/07 — דשבורד בעלת עסק (1234/1236/1237/1238/1239) | **טופס מוצר חדש עם הכוונה (MEH-1239)** — עריכה → מוצרים → "הוסיפו מוצר" — **תוצאה מצופה:** placeholders לדוגמה… |
| 694 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **URL שבור נופל בחן ל-placeholder** — עסק/מוצר עם `image_url` לא-תקין (למשל `https://bread.jpg`) — **תוצאה מצו… |
| 695 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **HomeFeaturedProducer ללא רגרסיה** — `/` בלוק "הכירו בית עסק" (אם מוגדר featured) — **תוצאה מצופה:** תמונת פו… |
| 696 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **רצועת "נצפו לאחרונה" אחידה** — `/`, רצועת recently-viewed — **תוצאה מצופה:** כל התמונות בקרופ אחיד (16:10), … |
| 697 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **גלריית עמוד עסק ללא שינוי** — `/producer/[id]` עם תמונות — **תוצאה מצופה:** ה-hero/grid + ה-lightbox מציגים … |
| 699 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **מוצרים ואירועים בעמוד עסק** — `/producer/[id]` עם מוצרים + אירועים עם תמונות — **תוצאה מצופה:** thumbnails ר… |
| 700 | MEH-1229 — עקביות תמונות בתי-עסק (optimizeCloudinary + מפת י… | **רצועת "משלוח שישי" (FridayDeliveryStrip)** — `/` ביום עם עסקים במשלוח שישי — **תוצאה מצופה:** תמונות המיני-כ… |
| 716 | MEH-1479 — כפתור "נסו שוב" במצב שגיאה של /favorites (22/07) | **שגיאת טעינת מועדפים → כפתור retry** — כלקוחה מחוברת, פתחי `/favorites` וחסמי את `**/api/users/me/favorites` … |
| 750 | MEH-1186 — היררכיה ויזואלית ב-/producers (שפה אחת לכל התנהגו… | **תוויות מיקרו מעל שורות הצ'יפים** — פתחי `/producers` — **תוצאה מצופה:** מעל שורת הקטגוריות מופיעה התווית "קט… |
| 753 | MEH-1186 — היררכיה ויזואלית ב-/producers (שפה אחת לכל התנהגו… | **מצב אפס תוצאות — יש דרך יציאה** — סנני עד שאין תוצאות — **תוצאה מצופה:** הצ'יפים הניתנים להסרה + "נקו הכל" ע… |
| 762 | MEH-1173 — כרטיס "תיאור העסק" (עוזר AI בתוך השדה + משפט תדמי… | **כפתור מושבת עם סיבה** — לחצי "צרו תיאור עם AI" ואל תמלאי כלום — **תוצאה מצופה:** נפתח טופס 3 שאלות; כפתור "צ… |
| 763 | MEH-1173 — כרטיס "תיאור העסק" (עוזר AI בתוך השדה + משפט תדמי… | **יצירה מוצלחת** — מלאי את השאלות ולחצי "צרו תיאור" — **תוצאה מצופה:** התיאור נוחת בשדה הגיבור עם הדגשה חולפת … |
| 764 | MEH-1173 — כרטיס "תיאור העסק" (עוזר AI בתוך השדה + משפט תדמי… | **תוצאה ריקה לא מוחקת** — בעסק עם תיאור קיים, כשה-AI מחזיר ריק (fail-open) — **תוצאה מצופה:** הטקסט הקיים בשדה… |
...
| 864 | MEH-1085 — empty state מודע-סיבה בבית + פילטרי /events ב-URL | **deep-link /events** — פתחי `/events?city=חיפה&category=קטיף` בטאב חדש — **תוצאה מצופה:** שדה העיר מלא, הצ'יפ… |
| 865 | MEH-1085 — empty state מודע-סיבה בבית + פילטרי /events ב-URL | **החלפת טאב מנקה גם את ה-URL** — עם עיר+קטגוריה פעילות, עברי ל"חוויות וסדנאות" — **תוצאה מצופה:** ה-URL מתנקה … |
| 872 | MEH-1103 — כיול גדלים: טקסט אינטראקטיבי ומטרות מגע (6 PRs) | **Footer — קישורי השורה התחתונה** — כל דף, מובייל — **תוצאה מצופה:** כניסה/תנאים/פרטיות/נגישות קריאים (13px) ו… |
| 873 | MEH-1103 — כיול גדלים: טקסט אינטראקטיבי ומטרות מגע (6 PRs) | **באנר חג — CTA וכפתור סגירה** — בתקופת חג — **תוצאה מצופה:** ה-CTA קריא (14px); ה-× נסגר בלחיצה ראשונה גם באצ… |
| 874 | MEH-1103 — כיול גדלים: טקסט אינטראקטיבי ומטרות מגע (6 PRs) | **ביקורות — כפתורי פעולה** — עמוד עסק, טופס ביקורת + תגובת בעלת עסק — **תוצאה מצופה:** שליחה/ביטול/שמירה/הוספת… |
| 882 | MEH-1048 — עמוד עסק: trust strip ליד ה-h1 (דירוג + מספר ביקו… | **דירוג + מספר ביקורות ליד השם** — עסק עם ביקורות — **תוצאה מצופה:** צ'יפ ירוק "★ 4.8 · N ביקורות" ליד ה-h1 (ה… |
...
| 895 | MEH-1490 — שורת "דירוג ב-Google" שקטה (live-fetch בלבד) | **עסק לא ממופה** — עסק ללא `google_place_id` — **תוצאה מצופה:** אין שורה, ו-**אין קריאת רשת** ל-`/google-ratin… |
| 896 | MEH-1490 — שורת "דירוג ב-Google" שקטה (live-fetch בלבד) | **פחות מ-20 ביקורות / שגיאת API / אין מפתח** — **תוצאה מצופה:** אין שורה (204), אין חור בלייאאוט, אין שגיאה בק… |
| 897 | MEH-1490 — שורת "דירוג ב-Google" שקטה (live-fetch בלבד) | **RTL** — 375px — **תוצאה מצופה:** הכוכב + הטקסט + אייקון הלינק מיושרים נכון ב-RTL; "ביקורות ב-Google Maps" קר… |
| 903 | MEH-1146 — עמוד עסק: לולאת גילוי "עוד בתי עסק באזור" (Chunk … | **לולאת גילוי מופיעה עם ≥4** — עסק בעיר עם 4+ עסקים אחרים — **תוצאה מצופה:** בתחתית העמוד סקציית "עוד בתי עסק … |
...
| 917 | MEH-1146 — עמוד עסק: כותרת דו-שכבתית + סדר סקציות (Chunk B) | **איסוף עצמי** — עסק עם pickup_points=true מול false — **תוצאה מצופה:** שורת "📦 איסוף עצמי" מופיעה רק כשהדגל ד… |
| 924 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **חיצים ומונה** — עסק עם 3+ מוצרים → פתחי מוצר — **תוצאה מצופה:** שני חיצים עגולים לבנים על התמונה, אחד בכל קצ… |
| 925 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **הריבוע זהה בין מוצרים** — עסק שבו למוצרים תמונות בפרופורציות שונות (לאורך / לרוחב / ריבועית) → דפדפי ביניהם … |
| 926 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **אין מעגליות** — במוצר הראשון — **תוצאה מצופה:** חץ "הקודם" מעומעם ולחיצה עליו לא עושה כלום; במוצר האחרון אות… |
| 927 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **מקלדת (RTL)** — גיליון פתוח — **תוצאה מצופה:** ‏`←` מתקדם למוצר הבא, ‏`→` חוזר לקודם, ‏`Esc` סוגר. הפוקוס נש… |
| 928 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **גלילה מתאפסת** — מוצר עם תיאור ארוך → גללי לתחתית התיאור → לחצי "הבא" — **תוצאה מצופה:** הגיליון החדש נפתח *… |
| 929 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **מוצר בודד** — עסק עם מוצר אחד בלבד — **תוצאה מצופה:** **אין** חיצים ו**אין** מונה; הגיליון נראה בדיוק כמו קו… |
| 930 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **מוצר בלי תמונה באמצע הרשימה** — דפדפי אל מוצר ללא תמונה — **תוצאה מצופה:** הפס הקצר (h-28) נשמר; החיצים יורד… |
| 932 | MEH-2045 — ProductSheet: ניווט קודם/הבא בין מוצרים | **פתיחה מהמוצר החתום** — עסק עם top_product_name → לחצי על כרטיס המוצר החתום — **תוצאה מצופה:** המונה מראה את … |
| 949 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **דסקטופ 4+ תמונות** — עסק עם 4+ תמונות (דסקטופ) — **תוצאה מצופה:** grid — תמונת hero בצד inline-start (~62%) … |
| 950 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **דסקטופ 3 תמונות** — עסק עם 3 תמונות — **תוצאה מצופה:** אותו grid (hero + 2 מוערמות), צ'יפ "כל התמונות (3)". |
| 951 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **דסקטופ 2 תמונות** — עסק עם 2 תמונות — **תוצאה מצופה:** hero + תמונת ליווי יחידה גבוהה, **בלי** צ'יפ. |
| 952 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **תמונה אחת** — עסק עם תמונה יחידה — **תוצאה מצופה:** באנר מלא-רוחב כמו קודם (בלי grid, בלי צ'יפ, בלי מונה). |
| 953 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **"כל התמונות" פותח lightbox** — לחצי על הצ'יפ או על כל תא — **תוצאה מצופה:** נפתח ה-lightbox הקיים בתמונה המת… |
| 954 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **מובייל 375 — מונה + פס התקדמות** — עסק עם 2+ תמונות (מובייל) — **תוצאה מצופה:** תמונה יחידה בהחלקה (swipe); … |
| 955 | MEH-1047 — עמוד עסק: גלריית hero חדשה (grid, מצב עם תמונות) | **כפתור מועדפים בדסקטופ** — עסק עם 2+ תמונות (דסקטופ) — **תוצאה מצופה:** הלב מופיע ב-top-start מעל ה-hero (reg… |
| 959 | MEH-1045 — bot hardening: fast-404 ל-catch-all + robots.txt … | **עמוד עסק אמיתי נטען** — פתחו עמוד slug של עסק קיים (מובייל) — **תוצאה מצופה:** העמוד נטען רגיל, ללא שינוי. |
| 960 | MEH-1045 — bot hardening: fast-404 ל-catch-all + robots.txt … | **נתיב סורק לא מגיע ל-backend** — פתחו `/wp-admin` — **תוצאה מצופה:** עמוד "לא נמצא" מיידי (הערה: הסטטוס הוא 2… |
| 961 | MEH-1045 — bot hardening: fast-404 ל-catch-all + robots.txt … | **localeDetection כבוי** — דפדפן באנגלית (Accept-Language: en) פותח `/` — **תוצאה מצופה:** נחיתה על העברית (בר… |
| 962 | MEH-1045 — bot hardening: fast-404 ל-catch-all + robots.txt … | **robots.txt** — פתחו `/robots.txt` — **תוצאה מצופה:** שורת Sitemap מצביעה על `https://mehamakor.online/sitema… |
...
| 1043 | MEH-964 chunk 1A — producer dashboard nested-route shell | **עריכה** (`/producer/dashboard/edit`) — 3 טפסי העריכה (ביו AI / שאלות מותאמות / ערוצי קשר) עובדים זהה לקודם (… |
| 1044 | MEH-964 chunk 1A — producer dashboard nested-route shell | **כלים** (`/producer/dashboard/tools`) — גריד הקישורים המהירים; "הוסיפי אירוע" → `/producer/dashboard/events/n… |
| 1045 | MEH-964 chunk 1A — producer dashboard nested-route shell | **תובנות** — הטאב **לא** מופיע עדיין ב-1A (נוסף ב-1B); אין טאב מת / "בקרוב" |
| 1046 | MEH-964 chunk 1A — producer dashboard nested-route shell | **נייד (375px)** — שורת הטאבים נקראת ללא horizontal scroll; כל טאב נפתח תקין |
| 1047 | MEH-964 chunk 1A — producer dashboard nested-route shell | **/en** — תוויות הטאבים באנגלית (Overview / Edit / Tools); אין מחרוזות מפתח גולמיות |
| 1054 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **ללא דלתות/חצים** — אין מגמה/חץ ליד מספר; תווית חלון אחידה "7 הימים האחרונים" בשלושה הקלפים (דירוג מציג "{N} … |
| 1056 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **תג "בעלת עסק השבוע"** — נשאר ב-Overview כשמתקיימים התנאים (profile_strength≥80 + rank=1) |
| 1058 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **אין כפילות** — רצועת ה-KPI מופיעה רק ב-Overview, לא ב-תובנות (אנטי-MEH-961/963) |
| 1059 | MEH-964 chunk 1B — KPI strip on Overview + תובנות tab | **נייד (375px)** — 4 הטאבים: אם צרים מדי יש גלילה אופקית (overflow-x-auto), בלי חיתוך; רצועת ה-2×2 נקראת |
| 1066 | MEH-1134 — סקירה: סדר כרטיסים מותנה-מצב (completeness מעל av… | **עסק pending (או פרופיל חלקי)** — כרטיס השלמת הפרופיל מופיע מיד מתחת לבאנרי הסטטוס ו**מעל** כרטיס הזמינות (המ… |
| 1067 | MEH-1134 — סקירה: סדר כרטיסים מותנה-מצב (completeness מעל av… | **עסק מאושר + פרופיל מלא** — הסדר הנוכחי נשמר: זמינות קודם, כרטיס ההשלמה מתחתיו (מעל ה-KPI) — **תוצאה מצופה:**… |
| 1069 | MEH-1134 — סקירה: סדר כרטיסים מותנה-מצב (completeness מעל av… | **נטרול זמינות נשמר** — עסק pending: פילי הזמינות עדיין מנוטרלים עם רמז "למה נעול" |
| 1075 | MEH-1099 — עורך תמונות: drag-drop + טיפי צילום | **גרירת תמונה** — גוררים קובץ תמונה מעל האזור המקווקו — **תוצאה מצופה:** המסגרת נצבעת ירוק והטקסט מתחלף ל"שחרר… |
| 1076 | MEH-1099 — עורך תמונות: drag-drop + טיפי צילום | **קובץ שאינו תמונה** — גרירת PDF ושחרורו — **תוצאה מצופה:** לא קורה כלום (מסונן בשקט, אין שגיאה) |
| 1077 | MEH-1099 — עורך תמונות: drag-drop + טיפי צילום | **Click עדיין עובד** — לחיצה על האזור פותחת בורר קבצים ומעלה כרגיל |
| 1078 | MEH-1099 — עורך תמונות: drag-drop + טיפי צילום | **טיפי צילום** — 3 טיפים מוצגים מתחת לאזור ההעלאה (אור טבעי / תוצרת אמיתית / בלי סטוק); בעברית, ללא אימוג'י |
| 1079 | MEH-1099 — עורך תמונות: drag-drop + טיפי צילום | **נייד (375px)** — האזור לחיץ והטיפים נקראים ללא גלילה אופקית |
| 1085 | MEH-1101 — תובנות: zero-state לפני פרסום + ערים ועוקבות עם n… | **לפני אישור (status=pending)** — באנר בראש הדף: "העמוד עדיין לא פורסם" + "ברגע שיאושר, כאן יופיעו צפיות ופניו… |
| 1086 | MEH-1101 — תובנות: zero-state לפני פרסום + ערים ועוקבות עם n… | **אחרי אישור** — הבאנר לא מופיע |
| 1087 | MEH-1101 — תובנות: zero-state לפני פרסום + ערים ועוקבות עם n… | **ערים מובילות עם 1-2 ערים** — רשימה טקסטואלית (עיר · מספר) במקום bars; עם 3+ ערים — ה-bars חוזרים |
| 1088 | MEH-1101 — תובנות: zero-state לפני פרסום + ערים ועוקבות עם n… | **0 עוקבות** — במקום "0 · +0 השבוע": "עדיין אין עוקבות — שתפו את העמוד"; כשהעסק מאושר "שתפו את העמוד" מקושר לע… |
| 1089 | MEH-1101 — תובנות: zero-state לפני פרסום + ערים ועוקבות עם n… | **נייד (375px)** — הבאנר והקלפים נקראים ללא גלילה אופקית |
| 1095 | MEH-1102 — טאב כלים: ניקוי כפילות + אייקונים | **אין כרטיס "עריכת פרופיל"** — הגריד מציג 4 כרטיסים בלבד (הוספת אירוע / הצגת העסק באתר / ניהול קבוצות רכש / ני… |
| 1096 | MEH-1102 — טאב כלים: ניקוי כפילות + אייקונים | **אייקון לכל כרטיס** — כל כרטיס עם אייקון Phosphor ירוק מעל הכותרת |
| 1097 | MEH-1102 — טאב כלים: ניקוי כפילות + אייקונים | **יעדים ללא שינוי** — כל כרטיס מוביל לאותו יעד כמו קודם |
| 1098 | MEH-1102 — טאב כלים: ניקוי כפילות + אייקונים | **/settings עדיין נגיש** — דרך תפריט החשבון (AccountSheet) ודרך כרטיס השלמת הפרופיל (שדה עיר) |
| 1099 | MEH-1102 — טאב כלים: ניקוי כפילות + אייקונים | **נייד (375px)** — הגריד נקרא, הכרטיסים לחיצים |
| 1105 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **תמונות** — לעסק עם 5 תמונות — **תוצאה מצופה:** 3 תמונות ממוזערות 40px מעוגלות + תג "2+" בכותרת הסגורה |
| 1106 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **קטגוריות** — לעסק עם 4 קטגוריות — **תוצאה מצופה:** 3 שבבי קטגוריה + "1+" |
| 1107 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **ביו** — השורה הראשונה של התיאור הנוכחי, חתוכה בשורה אחת, אפור מעומעם |
| 1108 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **מיקום** — אייקון סיכת מפה + שם היישוב (בלי אריחי מפה בכותרת) |
| 1109 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **מוצרים / ערוצי קשר** — שם המוצר הראשון · אייקון הערוץ הראשי + שמו |
| 1110 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **כרטיס ריק** — עסק חדש בלי תוכן — **תוצאה מצופה:** שורת הסיכום הקיימת + לוחית מקווקוות מעומעמת (בלי טקסט שיוו… |
| 1111 | MEH-1158 — טאב עריכה: תצוגה מקדימה בכותרות האקורדיון | **נייד 390px** — אין גלילה אופקית; כל כותרת עדיין לחיצה אחת (44px+); פתיחה/סגירה ועוגן `#images` עובדים כרגיל |
| 1126 | MEH-1163 — כרטיס ביו: שדה ידני תמיד גלוי | **ללא ביו שמור** — פתיחת הכרטיס בעסק בלי תיאור — **תוצאה מצופה:** תיבת הטקסט של הביו מוצגת מיד (ריקה, עם מונה … |
| 1127 | MEH-1163 — כרטיס ביו: שדה ידני תמיד גלוי | **ביו שמור קיים** — פתיחת הכרטיס בעסק עם תיאור — **תוצאה מצופה:** התיבה ממולאת מראש בביו השמור |
| 1128 | MEH-1163 — כרטיס ביו: שדה ידני תמיד גלוי | **יצירת AI ממלאת** — הזנת מקור ולחיצה על "יצירת ביו" — **תוצאה מצופה:** הטקסט שנוצר מחליף את תוכן התיבה, ואפשר… |
| 1129 | MEH-1163 — כרטיס ביו: שדה ידני תמיד גלוי | **AI לא זמין לא מוחק** — כשהשרת מחזיר ביו ריק (fail-open) — **תוצאה מצופה:** הטקסט הקיים בתיבה נשאר, מוצגת רק … |
| 1130 | MEH-1163 — כרטיס ביו: שדה ידני תמיד גלוי | **שמירה ידנית בלבד** — כתיבת ביו ידנית (בלי AI) ולחיצה על "שמרו ביו" ורענון — **תוצאה מצופה:** הביו נשמר ומופי… |
| 1136 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **דף נקי** — כניסה לעורך ומעבר לטאב אחר בלי לערוך — **תוצאה מצופה:** אין באנר, אין דיאלוג, המעבר חופשי |
| 1137 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **עריכה → באנר** — הקלדה בשדה כלשהו (למשל טלפון) — **תוצאה מצופה:** באנר "יש שינויים שלא נשמרו" מופיע מתחת לטא… |
| 1138 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **ניווט עם שינויים** — לחיצה על טאב/קישור אחר — **תוצאה מצופה:** דיאלוג אישור; ביטול משאיר בעמוד, אישור עוזב |
| 1139 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **סגירת טאב/רענון עם שינויים** — **תוצאה מצופה:** דפדפן מציג את חלון האזהרה המובנה |
| 1140 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **שמירה מנקה** — שמירת הכרטיס שנערך — **תוצאה מצופה:** הבאנר נעלם והניווט חופשי שוב |
| 1141 | MEH-1100 — עורך הפרופיל: הגנת שינויים לא שמורים | **נייד (375px)** — הבאנר נקרא, הדיאלוג עובד |
| 1147 | MEH-288 — ProfileCompletenessCard on producer dashboard | **State red** — עסק חסר עיר / מיקום / פרטי קשר → כותרת "הפרופיל שלך חסר פרטים קריטיים" + טבעת אדומה + כפתור "ה… |
| 1148 | MEH-288 — ProfileCompletenessCard on producer dashboard | **State yellow ≤70%** — עסק עם עיר+מיקום+קשר אבל בלי קטגוריה/תמונה → כותרת "הפרופיל שלך X% מוכן", שורת "השלב ה… |
| 1149 | MEH-288 — ProfileCompletenessCard on producer dashboard | **State yellow >70%** — חסר רק שדה אחד (למשל תמונה) → כותרת "כמעט שם — X% מוכן" + "רק N פרטים עד שהפרופיל מלא" |
| 1150 | MEH-288 — ProfileCompletenessCard on producer dashboard | **State green** — כל השדות מלאים → הכרטיס מתכווץ לשורה אחת "✓ הפרופיל מלא" (לא נעלם) |
| 1151 | MEH-288 — ProfileCompletenessCard on producer dashboard | **נייד (375px)** — הכרטיס נקרא, הטבעת + הכותרת לא נשברות; אין horizontal scroll |
| 1152 | MEH-288 — ProfileCompletenessCard on producer dashboard | **a11y** — קורא-מסך מכריז על אחוז ההשלמה (role=progressbar) ועל ה-CTA ("השלימי את הפרופיל שלך") |
| 1153 | MEH-288 — ProfileCompletenessCard on producer dashboard | **/en** — אותו כרטיס באנגלית (Your profile is X% ready / Complete profile) — אין מחרוזות עבריות גולמיות |
| 1154 | MEH-288 — ProfileCompletenessCard on producer dashboard | **Regression** — קלפי ה-analytics, חוזק פרופיל, וכל שאר ה-dashboard נשארים מתחת לכרטיס ללא שינוי |
| 1156 | MEH-288 — ProfileCompletenessCard on producer dashboard | **MEH-1002 — שדה "תיאור קצר" (שישי)** — עסק מלא בלי תיאור (גם tagline וגם סיפור ריקים) → הכרטיס מציג "כמעט שם … |
| 1162 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **4/4 הושלמו** — עסק עם תמונה + קטגוריה+מיקום + ≥3 מוצרים + קשר → הכרטיס מתכווץ לשורה אחת "✓ הפרופיל מלא" |
| 1163 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **חסרה תמונה (75%)** — טבעת זהב 75% (לא אדום), כותרת "כמעט שם — 75% מוכן", צ'קליסט 4 שורות (3 עם ✓ + "תמונה רא… |
| 1164 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **חסרים 2 צעדים (50%)** — כותרת "הפרופיל שלך 50% מוכן", טבעת זהב |
| 1165 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **חסרים רק מוצרים (<3)** — צעד "3 מוצרים בקטלוג" מסומן כחסר; זהו signal card-only — **רשימת האדמין לא מושפעת**… |
| 1166 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **deep-link לכל צעד** — לחיצה על שורה → `/producer/dashboard/edit#profile-images\|profile-categories\|profile-pr… |
| 1167 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **delivery-only** — עסק משלוחים בלבד עם אזורי משלוח → צעד המיקום הושלם דרך "אזורי משלוח" (לא דורש קואורדינטות) |
| 1168 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **נייד (375px)** — הכרטיס + 4 השורות + הקופסה + CTA נקראים, אין horizontal scroll |
| 1169 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **/en** — הצ'קליסט מוצג באנגלית (Main photo / Categories & location / 3 products in your catalog / Contact det… |
| 1170 | MEH-1106 — ProfileCompletenessCard צ'קליסט 4-צעדים (card-onl… | **Regression — ChangesRequestedBanner** — באנר "נשאר להשלים" (ערוץ אדמין נפרד) ממשיך להופיע עצמאית כשיש `reque… |
...
| 1203 | Overnight design batch 2026-06-12/13 (PRs #1073–#1080) | Hero חוויות + רכישות קבוצתיות (MEH-797) — תמונות Cloudinary נטענות, טקסט קריא על הסקרים ב-375 |
| 1220 | MEH-534 — /about/process "תהליך הקבלה" (S11 Direction D) | **Footer link** — "תהליך הקבלה" מופיע בעמודת הניווט בפוטר → מוביל ל-`/about/process` |
| 1231 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Favorite — save (first time)** — לחצי ❤ על עסק (פעם ראשונה במכשיר) — toast עם |
| 1233 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Favorite — save (repeat)** — שמירה נוספת — toast לב מלא + "נשמר למועדפים" |
| 1234 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Favorite — remove** — הסרה ממועדפים — toast אייקון ✓ ברירת־מחדל + "הוסר מהמועדפים" |
| 1235 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Follow** — מעקב אחרי עסק — toast פעמון (Bell) + "מעכשיו תקבלי עדכונים…", בלי 🔔 |
| 1236 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Share / copy link** — העתקת קישור (ShareButton) — toast אייקון Check + "הקישור הועתק" |
| 1237 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Review saved** — שליחת ביקורת — toast כוכב (Star) + "הביקורת שלך נשמרה", בלי ⭐ |
| 1238 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | ~~**Publish neighbor product** — פרסום מוצר שכן — toast עלה (Leaf) + "המוצר פורסם!", בלי 🌿~~ — **N/A (MEH-1406… |
| 1239 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | ~~**Under review** — מוצר שסומן בבדיקה — toast זכוכית מגדלת (MagnifyingGlass) + "…בבדיקה", בלי 🔍~~ — **N/A (ME… |
| 1240 | MEH-685 — Toast semantic icon API (Category D2 emoji strip) | **Error toast** — כשל רשת/פעולה — toast אדום + אייקון WarningCircle |
...
| 1289 | Desktop (≥768px) | מחובר (consumer): avatar dropdown = **הגדרות → התנתקות בלבד** (MEH-1226 — אין `הפרופיל שלי` ללקוחה) — איך לבדו… |
| 1290 | Desktop (≥768px) | מחובר (producer): avatar dropdown = `לוח הבקרה שלי` → `הפרופיל שלי` → `הגדרות` → התנתקות (MEH-1226) — איך לבדו… |
| 1291 | Desktop (≥768px) | מחובר (admin): avatar dropdown = `הגדרות` → `ממשק אדמין` → התנתקות (MEH-1226 — ללא dashboard/profile) — תוצאה:… |
| 1292 | Desktop (≥768px) | מחובר (producer/admin): **אין** `הוסיפו עסק` (MEH-669) — תוצאה: CTA מוסתר |
| 1296 | Mobile (375px) | hamburger over-hero = glass יחיד (`bg-white/15 backdrop-blur`) — תוצאה: כפתור מטושטש קריא מעל התמונה |
| 1297 | Mobile (375px) | פתיחת drawer = warm-dark (`green-900`) — קישורי Frank Ruhl 24px + ספרות זהב `01·02·03` — תוצאה: drawer כהה, מס… |
| 1298 | Mobile (375px) | drawer: green `הוסיפו עסק` + ghost-on-dark `כניסה לחשבון` (אורח) — תוצאה: שתי כפתורים full-width |
| 1299 | Mobile (375px) | drawer מחובר: favorites + admin (אם admin) + logout, restyled כהה — תוצאה: כולם נוכחים וקריאים |
| 1317 | MEH-671 — Producer-signup smoke (now automated) | Railway log shows `[RISK] scored producer=` (not `… ANTHROPIC_API_KEY not set` / `… unparseable` / `… crashed`… |
| 1318 | MEH-671 — Producer-signup smoke (now automated) | Admin badge for the row shows a numeric `risk_score` 0–100 (not `אין מידע`) |
| 1319 | MEH-671 — Producer-signup smoke (now automated) | **Cleanup**: after the run, `SELECT count(*) FROM users WHERE email LIKE 'smoke+%@mehamakor.online'` → **0** (… |
| 1352 | Test 3 — Regression: consumer can still upgrade to producer | Log in as regular consumer (any non-admin, non-producer account) |
| 1354 | Test 3 — Regression: consumer can still upgrade to producer | Submit a valid producer signup → **expected:** 200, role flips to `"producer"`, redirects to `/producer/dashbo… |
...
| 2492 | Regression | Producer page still loads and renders correctly |
| 2494 | Regression | JSON-LD structured data still present in page source |
| 2501 | Image optimization (LCP) | `grep -rn 'images.unsplash.com' frontend/app/ frontend/components/ \| grep -v fm=webp \| grep -v next.config \| g… |
| 2502 | Image optimization (LCP) | `grep -rn 'images.unsplash.com.*w=600' frontend/app/page.js` → all 6 category cards include `&q=80&fm=webp` |
| 2503 | Image optimization (LCP) | Network tab: hero image response header `Content-Type: image/webp` (when browser supports it) |
| 2506 | Layout shift (CLS) | ProducerCard image container has explicit `h-[140px] md:h-[200px]` |
| 2507 | Layout shift (CLS) | HomeProductCard image container has explicit `h-48` |
| 2508 | Layout shift (CLS) | Category cards have explicit `height: 280px` |
| 2509 | Layout shift (CLS) | Hero sections use `height: 100vh` |
| 2510 | Layout shift (CLS) | No visible content jump on page load (manual check) |
| 2514 | Bundle size | Shared chunk < 90kB |
| 2521 | Running | `cd frontend && npx vitest run --reporter=verbose` — all 33 tests pass |
| 2522 | Running | Stop hook runs vitest automatically on every task completion |
| 2525 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Anatomy** — image (1:1 mobile, 4:3 desktop) → name row with inline `★ rating · count` → location dot + city … |
| 2527 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Cloudinary smart-crop** — for a Cloudinary source image, the URL has `c_fill,g_auto,ar_4:3` injected. Portra… |
| 2528 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Rating gate** — producer with `reviews_count = 2` shows no rating; `reviews_count >= 3` shows `★ 4.5 · 12` i… |
| 2529 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Availability dot** — `is_available_today=true` → green; `availability_status="vacation"` → orange (overrides… |
| 2530 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Description fallback** — `short_description` shown when present; else `top_product_name`; else row hidden. D… |
| 2534 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Heart — authed flow** — tap heart while logged in → heart fills, `POST /users/me/favorites/{id}` fires, toas… |
| 2535 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Heart — error revert** — disable network, tap heart → heart fills then reverts, error toast "משהו השתבש, נסי… |
| 2536 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Heart — own-card hide** — as a logged-in producer, navigate to a page showing your own producer card (e.g. `… |
| 2538 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **RTL** — heart sits at `top-3 start-3` (physical right in Hebrew). Rating + distance numerics stay LTR via `d… |
| 2540 | ProducerCard — Phase A → B → C redesign (2026-04-18) | **Skeleton** — while `producers` loads, `SkeletonProducerGrid` renders the exact-same anatomy (square→4:3 imag… |
| 2543 | HomeProductCard (16 tests) | Renders image when available, placeholder when null |
| 2544 | HomeProductCard (16 tests) | Price: null → empty, 0 → "🎁 במתנה", number → "₪X / unit" |
| 2545 | HomeProductCard (16 tests) | Neighborhood when available, city as fallback |
| 2546 | HomeProductCard (16 tests) | Allergens/quantity/seller shown only when present |
| 2547 | HomeProductCard (16 tests) | Moderation badge for FLAGGED status |
| 2548 | HomeProductCard (16 tests) | Organic badge only when is_organic=true |
...
| 2579 | localStorage persistence | Visit any producer page (e.g. `/producer/1`) → open DevTools → Application → Local Storage → `recently_viewed`… |
...
| 2661 | Regression checks | Category card clicks still filter the grid |
| 2662 | Regression checks | "הצגי עוד" load-more button still works after geolocation filter |
| 2669 | City-filtered empty state | `/neighbor` — select a city with no products (e.g. an obscure city) — empty state appears |
| 2670 | City-filtered empty state | Large emoji: **🏡** (house with garden) in a round `bg-light` container |
| 2671 | City-filtered empty state | Heading: **"אין מוצרים באזור הזה עדיין 🌱"** (exact text) |
| 2672 | City-filtered empty state | Subtext (logged in): **"היי את הראשונה לפרסם מוצר בית!"** (exact text) |
| 2673 | City-filtered empty state | CTA button (logged in): **"פרסמי מוצר +"** — click opens the product form |
| 2675 | City-filtered empty state | CTA button hidden when logged out |
| 2678 | General empty state (no city filter) | `/neighbor` with no products at all — heading: **"אין עדיין מוצרים ביתיים 🌱"** |
| 2679 | General empty state (no city filter) | Same emoji, subtext, and CTA behavior as above |
| 2682 | Regression checks | `/neighbor` with products — grid renders normally, no empty state shown |
| 2683 | Regression checks | "הצגי הכל" button still clears city filter |
| 2684 | Regression checks | Mobile floating CTA button still works |
| 2685 | Regression checks | Disclaimer banner still visible above the grid |
| 2701 | Admin sidebar (start-0) | Log in as admin → `/admin` — sidebar appears on the **right** side; main content fills the left |
| 2707 | ESLint CI | Open a PR to staging → GitHub Actions "Frontend lint" job runs and passes (exits 0 even with pre-existing warn… |
| 2708 | ESLint CI | After PR #137 merges → run lint locally; RTL warnings should be nearly zero |
| 2716 | Session Handoff | Start new session → Claude reads HANDOFF.md before any other file (Rule 1 step a) |
| 2717 | Session Handoff | End of session → HANDOFF.md updated with: last PR number, current branch state, next task, any new decisions |
| 2718 | Session Handoff | `/compact` fires mid-session → HANDOFF.md updated immediately before continuing work |
| 2719 | Session Handoff | Open new session next day → "Next task" section matches what was left unfinished |
| 2734 | Admin ProducerForm — "סוג העסק" section | Create new producer → "סוג העסק" section shows 2 checkboxes: "חנות פיזית" (checked by default) + "משלוחים" (un… |
| 2735 | Admin ProducerForm — "סוג העסק" section | Uncheck both checkboxes → inline error "חייב לסמן לפחות אחד מהשניים" appears; save button becomes disabled |
| 2736 | Admin ProducerForm — "סוג העסק" section | Check "משלוחים" only → cascading section appears with "משלוחים לכל הארץ" checkbox + CitiesAutocomplete below i… |
| 2737 | Admin ProducerForm — "סוג העסק" section | Check "משלוחים לכל הארץ" → CitiesAutocomplete disappears; save is enabled |
| 2738 | Admin ProducerForm — "סוג העסק" section | Uncheck "משלוחים לכל הארץ" with no cities selected → inline error "יש לבחור לפחות עיר אחת"; save disabled |
| 2739 | Admin ProducerForm — "סוג העסק" section | Type "תל" in CitiesAutocomplete → dropdown shows cities starting with "תל" (requires seeded cities table); cli… |
| 2740 | Admin ProducerForm — "סוג העסק" section | Click × on a city chip → chip is removed |
| 2741 | Admin ProducerForm — "סוג העסק" section | Keyboard: ArrowDown/Up navigates dropdown; Enter adds selected city; Backspace removes last chip when input is… |
| 2742 | Admin ProducerForm — "סוג העסק" section | Save delivery-only producer (no physical, offers_delivery=true, delivery_nationwide=true) → producer created; … |
| 2745 | ProducerDetail — 4 location modes | Physical-only producer → MiniMap visible, Waze/Gmaps buttons visible, no DeliveryBlock |
| 2746 | ProducerDetail — 4 location modes | Physical + delivery producer → MiniMap visible AND DeliveryBlock visible below |
| 2747 | ProducerDetail — 4 location modes | Delivery-only + nationwide → no MiniMap, no Waze/Gmaps; DeliveryBlock shows "🚚 משלוחים לכל הארץ" badge |
| 2748 | ProducerDetail — 4 location modes | Delivery-only + city list → no MiniMap; DeliveryBlock shows city chips |
| 2749 | ProducerDetail — 4 location modes | Delivery-only + no area set → DeliveryBlock shows "משלוחים בתיאום מראש — צרי קשר לפרטים" |
| 2753 | ProducerCard — "משלוחים בלבד" badge | Delivery-only producer in list grid → shows "🚚 משלוחים בלבד" chip in badge row |
| 2754 | ProducerCard — "משלוחים בלבד" badge | Physical-only or physical+delivery producer → no "משלוחים בלבד" chip |
| 2760 | Admin completeness dot | Delivery-only producer with delivery_nationwide=true but no lat/lng → completeness dot is green (not red) |
| 2761 | Admin completeness dot | Delivery-only producer with offers_delivery=true but no cities and no nationwide → completeness dot is yellow … |
| 2764 | GET /cities?q= endpoint | After running seed script: GET /api/cities?q=תל → returns ["תל אביב-יפו", "תל מונד", ...] (Hebrew sorted) |
| 2765 | GET /cities?q= endpoint | GET /api/cities?q= (empty) → returns up to 20 cities alphabetically |
| 2766 | GET /cities?q= endpoint | GET /api/cities?q=xxxxnotacityxxx → returns [] |
| 2784 | /producers?q= results page | Navigate to `/producers?q=עגבנייה` → heading **"תוצאות עבור: עגבנייה"** appears above the grid |
| 2785 | /producers?q= results page | Active filter chip **🔍 עגבנייה** appears in the chip row; click × → clears `q`, heading and chip disappear, fu… |
| 2786 | /producers?q= results page | ProducerCard names and descriptions show matched text in **bold** (no yellow background — `bg-transparent font… |
| 2787 | /producers?q= results page | `/producers?q=xxxnotexist` → empty state shows "לא נמצאו בתי עסק" with category pill shortcuts |
| 2788 | /producers?q= results page | `/producers?q=` (empty q) → behaves as normal unfiltered grid (no heading, no chip) |
| 2789 | /producers?q= results page | `GET /producers?q=50%` → backend handles `%` as literal character (wildcard escaping), returns correct results… |
| 2790 | /producers?q= results page | `GET /producers?q=ח_ל_ב` → `_` treated as literal underscore, not LIKE wildcard |
| 2820 | Case A — Silent refresh on access expiry | Wait 16 minutes (access TTL = 15min) |
| 2821 | Case A — Silent refresh on access expiry | Click any protected action (e.g. favorite a producer) |
| 2823 | Case A — Silent refresh on access expiry | DevTools → Network → `/auth/refresh` returned 200 immediately before the retried action request |
| 2827 | Case B — Forced logout when refresh expired / missing | DevTools → Application → Cookies → delete `refresh_token` cookie |
| 2828 | Case B — Forced logout when refresh expired / missing | Edit `localStorage.token` to garbage (or wait for access to expire) |
| 2829 | Case B — Forced logout when refresh expired / missing | Click any protected action |
| 2834 | Case C — logout-all-devices stays authenticated on current d… | In browser A: call `POST /auth/logout-all-devices` |
| 2835 | Case C — logout-all-devices stays authenticated on current d… | EXPECT browser A: stays authenticated (new access token + rotated refresh cookie) |
| 2836 | Case C — logout-all-devices stays authenticated on current d… | In browser B: click any protected action |
| 2842 | Producer dashboard (/producer/dashboard) | Open dashboard while logged in as a producer — איך לבדוק: navigate to `/producer/dashboard` — תוצאה מצופה: a s… |
| 2843 | Producer dashboard (/producer/dashboard) | Click "פתוח להזמנות" — תוצאה מצופה: pill highlights, no vacation date input shown. |
| 2844 | Producer dashboard (/producer/dashboard) | Click "זמינה היום 🟢" — תוצאה מצופה: pill highlights, no vacation date input. |
| 2845 | Producer dashboard (/producer/dashboard) | Click "עמוסה השבוע 🟠" — תוצאה מצופה: pill highlights, no vacation date input. |
| 2846 | Producer dashboard (/producer/dashboard) | Click "בהפסקה ⏸" — תוצאה מצופה: pill highlights, vacation date input + "שמרו" button appear below — BEFORE any… |
| 2847 | Producer dashboard (/producer/dashboard) | Click "שמרו" with the date empty — תוצאה מצופה: inline red error "בחרו תאריך חזרה כדי לעבור להפסקה"; no POST t… |
| 2848 | Producer dashboard (/producer/dashboard) | Pick a future date + click "שמרו" — תוצאה מצופה: POST `{state:"on_vacation", vacation_until:...}` succeeds; re… |
| 2849 | Producer dashboard (/producer/dashboard) | Switch back to "פתוח להזמנות" — תוצאה מצופה: vacation date cleared. |
| 2853 | ProducerCard badge dot | Producer with `full_this_week` — תוצאה מצופה: orange dot. |
| 2854 | ProducerCard badge dot | Producer with `on_vacation` — תוצאה מצופה: NOT visible in default `/producers` listing (default-hide). Visible… |
| 2857 | ProducerDetail banners (/producer/[id]) | Producer with `accepting_orders` — תוצאה מצופה: AvailabilityBadge "פתוח להזמנות"; no extra banner. |
| 2858 | ProducerDetail banners (/producer/[id]) | Producer with `available_today` — תוצאה מצופה: AvailabilityBadge "זמינה היום 🟢"; no extra banner. |
| 2859 | ProducerDetail banners (/producer/[id]) | Producer with `full_this_week` — תוצאה מצופה: amber banner "⏳ זמני תגובה ארוכים יותר השבוע" below the highligh… |
| 2860 | ProducerDetail banners (/producer/[id]) | Producer with `on_vacation` — תוצאה מצופה: slate vacation banner "🌙 בית עסק זה בהפסקה כרגע" + return-date labe… |
| 2863 | Admin form (/admin/producers/new + /admin/producers/[id]) | Open admin producer form — תוצאה מצופה: "מצב זמינות" section shows 4 radio pills matching dashboard labels. |
| 2864 | Admin form (/admin/producers/new + /admin/producers/[id]) | Choose "בהפסקה ⏸" — תוצאה מצופה: vacation date field appears below. |
| 2865 | Admin form (/admin/producers/new + /admin/producers/[id]) | Save form — תוצאה מצופה: PUT `/admin/producers/{id}` succeeds with `availability_state` in payload. |
| 2866 | Admin form (/admin/producers/new + /admin/producers/[id]) | Reload edit page — תוצאה מצופה: form pre-populates with the saved state. |
| 2872 | Default `/producers` listing change | Mark a test producer as `on_vacation` via the dashboard or admin form — תוצאה מצופה: producer disappears from … |
| 2873 | Default `/producers` listing change | Hit `GET /producers?availability_state=on_vacation` directly — תוצאה מצופה: producer appears. |
| 2874 | Default `/producers` listing change | Visit producer's direct slug URL — תוצאה מצופה: detail page still loads with the vacation banner. |
| 2890 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **1. Find latest R2 backup.** |
| 2894 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **2. Create empty test DB locally.** |
| 2898 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **3. Restore via the script.** |
| 2902 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **4. Verify row counts roughly match production.** |
| 2909 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **5. Drop the test DB.** |
| 2913 | MEH-408 Phase 4 — DR drill (one-time, before MEH-408 closes) | **6. Log the result.** |
| 3054 | When to (re-)run | Once, the week before public launch (the canonical MEH-559 run). |
| 3055 | When to (re-)run | After any major backend refactor of `backend/app/routers/producers.py`, `chat.py`, or `favorites.py`. |
| 3056 | When to (re-)run | After a Railway plan change (free -> hobby -> pro) — the latency numbers shift and the baseline must be re-anc… |
| 3057 | When to (re-)run | After any Anthropic model swap on `/chat` (Haiku -> Sonnet, version bumps) — verify the `/chat` p95 still fits… |
| 3075 | UIS Pattern A (MEH-228) — admin double-submit protection | `/admin/reports` — לחיצה כפולה מהירה על "השעה"/"אשר"/"הסר"/"שחזר" — תוצאה: הפעולה רצה פעם אחת, הכפתור מושבת בז… |
| 3076 | UIS Pattern A (MEH-228) — admin double-submit protection | `/admin/users` — לחיצה כפולה על "חסום/בטל חסימה" — תוצאה: בקשה אחת, כפתור מושבת בזמן הריצה |
| 3077 | UIS Pattern A (MEH-228) — admin double-submit protection | `/admin/content` (מוצרי בית מוסתרים) — לחיצה כפולה על "שחזר"/"מחק" — תוצאה: בקשה אחת |
| 3078 | UIS Pattern A (MEH-228) — admin double-submit protection | `/admin/producers` — לחיצה כפולה על "אשר"/"השעה"/"שגרירה"/"מחק" — תוצאה: בקשה אחת, הכפתור מושבת |
| 3079 | UIS Pattern A (MEH-228) — admin double-submit protection | כשל רשת על כל פעולת אדמין מהנ"ל — תוצאה: toast שגיאה בעברית (לא כשל שקט) |
| 3094 | MEH-848 — error toasts collapsed to error.generic (i18n refa… | **מועדף בכרטיס (ProducerCard)** — לחיצה על לב כשמנותקים/כשל רשת → טוסט "משהו השתבש, נסו שוב" |
| 3095 | MEH-848 — error toasts collapsed to error.generic (i18n refa… | **שליחת ביקורת נכשלת** (`ReviewsSection`) + **קבוצת רכש** (`GroupBuyDetailClient`) → אותה הודעה כללית |
| 3096 | MEH-848 — error toasts collapsed to error.generic (i18n refa… | **/en** — אותם מסכים באנגלית → "Something went wrong, try again" (לא מפתח גולמי, פריטי he/en זהים) |
| 3111 | MEH-992 — group-buy dashboard form clarity | **₪ בשדות מחיר** — פתחי את הטופס → בשני שדות המחיר (רגיל + קבוצתי) מופיע ₪ בצד ימין של השדה, המספר מיושר לצדו … |
| 3112 | MEH-992 — group-buy dashboard form clarity | **helper מחיר לפני שליחה** — הזיני מחיר קבוצתי ≥ מחיר רגיל (למשל 25 מול 20) → הטקסט "המחיר הקבוצתי חייב להיות … |
| 3113 | MEH-992 — group-buy dashboard form clarity | **helper מועד אחרון** — מתחת לשדה המועד האחרון מופיע "המועד האחרון להצטרפות, לפי שעון ישראל." — תוצאה מצופה: מ… |
| 3114 | MEH-992 — group-buy dashboard form clarity | **intro מושג** — מתחת לכותרת "קבוצת רכש חדשה" מופיעה שורה אחת שמסבירה מה זו קבוצת רכש (מספיק לקוחות → מחיר סיט… |
| 3121 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **תור ממתינים** — צרי מתכון כבעלת עסק (`/producer/dashboard/recipes`) → פתחי `/admin/recipes` כמנהלת → תוצאה מ… |
| 3122 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **badge בסיידבר** — כשיש מתכון ממתין → תוצאה מצופה: המונה ליד "לוח מחוונים" בסיידבר כולל אותו (pending_moderat… |
| 3123 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **אישור** — לחצי "אשרי" → תוצאה מצופה: toast "המתכון אושר ופורסם"; המתכון עובר לטאב "מאושרים"; מופיע בעמוד הצי… |
| 3124 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **בקשת שינויים** — "שינויים" בלי הערה → נחסם ("יש למלא הערה"); עם הערה → המתכון עובר ל"דרוש תיקון" ובעלת העסק … |
| 3125 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **דחייה** — "דחי" עם סיבה → המתכון עובר ל"נדחו" ולא מופיע בעמוד הציבורי |
| 3126 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **טאבים** — מעבר בין 5 הטאבים (ממתינים / דרוש תיקון / מאושרים / נדחו / הכל) — תוצאה מצופה: כל טאב מסנן לפי הסט… |
| 3127 | MEH-997 — עמוד מודרציית מתכונים חדש (/admin/recipes) | **מובייל 375px** — העמוד נטען, הטבלה נגללת אופקית בתוך הכרטיס, הטאבים נגללים — תוצאה מצופה: אין גלילה אופקית ש… |
| 3135 | MEH-1115 — הסברי "מה זה?" בלוח הבקרה | **ערוץ ראשי** — פותחות את כרטיס "ערוצי קשר"; מתחת לכותרת "הערוץ הראשי" מופיע קישור "מה זה?" — **תוצאה מצופה:**… |
| 3136 | MEH-1115 — הסברי "מה זה?" בלוח הבקרה | **טופס הזמנות** — מתחת לשדה "טופס הזמנות (קישור)" — "מה זה?" נפתח/נסגר באותו אופן |
| 3137 | MEH-1115 — הסברי "מה זה?" בלוח הבקרה | **קבוצת רכש** — בראש עמוד קבוצות הרכש, מתחת לכותרת — "מה זה?" מסביר מהי הזמנה מרוכזת |
| 3138 | MEH-1115 — הסברי "מה זה?" בלוח הבקרה | **מובייל 375px** — שלושת הטריגרים לחיצים בקלות (יעד הקשה ≥44px); הפאנל לא שובר את הפריסה |
| 3139 | MEH-1115 — הסברי "מה זה?" בלוח הבקרה | **/en** — הטריגר "What's this?" והפאנלים באנגלית (לא עברית) |
| 3146 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **אחד פתוח בכל רגע** — פתיחת כרטיס שני סוגרת את הראשון |
| 3147 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **שימור מצב** — פותחים כרטיס, מקלידים בלי לשמור, סוגרים ופותחים שוב — **תוצאה מצופה:** הטקסט שהוקלד נשמר; באנר… |
| 3148 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **שומר הניווט (MEH-1100)** — עם שדה שלא נשמר, לוחצות על טאב אחר — **תוצאה מצופה:** דיאלוג אישור עוצר את הניווט |
| 3149 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **עוגן URL** — כניסה ישירה ל-`/producer/dashboard/edit#categories` — **תוצאה מצופה:** כרטיס הקטגוריות נפתח אוט… |
| 3150 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **מקלדת** — Tab מגיע לכותרות הכרטיסים; Enter/רווח פותח וסוגר; חץ הכיוון (CaretDown) מתהפך כשפתוח |
| 3151 | MEH-1116 — טאב עריכה כאקורדיון + עוגני URL | **מובייל 375px** — הכותרות + הסיכומים נקראים בלי חיתוך; אין גלילה אופקית |
| 3157 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **cap 3 (רישום)** — בוחרות 3 קטגוריות — **תוצאה מצופה:** מונה מציג "3/3"; כל הכרטיסים הלא-נבחרים הופכים לא-פעי… |
| 3158 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **ביטול בחירה ב-cap** — עם 3 נבחרות, לוחצות על אחת מהנבחרות — **תוצאה מצופה:** היא מתבטלת והכרטיסים חוזרים להי… |
| 3159 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **תג "ראשית"** — הקטגוריה הראשונה שנבחרה נושאת תג "ראשית" (pill ירוק); בוחרות בסדר ב→א→ג — התג על ב |
| 3162 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **cap 3 (אדמין)** — בטופס האדמין, אחרי סימון 3 checkbox — **תוצאה מצופה:** ה-checkbox הרביעי מושבת (disabled);… |
| 3163 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **מחיקת קטגוריה משויכת (אדמין)** — מנסות למחוק קטגוריה שיש לה בתי עסק — **תוצאה מצופה:** שגיאה 409 "לא ניתן למ… |
| 3164 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **מחיקת קטגוריה ריקה** — מוחקות קטגוריה בלי בתי עסק — **תוצאה מצופה:** נמחקת בהצלחה |
| 3165 | MEH-1297 — ריבוי קטגוריות: סדר, cap 3, ותג "ראשית" | **מובייל 375px** — המונה + התג + ה-hint נקראים בלי חיתוך; אין גלילה אופקית |
| 3173 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **עלות בלבד** — עלות 35, סף ריק — **תוצאה מצופה:** "משלוח: 35₪" בלי החלק של "מעל" |
| 3174 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **סף בלבד** — עלות ריקה, סף 250 — **תוצאה מצופה:** "מעל 250₪ — חינם" לבד. זה מצב חוקי, לא באג |
| 3175 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **עלות 0 = חינם** — מקלידות 0 בעלות המשלוח — **תוצאה מצופה:** "משלוח חינם". **לעולם לא** "משלוח: 0₪" |
| 3176 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **0 שורד רענון** — אחרי שמירת 0, מרעננות את הדשבורד — **תוצאה מצופה:** השדה מציג 0, לא ריק (אם הוא ריק — ה-0 נ… |
| 3178 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **0 יחד עם נקודות איסוף** — עלות 0 + לעסק יש נקודות איסוף — **תוצאה מצופה:** "משלוח חינם" בשורת העלות, ו"חינם"… |
| 3179 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **ערכים שליליים** — מנסות -1 בעלות — **תוצאה מצופה:** שגיאת שמירה (422), לא נשמר |
| 3180 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **סף 0** — מנסות 0 בסף משלוח חינם — **תוצאה מצופה:** שגיאת שמירה (422). בניגוד לעלות, 0 כאן אסור |
| 3181 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **עסק בלי משלוחים** — מבטלות "משלוחים" בכרטיס — **תוצאה מצופה:** שדות העלות נעלמים מהטופס, והערכים מתאפסים בשמ… |
| 3182 | MEH-1577 — שדות משלוח מובנים (delivery_fee + free_delivery_a… | **מובייל 375px** — השורה המשולבת ("משלוח: 35₪ · מעל 250₪ — חינם") נקראת בשורה אחת בלי חיתוך ובלי גלילה אופקית;… |
| 3189 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **הוספת טווח שני** — דשבורד → "מתי מקבלים הזמנות" → סמני יום → "+ טווח נוסף" — **תוצאה מצופה:** נוספת שורת שעו… |
| 3190 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **שמירה ורענון** — הגדירי ראשון 09:00–13:00 ו-16:00–20:00, שמרי, רענני את העמוד — **תוצאה מצופה:** שני הטווחים… |
| 3191 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **תקרה של 3** — הוסיפי טווח שלישי — **תוצאה מצופה:** כפתור "+ טווח נוסף" נעלם. יש X לכל אחד משלושת הטווחים |
| 3192 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **הסרת טווח** — הקישי X על הטווח האמצעי — **תוצאה מצופה:** רק הוא נמחק, השאר נשארים. לטווח בודד אין X (סוגרים … |
| 3193 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **חפיפה נחסמת** — הגדירי 09:00–13:00 ואז טווח שמתחיל ב-12:00, נסי לשמור — **תוצאה מצופה:** הודעה "הטווחים חייב… |
| 3194 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **צמידות מותרת** — 09:00–13:00 ואז 13:00–17:00 — **תוצאה מצופה:** נשמר בלי שגיאה. זה רצף, לא חפיפה |
| 3195 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **עסק ותיק (שורה ישנה)** — עסק שהגדיר טווח יחיד לפני העדכון — **תוצאה מצופה:** העורך נפתח עם טווח אחד, בלי שגי… |
| 3197 | MEH-1869 — חלון הזמנות: כמה טווחים ביום (03/08) | **מובייל 375px** — שורת יום עם 3 טווחים נקראת בלי גלילה אופקית; שדות השעה נשארים LTR |
| 3205 | MEH-1871 — פאנל overlay נסגר בגלילה (03/08) | **פאנל "+N" בדסקטופ נסגר בגלילה** — בדסקטופ (רוחב ≥1024px), כרטיס עם 3+ תגים → הקישי על "+N" → גללי — **תוצאה … |
| 3206 | MEH-1871 — פאנל overlay נסגר בגלילה (03/08) | **סיבוב מכשיר** — פתחי בועת תג במובייל וסובבי את המכשיר — **תוצאה מצופה:** הבועה נסגרת (לא נשארת במיקום ישן) |
| 3207 | MEH-1871 — פאנל overlay נסגר בגלילה (03/08) | **מגירה תחתונה במובייל לא הושפעה** — במובייל (<1024px), "+N" נפתח כמגירה תחתונה — **תוצאה מצופה:** המגירה מתנה… |
| 3208 | MEH-1871 — פאנל overlay נסגר בגלילה (03/08) | **סגירה רגילה עדיין עובדת** — Esc, הקשה מחוץ לפאנל, והקשה חוזרת על התג — **תוצאה מצופה:** כולן סוגרות כמו קודם |
| 3215 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **הוספת טווח שני** — דשבורד → "שעות פתיחה" → יום פתוח → "+ טווח נוסף" — **תוצאה מצופה:** נוספת שורה שממשיכה מה… |
| 3216 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **שמירה ורענון** — שישי 09:00–13:00 ו-16:00–19:00, שמרי, רענני — **תוצאה מצופה:** שני הטווחים חוזרים בדיוק |
| 3218 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **סטטוס בין הטווחים** — בשעה 14:30 ביום שיש בו הפסקה — **תוצאה מצופה:** "סגור", והפתיחה הבאה היא **באותו יום**… |
| 3219 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **תקרה של 3 / אין מקום** — הוסיפי טווח שלישי (הכפתור נעלם). ליום שנגמר ב-23:59 — **תוצאה מצופה:** כפתור "+ טוו… |
| 3220 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **חפיפה נחסמת** — טווח שמתחיל לפני שהקודם נגמר — **תוצאה מצופה:** "הטווחים חייבים להיות לפי הסדר ובלי חפיפה בי… |
| 3221 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **מחרוזת ישנה** — עסק עם "Sun-Thu 09:00-18:00, Fri 09:00-14:00" — **תוצאה מצופה:** נטענת בדיוק כמו קודם, ושמיר… |
| 3222 | MEH-1870 — שעות פתיחה: כמה טווחים ביום (03/08) | **מובייל 375px** — יום עם 3 טווחים נקרא בלי גלילה אופקית; שדות השעה נשארים LTR |
| 3231 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **סגור עכשיו** — אותו עסק, אחרי שעת הסגירה — **תוצאה מצופה:** הכרטיס נראה **בדיוק** כמו לפני העדכון. אין שורה,… |
| 3232 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **בלי חלון בכלל** — עסק שלא הגדיר "מתי מקבלים הזמנות" — **תוצאה מצופה:** אין שורה. זהה למצב הסגור |
| 3233 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **בחופשה** — עסק בחופשה שיש לו חלון פתוח עכשיו — **תוצאה מצופה:** **אין** שורת הזמנות. חופשה גוברת — אסור שהכר… |
| 3234 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **בין שני טווחים באותו יום** — עסק עם 09:00–13:00 ו-16:00–20:00, בשעה 14:30 — **תוצאה מצופה:** אין שורה (זה מצ… |
| 3235 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **מובייל 375px** — כרטיס עם חלון פתוח — **תוצאה מצופה:** השורה נשברת לשתי שורות והשעה **נראית במלואה**. אסור ש… |
| 3236 | MEH-1880 — שורת "פתוח להזמנות" על כרטיס העסק (04/08) | **השעה לא הפוכה** — בכל הרוחבים — **תוצאה מצופה:** "05:31" ולא "31:05" |
| 3249 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **חמישה סוגים ב-dropdown** — לוח בקרה → עריכה → הטבה — **תוצאה מצופה:** חמישה סוגים + "אין הטבה פעילה" (שש אפש… |
| 3250 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **שדות הסף נעלמים** — בוחרות "הטבה בניסוח חופשי" — **תוצאה מצופה:** שדה הסכום ותפריט יחידת המידה **נעלמים לגמר… |
| 3251 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **הכותרת חובה** — "הטבה בניסוח חופשי" עם תאריך סיום תקין וכותרת ריקה — **תוצאה מצופה:** כפתור השמירה חסום, ומת… |
| 3252 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **הכותרת מתמלאת** — כותבות משהו — **תוצאה מצופה:** השגיאה נעלמת והשמירה נפתחת |
| 3255 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **סף שנשאר מסוג קודם** — בוחרות "מתנה", ממלאות 150 ₪, ואז עוברות ל"ניסוח חופשי" ושומרות — **תוצאה מצופה:** הסף… |
| 3256 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **מובייל 375px** — כל האמור לעיל — **תוצאה מצופה:** הכותרת נקראת במלואה בתגית, והשגיאה האדומה בטופס לא נחתכת |
| 3257 | MEH-1898 — הטבה בניסוח חופשי (סוג חמישי) (04/08) | **סוג לא מוכר (edge, דרך API)** — `PUT /producers/me` עם `offer_type` שאינו אחד מהחמישה — **תוצאה מצופה:** 422… |
| 3273 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **מצב סגור (ברירת מחדל)** — עמוד עסק עם חלון הזמנות ראשון–חמישי → גללי ל"מתי מקבלים הזמנות" — **תוצאה מצופה:**… |
| 3274 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **פתיחה** — הקישי "כל השבוע" — **תוצאה מצופה:** הסיכום **נעלם** ובמקומו שורה לכל יום פתוח (ראשון, שני, שלישי…)… |
| 3275 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **היום מסומן** — באותו מצב פתוח — **תוצאה מצופה:** תגית קטנה "היום" ליד שם היום הנוכחי בלבד (ירוק בהיר), והשור… |
| 3276 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **סגירה** — הקישי שוב — **תוצאה מצופה:** חוזרים לסיכום המסוכם |
| 3277 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **יום מפוצל** — עסק עם שני טווחים באותו יום (למשל 9:00–13:00 ו-16:00–19:00) — **תוצאה מצופה:** ברשימה הפתוחה ש… |
| 3278 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **שעות בלי אפס מוביל** — **תוצאה מצופה:** "9:00–14:00" ולא "09:00–14:00", בשתי השכבות. (שימי לב: כרטיס "שעות פ… |
| 3279 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **עסק בלי מיזוג** — עסק שכל יום פתוח שלו בשעות שונות — **תוצאה מצופה:** אין כפתור "כל השבוע" בכלל; הסיכום כבר … |
| 3280 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **עסק עם יום פתוח אחד** — **תוצאה מצופה:** אין כפתור "כל השבוע" |
| 3281 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **עסק בלי חלון הזמנות** — **תוצאה מצופה:** הסקציה כולה לא קיימת (לא כותרת ריקה, לא מסגרת) |
| 3282 | MEH-1917 — חלון הזמנות: פריסה שבועית מלאה מאחורי disclosure … | **מובייל 375px** — **תוצאה מצופה:** כפתור ה-disclosure נוח להקשה (44px), הרשימה לא נחתכת, והמספרים נקראים משמא… |
| 3295 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **הקישור לאתר נפתח בלשונית חדשה עם ייחוס** — הקישי על "להזמנה באתר" — **תוצאה מצופה:** לשונית חדשה, וב-URL מופ… |
| 3300 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **עסק בלי אתר ובלי טלפון (edge)** — עסק שערוץ הקשר שלו "אתר" אבל שדה האתר ריק ואין טלפון — **תוצאה מצופה:** אי… |
| 3301 | MEH-1916 — ה-CTA בגיליון המוצר הולך אחרי הערוץ שבחרה בעלת הע… | **מובייל 375px** — כל האמור לעיל — **תוצאה מצופה:** שני היעדים נוגעים בנוחות (≥44px), הכפתור הראשי בולט והמשני… |
| 3318 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **גם ב-Footer** — גללי לתחתית — **תוצאה מצופה:** "חוויות" בעמודת "גלו", ליד "אירועים" |
| 3319 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **הקישור עובד ומוביל לדף לא ריק** — הקישי עליו — **תוצאה מצופה:** `/experiences` נטען עם לפחות 3 חוויות עתידיו… |
| 3320 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **BottomNav לא השתנה** — מובייל — **תוצאה מצופה:** סרגל הניווט התחתון זהה לחלוטין לקודם; אין שם "חוויות" (מחוץ… |
| 3321 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **מובייל — הגילוי דרך ה-Footer** — מובייל 375px, גללי לתחתית — **תוצאה מצופה:** "חוויות" נמצא בקבוצת "גלו"; ב-… |
| 3322 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **בקשה אחת לכל טעינת עמוד** — DevTools → Network → סנני `count` → רענני — **תוצאה מצופה:** קריאה אחת בלבד ל-`/… |
| 3323 | MEH-1918 — קישור "חוויות" בניווט, מגודר לפי היצע (06/08) | **התנהגות בכשל (אופציונלי, DevTools)** — חסמי את `/experiences/count` (Block request URL) ורענני — **תוצאה מצו… |
| 3331 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | הכרטיס מופיע **ראשון** בקבוצת «פרופיל» — פתחו `/producer/dashboard/edit?group=profile` — «שינוי שם העסק» לפני … |
| 3332 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | מצב ריק (אין בקשה פתוחה) — מוצג טופס עם «השם הנוכחי» לקריאה בלבד + «השם המבוקש» + «סיבה (אופציונלי)» |
| 3333 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | כפתור «שליחת בקשה לשינוי שם» **מושבת** כשהשדה ריק, וכשהשם המבוקש **זהה** לנוכחי (מופיעה השורה «השם המבוקש זהה … |
| 3334 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | אחרי שליחה — מוצג «הבקשה נשלחה. נעדכן אותך כשהשם החדש יאושר.» |
| 3336 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | רענון הדף → מצב ממתין: «בקשה לשינוי שם ממתינה לאישור» + השם המבוקש, **והטופס נעלם** (לא ניתן לשלוח בקשה שנייה) |
| 3337 | בעלת עסק — דשבורד → עריכה → «שינוי שם העסק» | ‏`/en/producer/dashboard/edit?group=profile` — כל המחרוזות באנגלית, אין מפתח גולמי על המסך |
| 3341 | אדמין — `/admin/name-change-requests` | הקישור «בקשות שינוי שם» מופיע בניווט הצד |
| 3342 | אדמין — `/admin/name-change-requests` | מצב 0 בקשות → «אין בקשות ממתינות.» |
| 3343 | אדמין — `/admin/name-change-requests` | מצב בקשה 1 ומצב כמה בקשות — כל שורה מציגה «שם נוכחי» מול «שם מבוקש» זה לצד זה (בנייד זה נערם, בדסקטופ שתי עמוד… |
| 3344 | אדמין — `/admin/name-change-requests` | «אישור» → השם הציבורי **מתעדכן**, השורה יורדת מהתור |
| 3345 | אדמין — `/admin/name-change-requests` | «דחייה» → השם הציבורי **נשאר**, השורה יורדת מהתור |
| 3346 | אדמין — `/admin/name-change-requests` | בקשה שכבר הוכרעה לא ניתנת להכרעה שנייה (השרת מחזיר 409) |
| 3370 | בעלת עסק — `/producer/dashboard` בסטטוס `draft` | **התא שמגיעים אליו רק במרוץ** — לפתוח את «אישור», ואז לגרום לפרופיל להיטען מחדש כשהוא כבר לא שלם (למחוק תמונה … |
| 3371 | בעלת עסק — `/producer/dashboard` בסטטוס `draft` | **ביטול** («לא») מחזיר לכפתור פעיל — לא לכפתור תקוע מושבת. |
| 3379 | מד ההשלמה — שש שורות, לא חמש (16/08) | האחוז מחושב על שישה פריטים (חסר אחד = 83%, לא 80%) |
| 3396 | אדמין — `/admin/producers` | טיוטות **לא** מופיעות בתור ברירת המחדל |
| 3397 | אדמין — `/admin/producers` | פילטר «טיוטות» מציג אותן |
| 3398 | אדמין — `/admin/producers` | `?status=all` מציג הכל, כולל טיוטות |
| 3399 | אדמין — `/admin/producers` | עסק שנשלח לבדיקה מופיע בתור, ותאריך ההגשה הוא רגע השליחה |
| 3403 | RTL ורוחבים | 375 ו-1440: הבאנר, התגים והכפתורים לא נחתכים ולא גולשים; שום `left/right` פיזי |
| 3404 | RTL ורוחבים | יעדי מגע ≥ 44px על «שליחה לבדיקה», «כן, שלחו» ו«לא» |
| 3430 | טופס האדמין | לעסק שיש לו **תאריך אבל לא מספר** — הבלוק נפתח מעצמו — תוצאה מצופה: התאריך גלוי בלי ללחוץ על ה-toggle (אחרת ער… |
| 3431 | טופס האדמין | שמירה → רענון → התאריך נטען חזרה — תוצאה מצופה: אותו תאריך, לא ריק |
| 3432 | טופס האדמין | ניקוי השדה ושמירה — תוצאה מצופה: התאריך נמחק (נשלח `null`), לא נשאר הערך הישן |
| 3438 | רשימת התזכורות | עסק שהתוקף שלו **ריק** — תוצאה מצופה: **לא** ברשימה (ריק = «טרם נלכד», לא «פג») |
| 3439 | רשימת התזכורות | עסק שהתוקף שלו **כבר עבר** — תוצאה מצופה: לא ברשימה |
| 3440 | רשימת התזכורות | עסק בתוקף 31 יום — תוצאה מצופה: לא ברשימה; ב-30 יום בדיוק — כן ברשימה |
| 3441 | רשימת התזכורות | עסק **ממתין** (pending) בתוך החלון — תוצאה מצופה: לא ברשימה (הוא ממילא בתור הסקירה) |
| 3442 | רשימת התזכורות | הטלפון מוצג ממוסך בלבד (`***4567`) — תוצאה מצופה: המספר המלא לא מופיע בתשובה בשום שדה |
| 3466 | עריכת הרשימה — `/admin/settings` | הבלוק «רשימת בדיקה לפני אישור» מופיע בהגדרות — איך לבדוק: נכנסים כאדמין — תוצאה מצופה: 7 הסעיפים המקוריים, כל … |
| 3467 | עריכת הרשימה — `/admin/settings` | שינוי ניסוח + «שמירת הרשימה» → רענון — תוצאה מצופה: הניסוח החדש נטען חזרה |
| 3468 | עריכת הרשימה — `/admin/settings` | «שמירת הרשימה» → מופיע «נשמר ✓» → מקלידים אות אחת בכל שדה — תוצאה מצופה: ה-«נשמר ✓» **נעלם מיד** (אישור על עבו… |
| 3469 | עריכת הרשימה — `/admin/settings` | אותה בדיקה עם חיצי הסדר ועם «הוספת סעיף» — תוצאה מצופה: גם שם ה-«נשמר ✓» נעלם |
| 3470 | עריכת הרשימה — `/admin/settings` | חיצי ↑/↓ משנים סדר → שמירה → רענון — תוצאה מצופה: הסדר החדש נשמר |
| 3471 | עריכת הרשימה — `/admin/settings` | מרוקנים שדה סעיף — תוצאה מצופה: כפתור השמירה מושבת + «לכל סעיף חייב להיות טקסט.» |
| 3472 | עריכת הרשימה — `/admin/settings` | «הוספת סעיף» → ניסוח → שמירה — תוצאה מצופה: הסעיף החדש מופיע בסוף, ומופיע גם בזרימת הסקירה |
| 3473 | עריכת הרשימה — `/admin/settings` | **אין כפתור מחיקה בשום מקום** — תוצאה מצופה: רק תיבת «בשימוש». זה מכוון: המפתח הזר הוא `ON DELETE RESTRICT`, ו… |
| 3474 | עריכת הרשימה — `/admin/settings` | מבטלים «בשימוש» לסעיף → שמירה — תוצאה מצופה: הסעיף נשאר **גלוי בהגדרות** (מעומעם, «הופסק השימוש») אך **נעלם מז… |
| 3478 | זרימת הסקירה — `/admin/producers` | שורת עסק ב-`pending` נושאת «רשימת בדיקה לפני אישור» — תוצאה מצופה: כפתור מתקפל עם מונה |
| 3479 | זרימת הסקירה — `/admin/producers` | שורת עסק **מאושר** — תוצאה מצופה: אין רשימת בדיקה כלל |
| 3480 | זרימת הסקירה — `/admin/producers` | לפני פתיחה, לעסק שכבר יש לו סימונים — תוצאה מצופה: המונה אומר **`(?/7)`**, לא `(0/7)`. הסימונים נטענים רק בפתי… |
| 3481 | זרימת הסקירה — `/admin/producers` | פותחים את השורה — תוצאה מצופה: המונה הופך למספר אמיתי, והסעיפים שסומנו קודם מגיעים **מסומנים מראש** |
| 3482 | זרימת הסקירה — `/admin/producers` | מסמנים סעיף → **רענון מלא של הדף** → פותחים שוב — תוצאה מצופה: הסימון עדיין שם (זה כל ההבדל מ-MEH-1396) |
| 3483 | זרימת הסקירה — `/admin/producers` | מסמנים ומיד מבטלים — תוצאה מצופה: אחרי רענון הסעיף אינו מסומן (שורה נמחקת, לא מסומנת כ-false) |
| 3484 | זרימת הסקירה — `/admin/producers` | מסמנים סעיף, יוצאים ליום אחר, חוזרים ומסמנים אותו שוב — תוצאה מצופה: **החותמת המקורית נשמרת** (סימון חוזר לא מ… |
| 3485 | זרימת הסקירה — `/admin/producers` | מקפלים את השורה ופותחים שוב — תוצאה מצופה: הסימונים לא נעלמו בקיפול |
| 3486 | זרימת הסקירה — `/admin/producers` | מאשרים עסק עם **אפס** סימונים — תוצאה מצופה: האישור **עובר**. אם נחסם — תקלה |
| 3487 | זרימת הסקירה — `/admin/producers` | כשאין אף סעיף פעיל (מבטלים «בשימוש» לכולם) — תוצאה מצופה: «אין סעיפים פעילים. אפשר להוסיף בהגדרות.» ולא רשימה … |
| 3492 | תיק בדיקה (chunk 4) | לחיצה על «העתקת שם העסק» — תוצאה מצופה: «הועתק ✓» למשך 2 שניות; הדבקה במאגר נותנת את השם המדויק |
| 3493 | תיק בדיקה (chunk 4) | כל הקישורים החיצוניים — תוצאה מצופה: נפתחים בלשונית חדשה, לא מנווטים מהאדמין |
| 3494 | תיק בדיקה (chunk 4) | לעסק בלי אתר ובלי אינסטגרם — תוצאה מצופה: «אין אתר או אינסטגרם», לא שורה ריקה |
| 3495 | תיק בדיקה (chunk 4) | לעסק בלי תמונות — תוצאה מצופה: «אין תמונות» |
| 3496 | תיק בדיקה (chunk 4) | **במסך 375** — תוצאה מצופה: «מספר: …» ו«תוקף: …» נקראים **בלי גלילה אופקית** של הטבלה |

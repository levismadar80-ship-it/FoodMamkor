...
## Status: 🚧 CONVERTING — checkpoint 4 (mobile near-me) pushed, CI-verify pending
...
| /map near-me mobile (MEH-970 2-lite) | 4 of 4 items → PW (mocked geolocation: grant-near / grant-far-25km / deny) | `frontend/e2e/flows/manual/map-near-me.spec.ts` | 🟡 pushed, **CI-verify pending** — the SANDBOX cannot mount /map under mobile emulation (the pre-existing `05-map-navigation` mobile spec fails locally too; environment limitation of the substituted chromium-1194 build, NOT a spec defect — CI runs mobile map specs green on every PR). Verification signal = the PR's e2e-qa-report; if CI fails the spec → fixme + rework on the next event. |

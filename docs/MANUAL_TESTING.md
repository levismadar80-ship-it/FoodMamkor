...
- [ ] **ביקרת לאחרונה = מיני-כרטיסים** — צפי בכמה עסקים ואז פתחי `/producers` — **תוצאה מצופה:** רצועת "ביקרת לאחרונה" מציגה מיני-כרטיסים (תמונה + שם + עיר), לא כפתורי טקסט. עסק בלי תמונה מציג עלה + "מהמקור". לחיצה מובילה לעמוד העסק. אין היסטוריה → הרצועה מוסתרת.

...
## MEH-1385 — כרטיס "מאחורי העסק" בדשבורד (owner_bio + owner_photo_url) (20/07)

טופס עריכה חדש בדשבורד הבעלים (`/producer/dashboard/edit`, אחרי "תיאור העסק") שממלא את שדות סיפור-הבעלים ששרתו נחת ב-MEH-1335. שדה `owner_bio` נשמר דרך `PUT /producers/me`; התמונה נשמרת אטומית ב-`POST /upload/owner-photo` (בלי freemium gate).

- [ ] **מילוי ביו + שמירה** — היכנסי כבעלת-עסק → `/producer/dashboard/edit` → כרטיס "מאחורי העסק" → כתבי טקסט ב"כמה מילים עלייך" ושמרי → רענון — **תוצאה מצופה:** הטקסט נשמר; המונה מציג `{אורך}/300` (דיגיטים משמאל לימין).
- [ ] **תקרת 300 תווים** — הדביקי טקסט ארוך מ-300 תווים — **תוצאה מצופה:** נחתך ל-300 בהקלדה עצמה (לא בשמירה); המונה עוצר על 300/300.
- [ ] **העלאת תמונה** — הקישי "העלאת תמונה" ובחרי תמונה — **תוצאה מצופה:** תצוגה מקדימה עגולה מתמלאת; אחרי רענון התמונה נשארת (נשמרה אטומית); הכיתוב הופך ל"החלפת התמונה".
- [ ] **מצב ריק** — עסק בלי ביו ובלי תמונה — **תוצאה מצופה:** אווטאר-placeholder של דמות (UserCircle), CTA "העלאת תמונה", textarea עם placeholder מזמין, מונה 0/300 — כרטיס שלם ולא שבור.
- [ ] **GET ציבורי מחזיר** — אחרי מילוי ושמירה, פתחי `GET /producers/{id}` — **תוצאה מצופה:** `owner_bio` + `owner_photo_url` מופיעים (חשופים ב-ProducerDetailOut).
- [ ] **מובייל 375px** — הכרטיס במובייל — **תוצאה מצופה:** שורת התמונה (אווטאר + תווית + כפתור) נכנסת בלי גלישה; textarea + מונה + כפתור שמירה מיושרים RTL. צילומי self-QA: `qa-artifacts/MEH-1385/`.

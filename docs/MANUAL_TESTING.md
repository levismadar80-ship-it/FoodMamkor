...
- [ ] **בועת תג נסגרת בגלילה** — דף הבית / `/producers` → הקישי על תג בכרטיס עסק (למשל "מהדרין" או "גראס פד") → גללי את העמוד — **תוצאה מצופה:** הבועה נעלמת מיד. **לא** נשארת תלויה במקום קבוע על המסך בזמן שהתוכן זז מתחתיה
- [ ] **פאנל "+N" בדסקטופ נסגר בגלילה** — בדסקטופ (רוחב ≥1024px), כרטיס עם 3+ תגים → הקישי על "+N" → גללי — **תוצאה מצופה:** הפאנל נסגר
- [ ] **סיבוב מכשיר** — פתחי בועת תג במובייל וסובבי את המכשיר — **תוצאה מצופה:** הבועה נסגרת (לא נשארת במיקום ישן)
- [ ] **מגירה תחתונה במובייל לא הושפעה** — במובייל (<1024px), "+N" נפתח כמגירה תחתונה — **תוצאה מצופה:** המגירה מתנהגת כרגיל וצמודה לתחתית. זו התנהגות מכוונת של מגירה, לא באג
- [ ] **סגירה רגילה עדיין עובדת** — Esc, הקשה מחוץ לפאנל, והקשה חוזרת על התג — **תוצאה מצופה:** כולן סוגרות כמו קודם
...
- [ ] **פתוח עכשיו** — עסק שהגדיר חלון שכולל את השעה הנוכחית → דף הבית / `/producers` — **תוצאה מצופה:** מתחת לשורת העיר מופיעה שורה ירוקה "פתוח להזמנות · עד HH:MM", והשעה היא שעת הסגירה **האמיתית** של הטווח הנוכחי
- [ ] **סגור עכשיו** — אותו עסק, אחרי שעת הסגירה — **תוצאה מצופה:** הכרטיס נראה **בדיוק** כמו לפני העדכון. אין שורה, אין רווח שמור, אין "סגור"
- [ ] **בלי חלון בכלל** — עסק שלא הגדיר "מתי מקבלים הזמנות" — **תוצאה מצופה:** אין שורה. זהה למצב הסגור
- [ ] **בחופשה** — עסק בחופשה שיש לו חלון פתוח עכשיו — **תוצאה מצופה:** **אין** שורת הזמנות. חופשה גוברת — אסור שהכרטיס יזמין הזמנות בזמן שהעסק הודיע שהוא לא כאן
- [ ] **בין שני טווחים באותו יום** — עסק עם 09:00–13:00 ו-16:00–20:00, בשעה 14:30 — **תוצאה מצופה:** אין שורה (זה מצב סגור אמיתי, לא הפסקה שמוצגת כפתוח)
- [ ] **מובייל 375px** — כרטיס עם חלון פתוח — **תוצאה מצופה:** השורה נשברת לשתי שורות והשעה **נראית במלואה**. אסור שהשעה תיחתך ל-"עד …" — היא כל המידע שבשורה
- [ ] **השעה לא הפוכה** — בכל הרוחבים — **תוצאה מצופה:** "05:31" ולא "31:05"

...
- [ ] **פיני המפה** — פתחי `/map` — **תוצאה מצופה:** פין "בשר" מציג glyph בשר (Tabler, קו לבן על עיגול אדום `#c04040`), "שמנים" בקבוק שמן (עיגול `#e8a020`), "דבש" חלת-דבש מלאה בזהב `#C8821E`, "דגים" דג (FishSimple). הצבעים ללא שינוי מקודם; אין אימוג'י.
- [ ] **סלקטור הרשמת עסק** — `/register/producer` → שלב הקטגוריות — **תוצאה מצופה:** 6 הפופולריות (חלב וגבינות/לחמים/בשר/שמנים/ירקות/סבונים) מציגות glyph גיאומטרי ירוק; שאר הקטגוריות עם Leaf fallback כרגיל; הבחירה/סימון עובדים כרגיל.
- [ ] **כרטיסי עמוד הבית — ללא רגרסיה** — פתחי `/` וגללי לגריד הקטגוריות — **תוצאה מצופה:** הכרטיסים מציגים את הצילומים (MEH-1183) **כרגיל** — ה-glyphs לא החליפו אותם; אין שינוי ויזואלי בכרטיסים.
- [ ] **משפחה אחידה** — השוו את ה-glyphs זה מול זה בשורת המפה/סלקטור — **תוצאה מצופה:** משקל הקו אחיד (המצוירים לא עבים יותר מ-Phosphor); חלת-הדבש היחידה מלאה (Material) — נראית מעט כבדה יותר, מקובל.

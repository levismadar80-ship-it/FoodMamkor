...
### מה השער מגן עליו — רישום בעלות הדאטה (MEH-2145)

השער למעלה אוכף **תאריך**. הוא לא אומר דבר על השאלה שקדמה לו: לאיזו עובדה יש
שדה קנוני אחד, ומי רשאי לכתוב אליו. זה [docs/DATA_OWNERSHIP.md](./DATA_OWNERSHIP.md)
— טבלה של עובדה → שדה קנוני → עורך יחיד → aliases מיושנים → כרטיס contract.

השניים משלימים ואינם חופפים, וכדאי לשמור על ההפרדה:

| | מה הוא אוכף | המנגנון |
|---|---|---|
| שער ה-LEGACY (MEH-1857) | ש-overlap **ייגמר** בתאריך שהוצהר | מרקר בקוד + `legacy-expiry-check.sh` |
| רישום הבעלות (MEH-2145) | שדה שנסגר לכתיבת בעלת העסק **לא יחזור** בשקט | `backend/app/data_ownership.py` + `tests/test_data_ownership.py` |

הרישום הוא **fitness function**, לא מנעול: מותר להחזיר שדה — התנאי הוא שהעורך
שלו נשלח באותו PR, והשורה בטבלה זזה איתו. מה שאסור הוא שזה יקרה בלי ששמים לב.

**סגירת נתיב כתיבה אינה הוצאת עמודה משימוש.** ה-admin, ייבוא ה-XLSX וה-seeds
ממשיכים לכתוב לכל השדות ברישום, וערכים היסטוריים עדיין נקראים ומוגשים. מה
שנסגר הוא הנתיב היחיד שבו ה-API קיבל ערך ש**שום ממשק של בעלת עסק לא יכול היה
לייצר**, או ש**שום משטח צרכן לא היה מרנדר** — ערך שלא הייתה לה דרך לראות את
השפעתו.
...
> ### ✅ הכלל הזה נאכף עכשיו — `scripts/checks/alembic-head-guard.sh`
>
> **הפסקה שמעליה נכתבה ב-06/08. ב-07/08 סשן אחר כתב בדיוק את אותו regex ונפל
> בדיוק באותה מלכודת** — דיווח שני ראשים על שרשרת תקינה, על אותו קובץ `meh_1508`.
> מסמך מתעד לקח; הוא לא אוכף אותו. **כלל שצריך לזכור הוא כלל שנכשל.**
>
> לכן הלקח הפך לסקריפט תחת [`scripts/checks/`](../scripts/checks/README.md),
> שרץ אוטומטית ב-job **Repo guards** (בלי עריכת workflow):
>
> ```bash
> bash scripts/checks/alembic-head-guard.sh   # exit 1 על יותר מראש אחד
> ```
>
> * **סופר ב-AST, לא ב-regex** — docstring הוא ביטוי מחרוזת, לעולם לא צומת השמה,
>   כך שהדקוי הפרוזאי אינו יכול להטעות אותו. מעדיף `alembic` אמיתי כשהוא מיובא.
> * **ה-self-test רץ ראשון בכל הרצה** ומעוגן ל**קובץ אמיתי בריפו** (`meh_1508`),
>   לא ל-fixture סינתטי: הוא מוודא שהדקוי עדיין שם, ש-regex התמים **כן** נופל
>   עליו, ושה-AST **כן** מחלץ את ההורה. אם הקובץ יאבד את הצורה הזו — הסקריפט
>   נכשל בקול, במקום לבדוק כלום בשקט.
> * **מוכיח שהוא יודע לומר "יותר מאחד"** — fixture דו-ראשי מובנה בתוכו; בלי זה
>   סקריפט שמחזיר תמיד `1` היה עובר כל בדיקה.
>
> `uv run alembic heads` נשאר הסמכות ל-release. הסקריפט הוא השער שתופס את המקרה
> לפני שמישהי סופרת ידנית.
...
**כשכותבים את האודיט הזה, `check-bash-safety.sh` יחסום heredoc שמכיל `drop_table` — זו התנהגות תקינה. כתבו את הקובץ ב-Write/Edit, לא ב-`cat <<EOF`.** ה-hook חוסם **הרצה** ואינו יודע להבחין בין פקודה הרסנית לפרוזה שמצטטת אותה; כלי הקבצים הוא המסלול הנכון, לא עקיפה. **אין לרכך את ה-hook בשביל נוחות תיעוד** — בריפו שבו לפקודה הרסנית אחת יש יום רע אחד, זהו כיוון שגוי, וכלל 32 ממילא אוסר על CC להסיר אילוץ. (נתקל בפועל בהכנת release #2, בכתיבת גוף PR שציטט את הטבלה שלמעלה.)

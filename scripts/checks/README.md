...
| [`builder-model-guard.sh`](./builder-model-guard.sh) | A commit with no `Builder-Model:` trailer, or one whose value **equals** the adversarial reviewer's pin (maker == checker). Inspects the last *authored* commit, walking first parents past sync merges. The bot exemption is **dependabot only** — `github-actions[bot]`, which `vrt-update.yml` commits as, is deliberately NOT exempt, because widening it would shrink the checked set (rule 32). `--self-test` pins that split: three synthetic cases plus a fourth that reads the committer identity out of the **real** `vrt-update.yml` rather than restating it, so it tracks the workflow instead of a fixture. An unknown flag exits 2 — before that it was silently ignored, so `--self-test` exited 0 having tested nothing. | MEH-1668 |

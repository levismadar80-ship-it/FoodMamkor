...
| [`docs-ordering-guard.sh`](./docs-ordering-guard.sh) | A PR declaring `Describes-PRs: #N` where `#N` is **not yet on the base branch** — documentation landing ahead of the code it documents (rule 31b). **Warn-only.** Reports three states, never two: `MERGED` / `NOT MERGED` / `UNVERIFIABLE`, the last for a clone too shallow to answer — `repo-guards` checks out at depth 1, so collapsing those two nulls would make every warning meaningless. `--self-test` reconstructs the 16/08 incident (which cannot be reproduced live — #2974 has since landed), and its last case runs the matcher against this repo's **real** staging log rather than a fixture. | MEH-2103 |
...
| [`builder-model-guard.sh`](./builder-model-guard.sh) | A commit with no `Builder-Model:` trailer, or one whose value **equals** the adversarial reviewer's pin (maker == checker). Inspects the last *authored* commit, walking first parents past sync merges. The bot exemption is **two-condition, not a list of identities**: `dependabot[bot]` is exempt on identity alone, while `github-actions[bot]` — which every workflow in the repo can commit as — is exempt **only** when the commit's diff is confined to `frontend/e2e/visual/*-snapshots/`. A bot commit touching anything else is still checked. An **unreadable** diff is never exempt: `repo-guards` checks out at `fetch-depth: 1`, where a depth-1 clone of a merge commit carries no parents and `git diff-tree` exits 0 with empty output, so the parent is fetched on demand and a still-undeterminable diff fails closed. `--self-test` covers the identity split, the path classifier (including the empty-diff vacuous-truth trap and a lookalike directory), the **live baseline files in the working tree**, `commit_changed_paths` against a real throwaway git repo, the fail-closed path for an unreachable commit, and the committer identity read out of the real `vrt-update.yml`. Every case is **depth-independent** — an earlier version anchored on two commit SHAs, whose objects are absent under `fetch-depth: 1`, and the preflight then failed every PR. An unknown flag exits 2. The self-test runs as a preflight on every normal invocation, so it is gated by CI rather than being a test nobody runs. | MEH-1668 · MEH-2223 |
| [`claude-md-line-cap-guard.sh`](./claude-md-line-cap-guard.sh) | `CLAUDE.md` exceeding its own documented **80-line cap** — the file sets the cap, names `wc -l` as the instrument, warns *"never from memory"*, and then relied on memory to enforce it (architectural smell #2). Uses `wc -l` **on purpose**, because that is the command CLAUDE.md tells you to run and a guard reporting a different number would be a second owner for one fact; the one-line blind spot that creates — a final line with no trailing newline — is detected and reported rather than left to surface at 81. Warns at ≥78. `AGENTS.md` is a **symlink** (MEH-490) so it is counted once, but a real file there is warned about rather than skipped: the mirror having become two files that can disagree is exactly the MEH-1801 misreading. `--self-test` pins the boundary in **both** directions (79/80/81 — an off-by-one in `over_cap` reds exactly one case), both directions of the mirror predicate, the wc-undercount, and fail-loud on an unreadable file; it runs as a **preflight on every invocation** (builder-model-guard precedent), so CI gates the guard's own correctness. | MEH-1929 |

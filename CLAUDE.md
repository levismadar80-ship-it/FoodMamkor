...
**Known bug pattern — "zero check runs on a PR" (absent, not red, not skipped).** Mechanism: `on: pull_request` workflows run against `refs/pull/N/merge`. Head stale or conflicting with base → GitHub cannot compute that ref → **no runs are created at all**.
- **Diagnose FIRST** (cheap, no API quota): `git ls-remote <repo> refs/pull/N/merge`. **ABSENT = this is the cause.**
- **Fix:** `git fetch origin && git merge origin/<base>` on the branch, then push. Runs appear immediately. If the merge conflicts, it needs manual resolution — no push will help.
- **NOT the cause** (both hypotheses tested and refuted 26/07): app-token suppression; draft status; quota/billing.
- **Corollary:** `mergeable_state` and 405 messages are unreliable in **BOTH** directions — one true reading after four false. Settle it with a local dry-run merge, never the field.

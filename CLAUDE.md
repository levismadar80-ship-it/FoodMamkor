...
- **"נבדק בנייד" does not apply to `/admin` screens outside the approvals queue** — they are desktop-only by decision (A-narrow, MEH-2070), and carry a banner saying so. Same class of DoD exception as tests-only / docs-only / CI YAML. `/admin/producers` and below IS mobile-ready and still needs the check.
- **Hebrew in Python: no rule forbids it, and one rule mandates part of it.** No line in `.claude/rules/` bans Hebrew in Python *comments* — 15/51 alembic revisions already use it (measured, not assumed) — and Hebrew in `detail=` is **required** by [backend/app/routers/CLAUDE.md](./backend/app/routers/CLAUDE.md). An automated reviewer flags both anyway; it was declined 5× on one PR (MEH-1823). Don't re-litigate.
...
**CC sandbox cannot reach Railway URLs** (re-verified 2026-08-15, MEH-2090: `curl …up.railway.app/health`, `railway.app`, `backboard.railway.app` **and `vercel.com`** all → `CONNECT tunnel failed, response 403`, while `api.github.com` / `pypi.org` / `mehamakor.co.il` return 200 in the same run — a proxy policy, not a broken probe). Egress is blocked by CC's envoy proxy with `x-deny-reason: host_not_allowed`. **`staging.mehamakor.online` is doubly blocked** — it answers `302` to `vercel.com/sso-api` (Vercel deployment protection) and that host is itself denied, so lifting either one alone does not open it; production (`mehamakor.co.il`) IS reachable for GET. Smoke verification, curl-based reachability tests, or any direct Railway/staging hits must run from user's local machine (Git Bash on Windows + curl) or via CI. CC must NOT claim smoke verification it cannot perform — instead state explicitly: "smoke verification deferred to user (CC sandbox limitation, see MEH-2090)". Reference: anthropics/claude-code#19087.

...
**Open the PR BEFORE pushing, and never as a draft — otherwise the required gates may never report.** A PR opened as a draft can end up with **zero** check runs (not "skipped" — none at all, including the ungated `Branch name gate` / `DO-NOT-MERGE marker gate`), and flipping draft→ready does **not** reliably fire a run even though `pr-checks.yml:27` lists `ready_for_review` (MEH-1582). With no `CI gate`/`Deploy gate` context reported, the PR is unmergeable and auto-merge can never fire — and rule 30 forbids CC from pushing a commit just to re-trigger it, so it becomes a Sapir-only unblock. Safe order: create the PR **non-draft**, then push (a `synchronize` event from a normal `git push` does fire). Also note the *separate* skip-green trap: heavy jobs are path-gated (`changes` job, `pr-checks.yml:146-155` — `backend: ['backend/**','tests/**']`) **and** draft-gated (`:281`), and a skipped leg still aggregates to `success` — so a green aggregator on a draft means "nothing ran". Source: 26/07 — PR #2180 (docs-only, opened draft) sat with 0 check runs; PR #2191 (opened non-draft) fired immediately.

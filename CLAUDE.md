...
- **Auto-deploy on merge to `main` or `staging`.** Vercel ships the frontend via its native GitHub integration; [`.github/workflows/deploy.yml`](./.github/workflows/deploy.yml) runs `railway redeploy` via the Railway CLI (auth: `RAILWAY_TOKEN`) against the matching environment so the backend can't lag behind. Setup: [docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md) → "GitHub Actions auto-deploy".
